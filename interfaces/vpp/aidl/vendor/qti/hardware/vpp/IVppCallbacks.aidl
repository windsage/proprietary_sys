...
import vendor.qti.hardware.vpp.FlushDone;
import vendor.qti.hardware.vpp.ReconfigDone;
import vendor.qti.hardware.vpp.VppBuffer;
import vendor.qti.hardware.vpp.VppError;

@VintfStability
interface IVppCallbacks {
     ...
     * @input       void
     *
     * @param out      VPP_OK          if the event was received successfully
     *                 VPP_ERR         if any error
     */
    VppError eventDrainDone();
     ...
     *                 VPP_ERR         if any error
     */
    VppError eventError(in VppError eError);
     ...
     *                 VPP_ERR         if any error
     */
    VppError eventFlushDone(in FlushDone stFlushDone);
     ...
     *                 VPP_ERR         if any error
     */
    VppError eventReconfigDone(in ReconfigDone stReconfigDone);
     ...
     *
     * @param out      VPP_OK         if the input buffer is returned successfully
     *                 VPP_ERR        if any error
     */
    VppError inputBufferDone(in VppBuffer stBuf);
     ...
     * @input       stBuf     Returned buffer
     *
     * @param out      VPP_OK         if the output buffer is returned successfully
     *                 VPP_ERR        if any error
     */
    VppError outputBufferDone(in VppBuffer stBuf);
}
