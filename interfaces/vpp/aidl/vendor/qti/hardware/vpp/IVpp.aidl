...
import vendor.qti.hardware.vpp.IVppCallbacks;
import vendor.qti.hardware.vpp.VppBuffer;
import vendor.qti.hardware.vpp.VppCodecType;
import vendor.qti.hardware.vpp.VppCtrlAie;
import vendor.qti.hardware.vpp.VppCtrlAis;
import vendor.qti.hardware.vpp.VppCtrlCade;
import vendor.qti.hardware.vpp.VppCtrlCnr;
import vendor.qti.hardware.vpp.VppCtrlDi;
import vendor.qti.hardware.vpp.VppCtrlEar;
import vendor.qti.hardware.vpp.VppCtrlFrc;
import vendor.qti.hardware.vpp.VppCtrlMeas;
import vendor.qti.hardware.vpp.VppCtrlQbr;
import vendor.qti.hardware.vpp.VppCtrlSplitScreen;
import vendor.qti.hardware.vpp.VppCtrlTnr;
import vendor.qti.hardware.vpp.VppError;
import vendor.qti.hardware.vpp.VppMode;
import vendor.qti.hardware.vpp.VppPort;
import vendor.qti.hardware.vpp.VppPortParam;
import vendor.qti.hardware.vpp.VppRequirements;
import vendor.qti.hardware.vpp.VppPriority;

@VintfStability
interface IVpp {
    // Adding return type to method instead of out param VppError eResult since there is only one return value.
    /**
     * !
     ...
     *
     * @param out      VPP_OK          if vpp transitions to inited state.
     *                 VPP_ERR         if there is an error
     */
    VppError closeSession();
     ...
     *
     *
     * @param out      VPP_OK          if the request was accepted
     ...
     *                 VPP_ERR         if the request can not be accepted
     */
    VppError flush(in VppPort ePort);

    // Adding return type to method instead of out param VppRequirements reqRet since there is only one return value.
    /**
     * !
     ...
     * @param out      void
     */
    VppRequirements getRequirements(in VppPortParam stInputParam, in VppPortParam stOutputParam);
     ...
     *
     * @param out      VPP_OK          if no error
     *                 VPP_PENDING     if waiting for parameter on another port
     *                 VPP_ERR         if there was an error
     *
     */
    VppError initSession(in int u32Flags, in IVppCallbacks cb);
     ...
     *
     * @param out      VPP_OK          if vpp transitions to active state.
     *                 VPP_ERR         if there is an error
     */
    VppError openSession();
     ...
     *
     * @param out      VPP_OK          if there is no error with accepting the buffer
     *                 VPP_ERR         if there is an error
     */
    VppError queueBuf(in VppPort ePort, in VppBuffer stBuf);
     ...
     *                 VPP_ERR         if the request can not be accepted
     */
    VppError reconfigure(in VppPortParam stInputParam, in VppPortParam stOutputParam);
     ...
     *
     * @param out      VPP_OK      if no error with accepting the control parameters
     *                 VPP_ERR     if there is an error
     */
    VppError setCtrlAie(in VppCtrlAie stCtrlAie);
     ...
     *                 VPP_ERR     if there is an error
     */
    VppError setCtrlAis(in VppCtrlAis stCtrlAis);
     ...
     *                 VPP_ERR     if there is an error
     */
    VppError setCtrlCade(in VppCtrlCade stCtrlCade);
     ...
     *                 VPP_ERR     if there is an error
     */
    VppError setCtrlCnr(in VppCtrlCnr stCtrlCnr);
     ...
     *                 VPP_ERR     if there is an error
     */
    VppError setCtrlDi(in VppCtrlDi stCtrlDi);
     ...
     *                 VPP_ERR     if there is an error
     */
    VppError setCtrlEar(in VppCtrlEar stCtrlEar);
     ...
     *                 VPP_ERR     if there is an error
     */
    VppError setCtrlFrc(in VppCtrlFrc stCtrlFrc);
     ...
     *                 VPP_ERR     if there is an error
     */
    VppError setCtrlMeas(in VppCtrlMeas stCtrlMeas);
     ...
     *                 VPP_ERR     if there is an error
     */
    VppError setCtrlQbr(in VppCtrlQbr stCtrlQbr);
     ...
     *                 VPP_ERR     if there is an error
     */
    VppError setCtrlSplitScreen(in VppCtrlSplitScreen stCtrlSplitScreen);
     ...
     *                 VPP_ERR     if there is an error
     */
    VppError setCtrlTnr(in VppCtrlTnr stCtrlTnr);
     ...
     *
     * @param out      VPP_OK      if no error with accepting the mode
     *                 VPP_ERR     if there is an error
     */
    VppError setCtrlVppMode(in VppMode eMode);
     ...
     *                 VPP_ERR         if there was an error
     */
    VppError setParameter(in VppPort ePort, in VppPortParam stParam);
     ...
     *
     * @param out      VPP_OK          if the property was accepted and stored
     *                 VPP_ERR         if the property was not accepted
     */
    VppError setVidPropCodec(in VppCodecType eCodec);
     ...
     *                 VPP_ERR         if the property was not accepted
     */
    VppError setVidPropNonRealTime(in int bNonRealtime);
     ...
     *                 VPP_ERR         if the property was not accepted
     */
    VppError setVidPropOperatingRate(in int u32OperatingRate);

    // Adding return type to method instead of out param VppError eResult since there is only one return value.
    /**
     * !
     * @brief       Synchronous. Sets session priority.
     *
     * @description Indicate to the VPP Service the priority of a session, which
     *              may be used by blocks to manage processing. The property will
     *              be stored in the context, available for any blocks that may
     *              need it.
     *
     * @input       ePriority        The priority level of the session. Will only
     *                               have effect on use cases that support it.
     *
     * @param out      VPP_OK          if the property was accepted and stored
     *                 VPP_ERR         if the property was not accepted
     */
    VppError setVidPropPriority(in VppPriority ePriority);
