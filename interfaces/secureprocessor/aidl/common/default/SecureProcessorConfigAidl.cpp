...
using ::aidl::vendor::qti::hardware::secureprocessor::config::ConfigTag;
using ::aidl::vendor::qti::hardware::secureprocessor::config::ConfigType;
...
SecureProcessorConfigAidl::SecureProcessorConfigAidl(uint32_t entryLimit,
                                             uint32_t dataLimit) {
    mVersion = ((MAJOR << 16) | MINOR);
    mConfigBuffer = _allocateConfigBuffer(entryLimit, dataLimit);
}

SecureProcessorConfigAidl::SecureProcessorConfigAidl(void *buffer) {
...
SecureProcessorConfigAidl::~SecureProcessorConfigAidl() {
    {
...
void* SecureProcessorConfigAidl::releaseAndGetBuffer() {
    void* buffer = nullptr;
...
uint32_t SecureProcessorConfigAidl::getVersion() const {
    std::lock_guard<std::mutex> lock(mLock);
    return mVersion;
}

void SecureProcessorConfigAidl::clear() {
    std::lock_guard<std::mutex> lock(mLock);
...
bool SecureProcessorConfigAidl::isEmpty() const {
    std::lock_guard<std::mutex> lock(mLock);
    return _isEmpty();
}

uint32_t SecureProcessorConfigAidl::getSize() const {
    std::lock_guard<std::mutex> lock(mLock);
    return _getSize();
}

uint32_t SecureProcessorConfigAidl::entryCount() const {
    std::lock_guard<std::mutex> lock(mLock);
    return _entryCount();
}

bool SecureProcessorConfigAidl::checkEntry(uint32_t tag) const {
    bool present = false;
...
status_t SecureProcessorConfigAidl::addEntry(uint32_t tag,
                      const uint8_t *data,
                      uint32_t data_count) {
...
ConfigEntry SecureProcessorConfigAidl::getEntry(uint32_t tag) {
    std::lock_guard<std::mutex> lock(mLock);
    return _getEntry(tag);
}

ConfigEntry SecureProcessorConfigAidl::getEntryByIndex(uint32_t index) {
    std::lock_guard<std::mutex> lock(mLock);
    return _getEntryByIndex(index);
}

/* Private functions */
uint32_t SecureProcessorConfigAidl::_computeSize(uint32_t entryLimit,
                                             uint32_t dataLimit) {
...
void SecureProcessorConfigAidl::_initConfigBuffer(void *buffer,
                                              uint32_t size,
                                              uint32_t entryLimit,
                                              uint32_t dataLimit) {
...
ConfigHeader *SecureProcessorConfigAidl::_allocateConfigBuffer(uint32_t entryLimit,
                                                           uint32_t dataLimit) {
    uint32_t size = 0;
    void *buffer = nullptr;
...
bool SecureProcessorConfigAidl::_validateConfigBuffer(void *buffer) {
...
ConfigBufferEntry *SecureProcessorConfigAidl::_getBufferEntries(ConfigHeader *configBuffer) const {
    return (ConfigBufferEntry *) (((uint8_t *) configBuffer) + configBuffer->entry.start);
}

uint8_t *SecureProcessorConfigAidl::_getBufferData(ConfigHeader *configBuffer) const {
    return (((uint8_t *) configBuffer) + configBuffer->data.start);
}

uint8_t SecureProcessorConfigAidl::_getDataTypeSize(uint8_t type) {
   uint8_t size = 0;
...
status_t SecureProcessorConfigAidl::_addEntry(uint32_t tag,
                                          uint8_t type,
                                          const void *data,
                                          uint32_t data_count) {
    if (!data || !data_count) {
...
void SecureProcessorConfigAidl::_createBufferEntry(uint32_t tag,
                                               uint8_t type,
                                               const void *data,
                                               uint32_t data_count,
                                               uint32_t sizeBytes) {
...
void SecureProcessorConfigAidl::_updateBufferEntry(uint32_t tag,
                                               const void *data,
                                               uint32_t data_count,
                                               uint32_t sizeBytes) {
    ConfigBufferEntry *entries = _getBufferEntries(mConfigBuffer);
...
status_t SecureProcessorConfigAidl::_copyConfigBuffer(ConfigHeader *dest,
                                                  ConfigHeader *src) {
    size_t copySize = 0;
    status_t status = OK;
...
uint32_t SecureProcessorConfigAidl::_getEntryIndex(uint32_t tag) {
    ConfigBufferEntry *entries = _getBufferEntries(mConfigBuffer);
    for (uint32_t i = 0; i < _entryCount(); i++) {
        if (entries[i].tag == tag) {
            return i;
...
bool SecureProcessorConfigAidl::_isEmpty() const {
    return (_entryCount() == 0);
}

uint32_t SecureProcessorConfigAidl::_getSize() const {
    return ((mConfigBuffer == nullptr) ? 0 : mConfigBuffer->size);
}

uint32_t SecureProcessorConfigAidl::_entryCount() const {
    return ((mConfigBuffer == nullptr) ? 0 : mConfigBuffer->entry.used);
}

ConfigEntry SecureProcessorConfigAidl::_getEntry(uint32_t tag) {
    ConfigEntry entry = {};
...
ConfigEntry SecureProcessorConfigAidl::_getEntryByIndex(uint32_t index) {
    ConfigEntry entry = {};
