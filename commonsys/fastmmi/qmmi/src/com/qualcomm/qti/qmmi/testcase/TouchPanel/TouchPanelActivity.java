...
public class TouchPanelActivity extends BaseActivity {
    ...
    public ArrayList<EdgePoint> getTestPoint() {
        ...
        int pointMaxX = mRadius + mStep * ((mWidthPix - mRadius) / mStep);
        int pointMaxY = mRadius + mStep * ((mHightPix - mRadius) / mStep);

        for (int x = mRadius; x <= mWidthPix; x += mStep) {
            for (int y = mRadius; y <= mHightPix; y += mStep) {
                if (x == mRadius || y == mRadius || x == pointMaxX || y == pointMaxY){
                    list.add(new EdgePoint(x, y, false));
                    LogUtils.logi("Edge Point: x: " + x + ",y: " + y);
                }
            }
        }
        return list;
