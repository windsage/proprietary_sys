...
public class CaseManager implements CaseOpCallback, ControllerInterface {
    ...
    private void initCasesAndResult(TestMode type) {
        ...
        switch(mCurMode){
        case PCBA:
            mTestCaseList = ignoreDisabledCases(pcbaCasesList);
            break;
        case ASSEMBLY:
            mTestCaseList = ignoreDisabledCases(assmebleCasesList);
            break;
        case UI:
            mTestCaseList = ignoreDisabledCases(uiCasesList);
            break;
        }
    ...
    private List<TestCase> ignoreDisabledCases(List<TestCase> casesList){
        List<TestCase> returnList = new ArrayList<TestCase>();
        for (TestCase testCase : casesList) {
            if (testCase != null && testCase.getEnable()) {
                TestCase tcTarget = new TestCase();
                tcTarget.copy(testCase);
                returnList.add(tcTarget);
            }
        }
        return returnList;
    }
    ...
    public int runCase(TestCase testCase) {
        ...
        /**update UI*/
        CaseServiceManager.getInstance().sendMsgForMainActivityHander(CaseServiceManager.MSG_DIAG_UPDATE_TESTCAST_RESULT, null);
    ...
    public boolean diagLoadCases(File cfg) {
        ...
        for (TestCase testCase : testCaseList) {
            ...
            if (testCase != null && testCase.getEnable()) {
                ...
                mTestCaseMap.put(tcTarget.getName(), tcTarget);
            } else {
    ...
    public int selectSequence(String cfgFileName) {
        ...
        /**update UI*/
        CaseServiceManager.getInstance().sendMsgForMainActivityHander(CaseServiceManager.MSG_DIAG_UPDATE_TESTCAST_LIST, null);
        return 0;
    }
    ...
    public int runSingleTest(String caseName) {
        ...
        if (testCase.getAuto()){
            ...
            CaseServiceManager.getInstance().sendMsgForMainActivityHander(CaseServiceManager.MSG_DIAG_START_TESTCAST, intent);
        }
    ...
    public int doUtility(String params) {
        ...
        if (aidlManager == null) {
            LogUtils.loge("AIDL Not support");
            HidlManager hidlManager = HidlManager.getInstance();
            if (hidlManager == null) {
                LogUtils.loge("HIDL Not support");
                return -1;
            }
            ...
            switch (utilityId) {
                ...
                case DiagCommand.UTILITY_ID_WLAN:
                    if (utilityOps == DiagCommand.OPS_ON) {
                        aidlManager.wifiEnable(true);
                        aidlManager.runFtmDaemonApp("-n;-dd");
                    } else {
                        aidlManager.wifiEnable(false);
                        aidlManager.stopFtmDaemonApp();
                    }
                    break;
