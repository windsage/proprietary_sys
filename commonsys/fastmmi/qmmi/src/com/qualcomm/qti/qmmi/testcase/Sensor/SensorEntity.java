...
public class SensorEntity {
    ...
    private final static HashMap<Integer,Integer> RESULT_LENGTH_MAP = new HashMap<Integer,Integer>();
    {
        ...
        RESULT_LENGTH_MAP.put(secondary_TYPE_ACCELEROMETER, NUM_3);
        RESULT_LENGTH_MAP.put(Sensor.TYPE_MAGNETIC_FIELD, NUM_3);
        RESULT_LENGTH_MAP.put(Sensor.TYPE_GYROSCOPE, NUM_3);
        RESULT_LENGTH_MAP.put(secondary_TYPE_GYROSCOPE, NUM_3);
        ...
        RESULT_LENGTH_MAP.put(TYPE_PPG, NUM_6);
    }
    ...
    private final static String PPG_STRING_TYPE_KEY = "ppg";
    private final static int TYPE_PPG = 65572;
    private static int mPPGSensorType = TYPE_PPG;

    protected final static int TYPE_UNSUPPORT = -99999;
    private final static int QTI_DUAL_SENSOR_TYPE_BASE  = 268369920;
    //second accelerometer type
    private final static int secondary_TYPE_ACCELEROMETER = QTI_DUAL_SENSOR_TYPE_BASE + Sensor.TYPE_ACCELEROMETER;
    private final static String secondary_ACCELEROMETER_STRING_TYPE_KEY = "secondary_accelerometer";
    //second gyroscope type
    private final static int secondary_TYPE_GYROSCOPE = QTI_DUAL_SENSOR_TYPE_BASE + Sensor.TYPE_GYROSCOPE;
    private final static String secondary_GYROSCOPE_STRING_TYPE_KEY = "secondary_gyroscope";
        ...
        if ( typeName.equalsIgnoreCase(HALL_EFFECT_STRING_TYPE_KEY)) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (s.getStringType() != null && s.getStringType().contains(HALL_EFFECT_STRING_TYPE_KEY)) {
                    ...
                    LogUtils.logi( "typeName : " + HALL_EFFECT_STRING_TYPE_KEY);
                    return mHallEffectSensorType;
                }
            }
            return TYPE_HALL_EFFECT;
        } else if( typeName.toLowerCase().startsWith(secondary_ACCELEROMETER_STRING_TYPE_KEY)) {
            LogUtils.logi( "typeName : qti accelerometer");
            return secondary_TYPE_ACCELEROMETER;
        } else if( typeName.toLowerCase().startsWith(secondary_GYROSCOPE_STRING_TYPE_KEY)) {
            LogUtils.logi( "typeName : qti gyroscope");
            return secondary_TYPE_GYROSCOPE;
        } else if( typeName.equalsIgnoreCase(PPG_STRING_TYPE_KEY)) {
            LogUtils.logi( "typeName : qti ppg");
            return TYPE_PPG;
        }
