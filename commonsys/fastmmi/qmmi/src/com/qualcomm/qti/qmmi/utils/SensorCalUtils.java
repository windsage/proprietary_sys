...
import vendor.qti.hardware.sensorscalibrate.V1_0.ISensorsCalibrate; //HIDL

public class SensorCalUtils {

    public static ISensorsCalibrate mSensorsHidlCalibrate;  //HIDL
    public static vendor.qti.hardware.sensorscalibrate.ISensorsCalibrate mSensorsAidlCalibrate;  //AIDL
    ...
    public static String calibrateSensor(Sensor sensor, int testType) {
        ...
        try {
            mSensorsAidlCalibrate = vendor.qti.hardware.sensorscalibrate.ISensorsCalibrate.Stub.asInterface(ServiceManager.waitForService("vendor.qti.hardware.sensorscalibrate.ISensorsCalibrate/default")); //AIDL
            if (mSensorsAidlCalibrate != null) {
                LogUtils.logi("Sensorscalibrate AIDL");
                return mSensorsAidlCalibrate.SensorsCal(sensor.getType(), (byte) testType);
            } else {
                mSensorsHidlCalibrate = ISensorsCalibrate.getService(true);  //HIDL
                if (mSensorsHidlCalibrate != null) {
                    LogUtils.logi("Sensorscalibrate HIDL");
                    return mSensorsHidlCalibrate.SensorsCal(sensor.getType(), (byte) testType);
                }
            }
        } catch (Exception e) {
            LogUtils.loge(e.toString());
        } catch (Throwable t) {
            // catch NoClassDefFoundError if both AIDL & HIDL not exist
