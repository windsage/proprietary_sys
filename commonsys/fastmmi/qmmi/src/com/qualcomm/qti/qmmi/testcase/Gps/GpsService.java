...
public class GpsService extends BaseService {
    ...
    private boolean existProvider = false;
    ...
    public int stop(TestCase testCase) {
        if (mLocationManager != null  && existProvider == true) {
    ...
    public int run(TestCase testCase) {
        ...
        if (mLocationManager != null) {
            try {
                ProviderProperties pps = mLocationManager.getProviderProperties(LocationManager.GPS_PROVIDER);
                existProvider = (pps != null)? true : false;
            } catch (IllegalArgumentException e) {
                existProvider = false;
                LogUtils.loge("provider gps does not exist!");
            }
            ...
            if (existProvider){
                ...
                updateView(mTestCase.getName(), getApplicationContext().getString(R.string.gps_provider_not_exist));
                return -1;
            }
        }
