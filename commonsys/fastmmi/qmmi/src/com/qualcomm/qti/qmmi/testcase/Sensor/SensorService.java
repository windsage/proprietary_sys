    ...
    private String parseData(String caseName, SensorEvent event){
        ...
        switch(entity.getValuesNum()){
            ...
            case SensorEntity.NUM_6 :
                boolean validateSix = false;

                if ( values.length < SensorEntity.NUM_6) {
                    result = this.getResources().getString(R.string.sensor_wrong_data_lenght);
                } else {
                    String resultSix = this.getResources().getString(R.string.sensor_result_six);

                    String valueSix1 = formatData(values[0]);
                    String valueSix2 = formatData(values[1]);
                    String valueSix3 = formatData(values[2]);
                    String valueSix4 = formatData(values[3]);
                    String valueSix5 = formatData(values[4]);
                    String valueSix6 = formatData(values[5]);
                    validateSix = ( valueSix1.equals("") && valueSix2.equals("") && valueSix3.equals("")
                        && valueSix4.equals("") && valueSix5.equals("") && valueSix6.equals(""))? false:true;

                    result = String.format(resultSix, valueSix1, valueSix2, valueSix3,valueSix4, valueSix5, valueSix6);

                    // Append Track Data
                    newData.append(valueSix1).append(TRACK_SEP_CONTENT)
                        .append(valueSix2).append(TRACK_SEP_CONTENT)
                        .append(valueSix3).append(TRACK_SEP_CONTENT)
                        .append(valueSix4).append(TRACK_SEP_CONTENT)
                        .append(valueSix5).append(TRACK_SEP_CONTENT)
                        .append(valueSix6).append(TRACK_SEP_ITEMS);

                    // Append sensor value for Indication View in UI, just for 3 DOF value
                    result = result + ":" + event.sensor.getType() + ":"  + values[0] + ":" + values[1] + ":" + values[2] + values[3] + ":" + values[4] + ":" + values[5];
                }
