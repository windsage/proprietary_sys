...
sp<IFactoryHidl> mFactoryClientHidl = nullptr; //hidl
std::shared_ptr<IFactoryAidl> mFactoryClientAidl = nullptr; //aidl
...
static int erase_all_files() {
    int res;
    if (mFactoryClientAidl.get() == NULL) {
        MMI_ALOGE("IFactory AIDL client null");
        IResultTypeHidl resultHidl = mFactoryClientHidl->eraseAllFiles(path);
        MMI_ALOGE("erase_all_files, resultHidl=%d\n", resultHidl);
        res = (int)resultHidl;
    } else {
        IResultTypeAidl resultAidl;
        mFactoryClientAidl->eraseAllFiles(path, &resultAidl);
        MMI_ALOGE("erase_all_files, resultAidl=%d\n", resultAidl);
        res = (int)resultAidl;
    }
...
static int dir_list_filename_to_file(const char *filename) {
    ...
    if (mFactoryClientAidl.get() == NULL) {
        MMI_ALOGE("IFactory AIDL client null");
        IResultTypeHidl resultHidl = mFactoryClientHidl->dirListFileNameToFile(path, name);
        MMI_ALOGE("dir_list_filename_to_file, resultHidl=%d\n", resultHidl);
        res = (int)resultHidl;
    } else {
        IResultTypeAidl resultAidl;
        mFactoryClientAidl->dirListFileNameToFile(path, name, &resultAidl);
        MMI_ALOGE("dir_list_filename_to_file, resultAidl=%d\n", resultAidl);
        res = (int)resultAidl;
    }
...
PACKED void *ftm_read_file(void *req_pkt, uint16 pkt_len) {
    ...
    ReadFileResultAidl fileResultAidl;
    ReadFileReqAidl readReqAidl;
    ReadFileReqHidl readReqHidl;
    ...
    if (mFactoryClientAidl.get() == NULL) {
        readReqHidl.offset = req->offset;
        readReqHidl.max_size = req->max_size;
        readReqHidl.file_name = pFileName;
    } else {
        readReqAidl.offset = req->offset;
        readReqAidl.max_size = req->max_size;
        readReqAidl.file_name = pFileName;
    }
    ...
    if (mFactoryClientAidl.get() == NULL) {
        MMI_ALOGE("IFactory AIDL client null");
        mFactoryClientHidl->readFile(path, readReqHidl, [&](auto& readResult){
            rsp->sftm_header.cmd_code = 75;
            rsp->sftm_header.sub_sys_id = DIAG_SUBSYS_FTM;
            rsp->sftm_header.sub_sys_cmd_code = FTM_AP_C;
            ...
            IResultTypeHidl result = readResult.result_type;
        ...
        mFactoryClientAidl->readFile(path, readReqAidl, &fileResultAidl);
            rsp->sftm_header.cmd_code = 75;
            rsp->sftm_header.sub_sys_id = DIAG_SUBSYS_FTM;
            rsp->sftm_header.sub_sys_cmd_code = FTM_AP_C;
            rsp->sftm_header.ftm_cmd_id = FTM_AP_READ_FILE;
            rsp->sftm_header.ftm_data_len = 0;
            rsp->sftm_header.ftm_rsp_pkt_size = 0;
            rsp->file_size = fileResultAidl.file_size;
            IResultTypeAidl result = fileResultAidl.result_type;
            rsp->ftm_error_code = (int)result;
            rsp->offset = fileResultAidl.offset;
            rsp->size = fileResultAidl.size;
            MMI_ALOGI("ftm_read_file: size=%d", fileResultAidl.data.size());
            memcpy(rsp->Data, fileResultAidl.data.data(), sizeof(rsp->Data));
    }
...
PACKED void *ftm_write_file(void *req_pkt, uint16 pkt_len) {
    ftm_common_response *rsp = NULL;
    WriteFileReqAidl writeReqAidl;
    WriteFileReqHidl writeReqHidl;
    ...
    if (mFactoryClientAidl.get() == NULL) {
        writeReqHidl.file_name = filename;
        writeReqHidl.append_data = *(unsigned char *) (pReq + WRITE_APPEND_DATA_OFFSET(filename));
        writeReqHidl.i_size = *(unsigned short *) (pReq + WRITE_ISIZE_OFFSET(filename));
        writeReqHidl.data = (char*)pData;
    } else {
        writeReqAidl.file_name = filename;
        writeReqAidl.append_data = *(unsigned char *) (pReq + WRITE_APPEND_DATA_OFFSET(filename));
        writeReqAidl.i_size = *(unsigned short *) (pReq + WRITE_ISIZE_OFFSET(filename));
        writeReqAidl.data = (char*)pData;
    }
    ...
    if (mFactoryClientAidl.get() == NULL) {
        MMI_ALOGE("IFactory AIDL client null");
        IResultTypeHidl resultHidl = mFactoryClientHidl->writeFile(path,writeReqHidl);
        MMI_ALOGE("ftm_write_file, resultHidl=%d\n", resultHidl);
        res = (int)resultHidl;
    } else {
        IResultTypeAidl resultAidl;
        mFactoryClientAidl->writeFile(path, writeReqAidl, &resultAidl);
        MMI_ALOGE("ftm_write_file, resultAidl=%d\n", resultAidl);
        res = (int)resultAidl;
    }
...
int diag_init(void) {
    ...
    const std::string instance = std::string() + IFactoryAidl::descriptor + "/default";
    auto factoryBinder = ::ndk::SpAIBinder(AServiceManager_waitForService(instance.c_str()));

    mFactoryClientAidl = IFactoryAidl::fromBinder(factoryBinder);

    if (mFactoryClientAidl.get() == NULL) {
        MMI_ALOGE("IFactory AIDL client null");

        mFactoryClientHidl = IFactoryHidl::getService();
        if (mFactoryClientHidl == NULL) {
            MMI_ALOGE("IFactory HIDL client null");
            return -1;
        }
    }
