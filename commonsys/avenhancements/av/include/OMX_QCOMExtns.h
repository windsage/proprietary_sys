 ...
 * which can be set using the OMX_SetParameter macro.
 * This parameter is a 'write only' parameter (Current value
 * cannot be queried using OMX_GetParameter macro).
 */
#define OMX_QCOM_EXTN_REGISTER_MMAP     "OMX.QCOM.index.param.register_mmap"

/**
 * This parameter must be set only 'after' populating a port with a buffer
 ...
 * When IL client specifies an LTR frame with its identifier
 * to the encoder, if the specified LTR frame can not be located by the
 * encoder in its LTR list, the encoder issues this error event to IL
 * client to notify the failure of LTRUse config.
 */
...
struct OMX_QCOM_PLATFORMPRIVATE_EXTN
{
    ...
    /** Type of extensions should match an entry from corresponding enum
    */
...
/**
 * Following is the enum for color formats supported on
...
{
/** Codecs support which are not listed in OMX 1.1.x
 *  spec
 *   */
...
/**
 * This structure describes the parameters corresponding to the
 * VIDEO_CONFIG_INTRAPERIODTYPE extension. This parameter can be set
...
/**
 * This structure describes the parameters corresponding to the
 * VIDEO_CONFIG_ULBUFFEROCCUPANCYTYPE extension. This parameter can be set
...
/**
 * This structure describes the parameters corresponding to the
 * VIDEO_CONFIG_RANDOMINTRAREFRESHTYPE extension. This parameter can be set
...
#define QOMX_VIDEO_HIGH_PERF_OPERATING_MODE    (0x7FFFFFFF)
