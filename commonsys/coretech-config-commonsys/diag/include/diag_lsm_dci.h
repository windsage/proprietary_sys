...
#endif
/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
# Copyright (c) 2012-2013, 2015-2016, 2020-2021, 2023 Qualcomm Technologies, Inc.
# All Rights Reserved.
...
/* Configure the  peripheral Diag's DCI TX mode to Streaming, Circular, or
  ...
  The input parameters are:
  1. proc - MSM or MDM
  2. peripheral - peripheral id to set the mode
  3. mode - mode to set
  4. low_wm_val - low wm val to set
  5. high_wm_val - high wm val to set */
int diag_dci_configure_buffering_mode(int proc, int peripheral, int mode, int low_wm_val, int high_wm_val);

/* Request  the peripheral to drain its dci Tx buffering pool immediately.
  ...
  The input parameters are:
  1. proc - MSM or MDM
  2. peripheral - peripheral id to drain the data */
int diag_dci_drain_buffer(int proc, int peripheral);

/* Register a callback to receive the DCI buffering mode status
   change updates.
   The inout parameters are:
   1. proc - MSM or MDM
   2. callback function ptr */
