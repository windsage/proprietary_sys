...
# Copyright (c) 2007-2011, 2014, 2016, 2018, 2020-2021, 2024 Qualcomm Technologies, Inc.
# All Rights Reserved.
...
/*===========================================================================
FUNCTION DIAG_QUERY_MAX_LOG_PKT_SIZE
DESCRIPTION
	This function returns the maximum supported log packet size, that
	can be submitted to diag. Log packets are secure and non-secure,
	each having a different header length, log_size_query_specific is used
	to fetch size of log packet specfic to a log_code.

	For Unix clients the params are not used and can be any value.
PARAM
	log_size_query_specific 	True if requesting log_code based packet size
					False for providing a generic log packet size(works for all log_codes)

	log_code			log code of the log packet item whose max packet size is queried
					if log_size_query_specific is False, then log_code is not used
RETURN VALUE
	Maximum supported log packet size.
*/
