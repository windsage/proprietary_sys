/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
Copyright (c) 2015-2021, 2024 by Qualcomm Technologies, Inc.
All Rights Reserved.
...
pthread_mutex_t qsr4_clear_entry_mutex;
...
static int process_qsr4_db_file_list_response(int peripheral_type, int peripheral)
{
	...
	int number_of_files, valid_guid = 0;
	...
	if (buf_ptr[0] == DIAG_BAD_CMD_F) {
		...
		if (rsp->status == 0) {
			...
			for (i = 0; i < number_of_files; i++) {
				valid_guid = 0;
				for (j = 0; j < GUID_LEN; j++) {
					if (rsp->info[i].guid[j]) {
						valid_guid = 1;
						break;
					}
				}
...
static void diag_clear_entry(qsr4_db_file_list* entry)
{
	pthread_mutex_lock(&qsr4_clear_entry_mutex);
	if (entry->buf) {
		free(entry->buf);
		entry->buf = NULL;
	}
	...
	entry->fd = -1;
	pthread_mutex_unlock(&qsr4_clear_entry_mutex);
}
...
{
	if(!db_thread_initialized || qshrink_kill_thread)
		return;
...
int create_diag_qshrink4_db_parser_thread(unsigned int peripheral_mask,unsigned int device_mask)
{
	...
	if (qsr_state == QSR4_INIT) {
                pthread_mutex_init(&qsr4_clear_entry_mutex, NULL);
	...
	} else if (qsr_state == QSR4_THREAD_CREATE) {
...
void diag_kill_qshrink4_threads(void)
{
	...
	if (qsr_state == QSR4_KILL_THREADS) {
		...
		pthread_mutex_destroy(&qsr4_clear_entry_mutex);
		...
		DIAG_LOGD("diag:In %s finished killing qshrink4 threads\n", __func__);
	}
}
...
void diag_cleanup_qshrink4_threads(void)
{
	...
	if (qsr_state == QSR4_CLEANUP) {
		...
		DIAG_LOGD("diag:In %s finished cleaning up qshrink4 db\n", __func__);
	}
}
