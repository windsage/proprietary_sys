...
int use_qmdl2_v2 = 0;
int use_qmdl2_v2_hdlc_disable = 0;
...
unsigned int write_buf_flush_timeout = 0;
...
pthread_cond_t read_thread_exit_cond;
pthread_mutex_t read_thread_exit_mutex;
volatile int read_thread_exit = STATUS_RDTHRD_CLR;
...
int diag_has_remote_device(uint16 *remote_mask)
{
	...
	ret = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_REMOTE_DEV, remote_mask, sizeof(uint16));
	if (!ret)
...
static void diag_signal_feature_threads(unsigned int state)
{
	if (state == THREADS_KILL) {

		qsr_state = QSR4_KILL_THREADS;
		...
		diag_kill_feature_setup_threads();

		key_info_status = THREADS_KILL;
		diag_kill_key_info_threads();

	} else if (state == THREADS_CLEANUP) {

		qsr_state = QSR4_CLEANUP;
		diag_cleanup_qshrink4_threads();

		key_info_status = THREADS_CLEANUP;
		diag_kill_key_info_threads();
	}
...
void flush_buffer(int signal)
{
	...
	diag_signal_feature_threads(THREADS_KILL);
	flush_log = 1;
	diag_lsm_kill = 1;
	diag_kill_comm_read();
	...
	diag_signal_feature_threads(THREADS_CLEANUP);
...
static void parse_data_for_command_rsp(uint8* ptr, uint32 count_received_bytes, int index, int *update_count)
{

	if (diag_feature_state)
		parse_data_for_diag_features_rsp(ptr, count_received_bytes, index, update_count);
...
void process_diag_payload(int num_bytes_read)
{
	...
	} else if (type == TIMESTAMP_SWITCH_TYPE) {
		...
		while (dci_bytes_processed < num_bytes_read) {
			...
			while (read_bytes < dci_data_len) {
				...
				if (dci_client_tbl->data_signal_flag == ENABLE) {
					pid = getpid();
					if (sigqueue(pid, dci_client_tbl->data_signal_type, (union sigval)0))
						DIAG_LOGE("diag: dci: signal sending failed for pid: %d, errno: %d\n", pid, errno);
				}
...
{
	int time_stamp_switch_val = 0;

	pthread_mutex_lock(&timestamp_switch_mutex);
	time_stamp_switch_val = time_stamp_switch_rcvd;
	pthread_mutex_unlock(&timestamp_switch_mutex);
	return time_stamp_switch_val;
}
...
void  write_buf_cond_wait(pthread_cond_t *cond, pthread_mutex_t *mutex)
{
	...
	if (write_buf_flush_timeout > 0) {
		clock_gettime(CLOCK_REALTIME, &time);
		time.tv_sec += write_buf_flush_timeout;
		pthread_cond_timedwait(&cond, &mutex, &time.tv_sec);
	} else {
...
void *WriteToDisk (void *ptr)
{
	...
	while (1) {
		...
		if (!pools[curr_write].data_ready){
			write_buf_cond_wait(&(pools[curr_write].write_cond),
		...
		for (z = 0; z < NUM_PROC; z++) {
			if (buffer_init[z] == READY) {
				bytes_in_buffer = pools[curr_write].bytes_in_buff[z];
				...
				if (((count_written_bytes[z] + bytes_in_buffer >= max_file_size) ||
					(count_written_bytes[z] + bytes_in_buffer > MDLOG_WRITTEN_BYTES_LIMIT))) {
...
int write_qdss_header(int fd, int proc)
{
	...
	header.hdlc_data_type = 2;
	if (keys_stored(proc)) {
		key_info_size = get_keys_header_size(proc);
		if (diag_qdss_dyn_atid_support()) {
			header.version = 4;
			/*
			 * pair of hw accel type and corresponding atid
			 * each of one byte size (2 * sizeof(uint8))
			 */
			atid_payload_size = get_atid_payload_size();
		} else {
			header.version = 3;
		}
	}
	...
	if (header.version >= 4)
		header.header_length += atid_payload_size;
		...
		if(logging_mode == MEMORY_DEVICE_MODE ) {
			...
			if ((use_qmdl2_v2 || hdlc_disabled[type]) && write_qshrink_header[type]) {
				...
				if (use_qmdl2_v2) {
					...
					len = DIAG_QMDL2_HDR_MIN_LEN;

					/* Query for key on every new file */
					diag_get_secure_diag_info(type, FILE_TYPE_QMDL2);

					/* Update header to v3 if secure key info is available */
					...
					if (qmdl2_v2_header_inited[type] &&
						...
						/* Write qshrink4 info in qmdl2 header for version 2 and beyond */
						len += (qshrink4_data[type].guid_list_entry_count * GUID_LEN);

						ret = write(fd_dev, (const void*)&qshrink4_data[type], len);
						if (ret > 0)
							count_written_bytes[type] += ret;

						/* Write diag-id count info in qmdl2 header for version 2 and beyond */
						...
						/*
						 * Write the diagid guid mapping info in qmdl2 header for
					...
					/*
					 * Add secure key information after diagid guid mapping
					 * in qmdl2 header for version 3 and beyond
					 */
					if (qshrink4_data[type].version >= 3 && keys_stored(type)) {
...
int diag_set_masks(int proc_type, const char *mask_file)
{
	...
	uint32_t pkt_size = 0;
	FILE *read_mask_fp;
	const char *open_mask_file = NULL;

	*(int *)mask_buf = USER_SPACE_DATA_TYPE;
	count_mask_bytes = USER_SPACE_DATA_TYPE_SIZE;
			...
			if (count_mask_bytes < MASK_FILE_BUF_SIZE) {
				if ((index > 0) && (count_mask_bytes == 4)) {
					*(int *)(mask_buf + count_mask_bytes) =
									token_list[index];
					/*
					 * If packet is for proc other than MSM,
					 * extra 4 bytes will be used for token
					 */
					count_mask_bytes = 8;
					pkt_size += 4;
				}
				if (i > 3 && !flag) {
					...
					pkt_size += (payload + DIAG_NON_HDLC_HEADER_SIZE + USER_SPACE_DATA_TYPE_SIZE);
					flag = 1;
				}
				...
				if ((count_mask_bytes >= pkt_size) &&
				    (mask_buf[count_mask_bytes] == CONTROL_CHAR) && (i != 0)) {
#ifdef DIAG_DEBUG
				DIAG_LOGE("********************************** \n");
				for (i = 0; i <= count_mask_bytes; i++) {
					DIAG_LOGE("%2x ", mask_buf[i]);
...
static int diag_reinit_switch_logging(void)
{
	...
	int ret;
	unsigned int qdss_mask = 0;

	if (logging_mode == USB_MODE)
		return 0;

	qdss_mask = get_qdss_mask();
	if (qdss_mask)
	...
	params.peripheral_mask = (diag_peripheral_mask) ?
			((qdss_mask) ? diag_peripheral_mask | qdss_mask :
...
{
	int err, success, dev_idx, device_mask = 1;
	uint16 remote_mask = 0;
	struct diag_logging_mode_param_t params;
...
static int diag_reinit_callbacks(void)
{
	...
	for (i = 0; i < NUM_PROC; i++) {
		...
		if (ret) {
			DIAG_LOGE("diag: %s: Callback registration failed with err:%d for proc:%d\n", __func__, ret, i);
			return -1;
		}
	}
	return 0;
...
void diag_register_callback(int (*client_cb_func_ptr)(unsigned char *ptr, int len, void *context_data), void *context_data)
{
	...
	err = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_REGISTER_CALLBACK, &reg, sizeof(struct diag_callback_reg_t));
	if (err)
...
int diag_send_data(unsigned char buf[], int bytes)
{
	...
	bytes_written = diag_lsm_comm_write(diag_fd, buf, bytes);
...
static int diag_reset_diag_fd(void)
{
	...
	diag_fd = fd;
	DIAG_LOGD("diag: %s: diag_fd succesfully reset fd:%d\n", __func__, diag_fd);
...
static int diag_reset_connection(void)
{
	...
	diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_QUERY_MASK, NULL, 0);
	if (!do_mask_sync()) {
	...
	DIAG_LOGE("diag: %s: reset connection succesful\n", __func__);
   ...
   pthread_mutex_lock(&lsm_init_count_mutex);
	if(lsm_init_count == 0) {
		...
		if (diag_fd == DIAG_INVALID_HANDLE) {
			...
			pthread_cond_init(&read_thread_exit_cond, NULL);
			pthread_mutex_init(&read_thread_exit_mutex, NULL);
			...
			ret = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_QUERY_MASK, NULL, 0);
...
boolean Diag_LSM_DeInit(void)
{
	...
	if (diag_fd != DIAG_INVALID_HANDLE || try_restart) {
		...
		if (!deinit_rcvd && !sig_rcvd) {
			rt = pthread_cond_timedwait(&deinit_cond, &deinit_mutex, &time);
			if (rt == ETIMEDOUT)
				DIAG_LOGE("diag: %s: Timeout while waiting for read thread exit\n", __func__);
		}
		pthread_mutex_unlock(&deinit_mutex);
		...
		pthread_mutex_lock(&read_thread_exit_mutex);
		while (!(read_thread_exit & STATUS_RDTHRD_EXIT) || (!deinit_rcvd && !sig_rcvd) ) {
			rt = pthread_cond_timedwait(&read_thread_exit_cond, &read_thread_exit_mutex, &time);
			if (rt == ETIMEDOUT){
				DIAG_LOGE("diag: %s: Read thread exit timeout\n", __func__);
				break;
			}
		}
		pthread_mutex_unlock(&read_thread_exit_mutex);
		deinit_rcvd = 0;
		sig_rcvd = 0;
...
static void *CreateWaitThread(void* param)
{
	...
	do{
		if (flush_log) {
			...
			/* read might return 0 across suspend */
				...
				num_bytes_read = diag_lsm_comm_read();
				if (!num_bytes_read) {
					if (flush_log) {
						continue;
					} else {
					...
					if (errno == ECANCELED) { /* diag_lsm_kill received */
						continue;
					} else if (errno == ESHUTDOWN) { /* DEINIT_TYPE received */
						deinit_rcvd = 1;
						break;
					}
				}
	...
	pthread_mutex_lock(&read_thread_exit_mutex);
	pthread_cond_signal(&read_thread_exit_cond);
