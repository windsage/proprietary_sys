...
Copyright (c) 2007-2011, 2014-2015, 2016, 2020-2024 Qualcomm Technologies, Inc.
All Rights Reserved.
...
/**
 * This API will make use of secure log packet header by setting ENCRYPTION_FLAG bit zero
 * to indicate that packet is non-secure packet.
 */
static void*  alloc_secure_log_packet(log_code_type code, unsigned int length, uint32 data_type)
{
	secure_log_pkt_type *plog_pkt_ptr;
	log_header_type *phdr_ptr = NULL;
	uint32 ts_lo = 0, ts_hi = 0;
	void *return_ptr = NULL;

	plog_pkt_ptr = (secure_log_pkt_type *) DiagSvc_Malloc
			(DIAG_REST_OF_DATA_POS + sizeof(secure_log_pkt_type) + length, GEN_SVC_ID);

	if (plog_pkt_ptr != NULL) {
		byte* temp = (byte*)plog_pkt_ptr;
		diag_data* pdiag_data = (diag_data*) plog_pkt_ptr;
		pdiag_data->diag_data_type = data_type;
		temp += DIAG_REST_OF_DATA_POS;
		plog_pkt_ptr = (secure_log_pkt_type*)temp;

		memset((void*)plog_pkt_ptr, 0, sizeof(secure_log_pkt_type));
		plog_pkt_ptr->cmd_code = 0x9E;
		plog_pkt_ptr->version = SECURE_LOG_PKT_VERSION1;
		plog_pkt_ptr->diag_id = diag_id;
		plog_pkt_ptr->timestamp_type = 0;

		phdr_ptr = (log_header_type*) (temp + sizeof(secure_log_pkt_type));
		ts_get_lohi(&ts_lo, &ts_hi);
		phdr_ptr->ts_lo = ts_lo;
		phdr_ptr->ts_hi = ts_hi;
		phdr_ptr->len = (uint16)length;
		phdr_ptr->code = code;
		return_ptr = (void*)phdr_ptr;
	} else {
		DIAG_LOGE("diag: %s: Dropped log 0x%x of len %d due to lack of memory",
...
)
{
	...
	uint32 ts_hi = 0, ts_lo = 0;

	if (-1 == diag_fd || !log_inited) {
		return NULL;
	}

	if (length <= sizeof(log_header_type) || (length >= MAX_USER_PKT_SIZE)) {
		DIAG_LOGE("diag: %s: Request for allocation with invalid len: %u",
			__func__, length);
	} else if ((enabled_mask = log_status_mask(code)) != 0) {
		/* check if new log packet format selected */
		if (is_diagid_logging_format_selected())
			return alloc_secure_log_packet(code, length, enabled_mask);
...
void log_commit (void *ptr)
{
	if (ptr) {
		...
		if (is_diagid_logging_format_selected())
			offset = DIAG_REST_OF_DATA_POS + sizeof(secure_log_pkt_type);
		else
			offset = DIAG_REST_OF_DATA_POS + LOG_DIAGPKT_OFFSET;

		/* Set pointer to begining of diag pkt, not the log */
		phdr_ptr = (log_header_type *)ptr;
		ptr = ((byte *) ptr - offset); /* WM7 prototyping */

		if (-1 != diag_fd) {
			int NumberOfBytesWritten = 0;

			NumberOfBytesWritten = diag_lsm_comm_write(diag_fd, (const void*) ptr, offset + phdr_ptr->len);
...
boolean log_submit (void *ptr)
{
	...
	/* The header is common to all logs, and is always at the beginning of the packet */
	log_header_type *plog_ptr = (log_header_type *) ptr;
	uint32 enabled_mask = 0;
	uint32 log_header_size = 0;
	...
	if (plog_ptr && (diag_fd != -1) && log_inited) {
		...
		if (length > sizeof(log_header_type)) {
			if ((enabled_mask = log_status_mask(code)) != 0) {
				if (is_diagid_logging_format_selected())
					log_header_size = DIAG_REST_OF_DATA_POS + sizeof(secure_log_pkt_type);
				else
					log_header_size = DIAG_REST_OF_DATA_POS + FPOS (diag_log_rsp_type, log);

				diag_data* pdiag_data = (diag_data*)DiagSvc_Malloc(log_header_size + length, GEN_SVC_ID);
				if (pdiag_data != NULL) {
					secure_log_pkt_type *psec_log_ptr = NULL;
					...
					if (is_diagid_logging_format_selected()) {
						psec_log_ptr = (secure_log_pkt_type*)temp;
						memset((void *) psec_log_ptr, 0, sizeof(secure_log_pkt_type));
						psec_log_ptr->cmd_code = 0x9E;
						psec_log_ptr->version = SECURE_LOG_PKT_VERSION1;
						psec_log_ptr->diag_id = diag_id;
						psec_log_ptr->timestamp_type = 1;
						temp += sizeof(secure_log_pkt_type);
					} else {
						plog_pkt_ptr = (diag_log_rsp_type*)temp;
						plog_pkt_ptr->cmd_code = DIAG_LOG_F;
						plog_pkt_ptr->more = 0;
						plog_pkt_ptr->len  = (uint16)length;
						temp = (byte*)&plog_pkt_ptr->log;
					}

					memcpy ((void *) temp, (void *) ptr, length);

					NumberOfBytesWritten = diag_lsm_comm_write(diag_fd, (const void*) pdiag_data, log_header_size + length);
					if (NumberOfBytesWritten != 0) {
...
/*===========================================================================
  FUNCTION log_clear_mask

  DESCRIPTION
    clears the local copy of the log masks.

===========================================================================*/
void log_clear_mask(void)
{
  memset(log_mask, 0, LOG_MASK_SIZE);
}

/*===========================================================================
  FUNCTION log_clear_dci_mask

  DESCRIPTION
    clears the local copy of the dci log masks.
...
uint32 diag_query_max_log_pkt_size(boolean log_size_query_specific, log_code_type log_code)
{
	(void)log_size_query_specific;
	(void)log_code;

	/* pkt_type byte accounts to 4 bytes */
	return MAX_USER_PKT_SIZE - (sizeof(uint32));
}
