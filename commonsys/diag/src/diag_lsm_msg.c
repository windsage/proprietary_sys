...
void msg_send(const msg_const_type *const_blk)
{
	...
	write_len = diag_lsm_comm_write(diag_fd, (unsigned char*)pMsg, nLength);
	if (write_len) {
...
void msg_send_1(const msg_const_type *pconst_blk, uint32 xx_arg1)
{
	...
	if (is_diagid_logging_format_selected()) {
		msg_pkt = (msg_diagid_type *)(pMsg + DIAG_REST_OF_DATA_POS);
		args = msg_pkt->args;
	} else {
...
void msg_send_2(const msg_const_type *pconst_blk, uint32 xx_arg1, uint32 xx_arg2)
{
	...
	if (is_diagid_logging_format_selected()) {
		msg_pkt = (msg_diagid_type *)(pMsg + DIAG_REST_OF_DATA_POS);
		args = MSG_LARGE_ARGS(msg_pkt->args);
	} else {
		pTemp = (msg_ext_type *)(pMsg + DIAG_REST_OF_DATA_POS);
...
void msg_sprintf(const msg_const_type *const_blk, ...)
{
	...
	bool diag_id_logging;
	msg_diagid_type *new_msg = NULL;
	...
	diag_id_logging = is_diagid_logging_format_selected();

	/* Calculate # of arguments to ensure enough space is allocated. */
	if (diag_id_logging)
		int_cnt = sizeof(msg_desc_type) - FSIZ(msg_diagid_ext_store_type, const_data_ptr) + fmt_length + fname_length;
	else
	...
	if (diag_id_logging) {
		new_msg = (msg_diagid_type *)(pMsg + DIAG_REST_OF_DATA_POS);
		/*
		 * This function embedds the argument in the string itself. Hence the
		 * num_args is assigned 0
		 */
		new_msg->hdr.arg_info = 0;
		new_msg->desc = const_blk->desc;
		str = (char *)new_msg->args;
		/* Calculate the buffer left to copy the format string */
		fmt_len_available = total_allocated - (FPOS (msg_diagid_type, args) + fname_length);
	} else {
	 ...
	 */
	write_len = diag_lsm_comm_write(diag_fd, (unsigned char*)pMsg, (uint32)(str - (char *)pMsg));
	if (write_len) {
		DIAG_LOGE("Diag_LSM_Msg: Write failed in %s, bytes written: %d, error: %d\n",
			  __func__, write_len, errno);
	}
...
{
	msg_qsr_diagid_type *qsr_msg_pkt = NULL;
	byte* pMsg = NULL;
	const unsigned int num_args = 1;
	unsigned int nLength = 0;
	int write_len = 0;
	...
	if (is_diagid_logging_format_selected()) {
		qsr_msg_pkt = (msg_qsr_diagid_type *)(pMsg + DIAG_REST_OF_DATA_POS);
		qsr_msg_pkt->args[0] = xx_arg1;
	} else {
...
void qsr_msg_send_var(const msg_qsr_const_type *const_blk, uint32 num_args, ...)
{
	...
	if (is_diagid_logging_format_selected()) {
		qsr_msg_pkt = (msg_qsr_diagid_type *)(pMsg + DIAG_REST_OF_DATA_POS);
		args = MSG_LARGE_ARGS(qsr_msg_pkt->args);
	} else {
...
/*===========================================================================
  FUNCTION msg_clear_mask

  DESCRIPTION
   This function clears the msg masks data structure.

===========================================================================*/
void msg_clear_mask(void)
{
	/* this only works which msg_mask is statically declared and not malloced */
...
FUNCTION ALLOC_DIAGID_BASED_MSG

DESCRIPTION
   Prepares the buffer needed by msg_send*() to send diag id based msg.
   Allocates, fills in all data except arguments, and returns a pointer
   to the allocated message buffer.
...
DEPENDENCIES
   None
===========================================================================*/
static byte *diagid_msg_send_prep(const msg_const_type *const_blk, unsigned int num_args,
				unsigned int *pLength, uint64 timestamp, boolean ts_valid)
{
	msg_diagid_head_type *msg_head;
	...
	filename = msg_format_filename2(const_blk->fname);

	/* total number of bytes to be allocated, including dereferenced FileName and Format strings */
	alloc_len = DIAG_REST_OF_DATA_POS + FPOS (msg_diagid_type, args) +
			num_args * FSIZ (msg_diagid_type, args[0]) + strlen(filename) + 1 +
	...
	msg_head = (msg_diagid_head_type *) ((byte*)pMsg + DIAG_REST_OF_DATA_POS);
	memset((void*)msg_head, 0, sizeof(msg_diagid_head_type));

	fmt_pos = DIAG_REST_OF_DATA_POS + sizeof(msg_diagid_head_type) +
			sizeof(msg_desc_type) + num_args * FSIZ (msg_diagid_type, args[0]);

	pdiag_data = (diag_data*) pMsg;
	pdiag_data->diag_data_type = DIAG_DATA_TYPE_F3;

	msg_head->cmd_code = 0xA0;
	msg_head->version = DIAG_ID_MSG_PKT_VERSION_1;
	msg_head->diag_id = diag_id;
	msg_head->msg_type = 0; /* 0 for DIAG_EXT_MSG_F */
	msg_head->timestamp_type = MSG_TS_TYPE;
	msg_head->arg_info = (num_args & 0xF);
	if (num_args)
		msg_head->arg_info |= (0x4 << 4); /* argument size is 4 bytes */
	...
	if (ts_valid) {
		...
		memcpy((char *) (&(msg_head->ts_lo)), (char *) &(timestamp), 4);
		memcpy((char *) (&(msg_head->ts_hi)), ((char *) &(timestamp)) + 4, 4);
	} else {
		ts_get_lohi(&ts_lo, &ts_hi);
		msg_head->ts_lo = ts_lo;
		msg_head->ts_hi = ts_hi;
	}

	if (pLength)
		*pLength = alloc_len;

	memcpy((void *)((char *)(pMsg) + DIAG_REST_OF_DATA_POS + sizeof(msg_diagid_head_type)),
				(void *)&(const_blk->desc), sizeof (msg_desc_type));
	memcpy((void *)((char *)(pMsg) + fmt_pos), (void *)(const_blk->fmt), strlen(const_blk->fmt) + 1);
	memcpy((void *)((char *)(pMsg) + fmt_pos + strlen(const_blk->fmt) + 1), (void *)(filename), strlen(filename) + 1);
			   ...
			   unsigned int *pLength, uint64 timestamp, boolean ts_valid)
{
	...
	if (is_diagid_logging_format_selected())
		return diagid_msg_send_prep(const_blk, num_args, pLength, timestamp, ts_valid);
...
static byte* diagid_qsr_msg_send_prep(const msg_qsr_const_type *const_blk,
			unsigned int num_args, unsigned int *pLength)
{
	msg_qsr_diagid_type *qsr_msg = NULL;
	diag_data *pdiag_data = NULL;
	unsigned int alloc_len = 0;
	byte *pMsg = NULL;
	uint32 ts_lo;
	...
	alloc_len = DIAG_REST_OF_DATA_POS + FPOS(msg_qsr_diagid_type, args) +
		num_args * FSIZ(msg_qsr_diagid_type, args[0]);
	...
	qsr_msg = (msg_qsr_diagid_type *)((byte *)pMsg + DIAG_REST_OF_DATA_POS);
	memset((void*)qsr_msg, 0, sizeof(msg_qsr_diagid_type));

	qsr_msg->hdr.cmd_code = 0xA0;
	qsr_msg->hdr.version = DIAG_ID_MSG_PKT_VERSION_1;
	qsr_msg->hdr.diag_id = diag_id;
	qsr_msg->hdr.msg_type = MSG_TYPE_OPT_EXT_PKT;
	qsr_msg->hdr.timestamp_type = MSG_TS_TYPE;
	qsr_msg->hdr.arg_info = (num_args & 0xF); /* set argument number in lower nibble */
	if (num_args)
		qsr_msg->hdr.arg_info = (0x4 << 4); /* set argument size in upper nibble */
	/* get timestamp */
	ts_get_lohi(&ts_lo, &ts_hi);
	qsr_msg->hdr.ts_lo = ts_lo;
	qsr_msg->hdr.ts_hi - ts_hi;

	qsr_msg->desc.line = const_blk->desc.line;
	qsr_msg->desc.ss_id = const_blk->desc.ss_id;
	qsr_msg->desc.ss_mask = const_blk->desc.ss_mask;
	qsr_msg->msg_hash = const_blk->msg_hash;
			       ...
			       unsigned int num_args, unsigned int *pLength)
{
	...
	if (is_diagid_logging_format_selected())
		return diagid_qsr_msg_send_prep(const_blk, num_args, pLength);
...
static byte *diagid_msg_sprintf_prep(const msg_const_type *pconst_blk, unsigned int num_args,
					unsigned int* pLength)
{
	msg_diagid_ext_store_type *msg_prep = NULL;
	diag_data *pdiag_data = NULL;
	...
	alloc_len = DIAG_REST_OF_DATA_POS + FPOS(msg_diagid_ext_store_type, args) +
		num_args * FSIZ(msg_diagid_ext_store_type, args[0]);
	...
	msg_prep = (msg_diagid_ext_store_type *)((byte *)pMsg + DIAG_REST_OF_DATA_POS);
	memset((void*)msg_prep, 0, sizeof(msg_diagid_ext_store_type));

	ts_get_lohi(&ts_lo, &ts_hi);
	msg_prep->hdr.cmd_code = 0xA0;
	msg_prep->hdr.version = DIAG_ID_MSG_PKT_VERSION_1;
	msg_prep->hdr.diag_id = diag_id;
	msg_prep->hdr.msg_type = MSG_TYPE_EXT_PKT;
	msg_prep->hdr.timestamp_type = MSG_TS_TYPE;
	msg_prep->hdr.ts_lo = ts_lo;
	msg_prep->hdr.ts_hi = ts_hi;
	msg_prep->const_data_ptr = pconst_blk;
			      ...
			      unsigned int* pLength)
{
	...
	if (is_diagid_logging_format_selected())
		return diagid_msg_sprintf_prep(pconst_blk, num_args, pLength);
