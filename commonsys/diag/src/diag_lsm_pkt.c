...
Copyright (c) 2007-2011, 2013-2016, 2018, 2020-2024 Qualcomm Technologies, Inc.
All Rights Reserved.
...
static uint16 diagpkt_get_delayed_rsp_id_lsm(void)
{
	...
	err = diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_GET_DELAYED_RSP_ID, &delayed_rsp_id, sizeof(uint16));
	if (err) {
		delayed_rsp_id = 0;
...
boolean Diag_LSM_Pkt_DeInit(void)
{
	...
	boolean ret = TRUE;
	diag_dci_delayed_rsp_tbl_t *walk_ptr = NULL, *head_ptr = NULL;
	diag_dci_delayed_rsp_tbl_t *next = NULL;

	if (diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_COMMAND_DEREG, (void*)&client_id, sizeof(client_id)) != 0) {
	...
	for (walk_ptr = head_ptr->next; next = walk_ptr->next, walk_ptr && walk_ptr != head_ptr; walk_ptr = next)
		diag_dci_delete_delayed_rsp_entry(walk_ptr);

	return ret;
} /* Diag_LSM_Pkt_DeInit */

void __diagpkt_tbl_reg(const diagpkt_user_table_type *tbl_ptr, diag_cmd_reg_entry_t *entries)
{
	int num_entries = tbl_ptr->count;
	...
	if (diag_lsm_comm_ioctl(diag_fd, DIAG_IOCTL_COMMAND_REG, &reg_tbl, sizeof(reg_tbl))) {
		DIAG_LOGE("diag: Unable to register commands with the driver, error: %d\n", errno);
	} else {
		DIAG_LOGD("diag: Successfully registered commands with the driver\n");
	}
}

int diagpkt_reinit(void)
{
	diagpkt_user_table_type *tbl_ptr;
	int i;
	...
	for (i = 0; i < DIAGPKT_USER_TBL_SIZE; i++) {
		...
		tbl_ptr = diagpkt_user_table[i];

		entries = (diag_cmd_reg_entry_t *)malloc(sizeof(diag_cmd_reg_entry_t) * tbl_ptr->count);
		if (!entries) {
			DIAG_LOGE("diag: In %s, unable to create temporary memory for registration\n", __func__);
			pthread_mutex_unlock(&user_tbl_mutex);
			return -1;
		}
		__diagpkt_tbl_reg(tbl_ptr, entries);
		free(entries);
	}
	pthread_mutex_unlock(&user_tbl_mutex);
...
void diagpkt_delay_commit (void *pkt)
{
	...
	if (pkt_len > 0 && -1 != diag_fd) {
		...
		if (is_dci_pkt) {
			...
			num_bytes = diag_lsm_comm_write(diag_fd, temp, rsp_len);
			...
			num_bytes = diag_lsm_comm_write(diag_fd, (unsigned char*)pdiag_data, pkt_len + DIAG_REST_OF_DATA_POS);
		}
		if(num_bytes != 0) {
...
void diagpkt_commit (void *pkt)
{
	...
	if ((ret = diag_lsm_comm_write(diag_fd, (unsigned char*) temp, DIAG_REST_OF_DATA_POS + rsp_length)) != 0) {
		DIAG_LOGE("Diag_LSM_Pkt: Write failed in %s, bytes written: %d, error: %d\n", __func__, ret, errno);
		gPkt_commit_fail++;
	}
	DiagSvc_Free(temp, PKT_SVC_ID);
...
uint32 diag_query_max_cmd_req_size(void)
{
	/* pkt_type + Diag Non HDLC Header + 7E pkt termination byte accounts to 9 bytes */
	return DIAG_MAX_TX_PKT_SIZ - (sizeof(uint32) + DIAG_NON_HDLC_HEADER_SIZE + 1);
}

uint32 diag_query_max_cmd_rsp_size(void)
{
	/* pkt_type byte accounts to 4 bytes */
	return DIAG_MAX_RX_PKT_SIZ - sizeof(uint32);
}
