...
static unsigned long adpl_count_written_bytes;
...
uint8_t adpl_config_thread_exit = 0;
...
int in_wait_for_adpl_config_complete = 0;

int adpl_in_write = 0;
int adpl_kill_thread = 0;
int adpl_enabled = 0;
volatile int adpl_modem_down;
int dpl_version = 0;
...
void diag_adpl_destroy_resources(void)
{
	...
	pthread_cond_destroy(&adpl_config_cond);
...
int diag_adpl_init()
{
	...
	/****************************************************************
	 * Wait time is 20 sec while config thread configures   		*
	 ****************************************************************/

	gettimeofday(&now, NULL);
	time.tv_sec = now.tv_sec + 20000 / 1000;
	time.tv_nsec = now.tv_usec + (10000 % 1000) * 1000000;
	in_wait_for_adpl_config_complete = 1;
	pthread_mutex_lock(&adpl_config_mutex);
	rt = pthread_cond_timedwait(&adpl_config_cond, &adpl_config_mutex, &time);
	if (rt == ETIMEDOUT || adpl_kill_thread) {
		DIAG_LOGE("diag:%s time out while waiting on adpl configuration complete\n", __func__);
		pthread_mutex_unlock(&adpl_config_mutex);
		goto failure_case;
	}
	pthread_mutex_unlock(&adpl_config_mutex);
	in_wait_for_adpl_config_complete = 0;
...
retry_ioctl:
		...
		DIAG_LOGE("diag: In %s: ioctl called: agg_byte_limit: %d\n", __func__, agg_byte_limit);
		if (err) {
		...
		if (adpl_modem_down)
			adpl_modem_down = 0;
		DIAG_LOGE("In %s: adpl_modem_down: %d\n", __func__, adpl_modem_down);
		pthread_cond_signal(&adpl_config_cond);
...
void diag_kill_adpl_threads(void)
{
	...
	if (!adpl_enabled || !adpl_config_done)
		return;
	...
	if (in_wait_for_adpl_config_complete)
		pthread_cond_signal(&adpl_config_cond);
