...
#define DIAG_CMD_APPS_LOOPBACK_SYSTEM_TEST	0x022B
#define DIAG_CMD_APPS_STRESS_SYSTEM_TEST	0x022C
#define DIAG_CMD_APPS_DELAYED_RSP_SYSTEM_TEST	0x022D
...
static void *diagdiag_apps_stress_test(void *req_pkt, uint16 pkt_len)
{
	...
	rsp = diagpkt_subsys_alloc(DIAG_SUBSYS_DIAG_SERV, DIAG_CMD_APPS_STRESS_SYSTEM_TEST, pkt_len);
	if (rsp != NULL) {
...
static void *diagdiag_loopback_test(void *req_pkt, uint16 pkt_len)
{
	...
	rsp = (unsigned char *)diagpkt_subsys_alloc(DIAG_SUBSYS_DIAG_SERV,
						    DIAG_CMD_APPS_LOOPBACK_SYSTEM_TEST,
						    actual_len);
	if (rsp != NULL) {
		printf("test_diag: Allocated a response of size: %d\n", actual_len);
...
static void *diagdiag_apps_delayed_rsp_test(void *req_pkt, uint16 pkt_len)
{
	...
	rsp = diagpkt_subsys_alloc_v2(DIAG_SUBSYS_DIAG_SERV, DIAG_CMD_APPS_DELAYED_RSP_SYSTEM_TEST,
	...
	for (i = 1; i < (int)count; i++) {
		...
		rsp = diagpkt_subsys_alloc_v2_delay(DIAG_SUBSYS_DIAG_SERV, DIAG_CMD_APPS_DELAYED_RSP_SYSTEM_TEST,
...
static const diagpkt_user_table_entry_type diag_test_tbl[] = {
	{DIAG_CMD_APPS_STRESS_SYSTEM_TEST, DIAG_CMD_APPS_STRESS_SYSTEM_TEST, diagdiag_apps_stress_test},
	{DIAG_CMD_APPS_LOOPBACK_SYSTEM_TEST, DIAG_CMD_APPS_LOOPBACK_SYSTEM_TEST, diagdiag_loopback_test}
};

static const diagpkt_user_table_entry_type diag_test_tbl_delay[] = {
	{DIAG_CMD_APPS_DELAYED_RSP_SYSTEM_TEST, DIAG_CMD_APPS_DELAYED_RSP_SYSTEM_TEST, diagdiag_apps_delayed_rsp_test}
};
...
static void usage(char *progname)
{
	...
	printf("  -r, --repeat_test: Repeats init and deinit specified by argument\n");
	printf("  -w, --wait_time: wait time between init and deinit\n");
	printf("  -h, --help:    help\n");
...
static void parse_args(int argc, char **argv)
{
	...
	while ((command = getopt_long(argc, argv, "t:p:r:w:vh", longopts, NULL)) != -1) {
		switch (command) {
		case 't':
		...
		case 'p':
			...
			break;
		case 'r':
			repeat_test = atoi(optarg);
			if (repeat_test < 0) {
				printf("test_diag: Invalid repeat test iteration given, %d\n", repeat_test);
				exit(0);
			}
			break;
		case 'w':
			...
			if (wait_time < 0) {
				printf("test_diag: Invalid wait time given, %d\n", wait_time);
				exit(0);
			}
			break;
		case 'h':
...
int main(int argc, char *argv[])
{
	...
	do {
		...
		if (wait_time) {
			printf("test_diag: waiting for %d s before deinit\n", wait_time);
			sleep(wait_time);
		}
		Diag_LSM_DeInit();
		repeat_test--;
