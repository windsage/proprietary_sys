...
Copyright (c) 2013-2016, 2018-2021, 2024 Qualcomm Technologies, Inc.
All Rights Reserved.
...
struct dci_channel_info {
	...
	void (*dci_cmd_rsp_cb)(unsigned char *buf, int len, void *data_ptr);
};
...
static void parse_args(int argc, char **argv)
{
	...
	while ((command = getopt_long(argc, argv, "b:c:h", longopts, NULL))
			!= -1) {
		switch (command) {
			case 'c':
...
void diag_dci_buffering_mode_status_cb(int mode)
{
	DIAG_LOGE("diag: Received buffering mode status change notification:%d\n", mode);
}
void *dci_execute_func(void* data)
{
	int err, client_id;
	...
	if (mode) {
		diag_dci_register_for_buffering_mode_status(MSM, diag_dci_buffering_mode_status_cb);
		diag_dci_configure_buffering_mode(MSM, DIAG_MODEM_PROC, mode, 20, 80);
	}
	...
	if (mode) {
		sleep(20);
		diag_dci_drain_buffer(MSM, DIAG_MODEM_PROC);
	}
