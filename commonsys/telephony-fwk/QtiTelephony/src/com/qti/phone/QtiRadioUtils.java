...
class QtiRadioUtils {
    ...
    private static CellInfo convertHalCellInfo(
        ...
        switch (cellInfo.ratSpecificInfo.getTag()) {
            case vendor.qti.hardware.radio.qtiradio.QtiCellInfoRatSpecificInfo.gsm:
                vendor.qti.hardware.radio.qtiradio.CellInfoGsm gsm = cellInfo.ratSpecificInfo.getGsm();
            ...
            case vendor.qti.hardware.radio.qtiradio.QtiCellInfoRatSpecificInfo.lte:
                vendor.qti.hardware.radio.qtiradio.CellInfoLte lte = cellInfo.ratSpecificInfo.getLte();
    ...
    public static CellIdentityNr convertHalCellIdentityNr(
        ...
        SnpnInfo snpnInfo = null;
        CagInfo cagInfo = null;
        if (cid.snpnInfo != null) {
            snpnInfo = new SnpnInfo(cid.snpnInfo.nid, cid.snpnInfo.mcc, cid.snpnInfo.mnc,
                    cid.snpnInfo.operatorNumeric, cid.snpnInfo.signalStrength,
                    cid.snpnInfo.signalQuality);
        }
        if(cid.cagInfo != null) {
            cagInfo = new CagInfo(cid.cagInfo.cagName, cid.cagInfo.cagId,
                    cid.cagInfo.cagOnlyAccess, cid.cagInfo.cagInAllowedList);
        }
        return new CellIdentityNr(cid.cNr.pci, cid.cNr.tac, cid.cNr.nrarfcn, cid.cNr.bands,
                ...
                primitiveArrayToArrayList(cid.cNr.additionalPlmns), snpnInfo, cagInfo);
    }
    ...
    public static int convertToHalSearchTypeAidl(int searchType) {
        switch (searchType) {
            case NetworkScanRequest.SEARCH_TYPE_PLMN_AND_CAG:
                return vendor.qti.hardware.radio.qtiradio.SearchType.PLMN_AND_CAG;
            case NetworkScanRequest.SEARCH_TYPE_PLMN_ONLY:
    ...
    public static int getAccessMode(NetworkScanRequest networkScanRequest) {
        return networkScanRequest.getAccessMode();
    }

    public static int getSearchType(NetworkScanRequest networkScanRequest) {
        return QtiRadioUtils.convertToHalSearchTypeAidl(networkScanRequest.getSearchType());
    }
    ...
    public static boolean isCneAidlAvailable() {
        if (isBoardApiLevelMatched()) {
            try {
                return ServiceManager.isDeclared(CNE_FACTORY_SAIDL_SERVICE_NAME);
    ...
    public static boolean initIFactoryAidl() {
        IBinder binder = ServiceManager.waitForService(CNE_FACTORY_SAIDL_SERVICE_NAME);
        if (binder == null) {
            Log.e(LOG_TAG, "initIFactoryAidl failed");
            return false;
        }
        if (vendor.qti.data.factoryservice.IFactory.Stub.asInterface(binder) == null) {
            Log.e(LOG_TAG, "Get binder for IFactory StableAIDL failed");
            return false;
        } else {
            Log.d(LOG_TAG, "Connected to IFactory StableAIDL service ... ");
            return true;
        }
    }

    public static void registerForNotifications(@NonNull String name,
            @NonNull IServiceCallback callback) {
        Log.e(LOG_TAG, "registerForNotifications : name = " + name);
        try {
    ...
    public static vendor.qti.hardware.data.dynamicddsaidlservice.
        ...
        IBinder binder = ServiceManager.waitForService(CNE_FACTORY_SAIDL_SERVICE_NAME);
        vendor.qti.data.factoryservice.IFactory factory =
                vendor.qti.data.factoryservice.IFactory.Stub.asInterface(binder);
        if (factory == null) {
            Log.e(LOG_TAG, "IFactory getService returned null");
            return null;
        }
        try {
            Log.d(LOG_TAG, "Call createAidlDynamicSubscriptionManager");
            vendor.qti.data.factoryservice.Result result =
                    new vendor.qti.data.factoryservice.Result();
            vendor.qti.hardware.data.dynamicddsaidlservice.ISubscriptionManager manager;
            AidlToken token = new AidlToken();
            manager = factory.createDynamicddsISubscriptionManager(token, result);
            if (result.status == 0) { //Success
                Log.d(LOG_TAG, "createAidlDynamicSubscriptionManager success");
                return manager;
            } else {
                Log.d(LOG_TAG, "createAidlDynamicSubscriptionManager failed "
                        + "with status " + result.status);
            }
        } catch (RemoteException e) {
            Log.e(LOG_TAG, "createDynamicddsISubscriptionManager exception", e);
        }
        return null;
    }
    ...
    public static ISubscriptionManager getDynamicSubscriptionManager() {
        ...
        IFactory cneDataFactory = null;
        try {
            Log.d(LOG_TAG, "Call IFactory getService");
            cneDataFactory = IFactory.getService();
        } catch (RemoteException | NoSuchElementException e) {
            Log.e(LOG_TAG, "CnE factory not supported", e);
        }
        if (cneDataFactory == null) {
            Log.e(LOG_TAG, "CnE IFactory getService returned null");
            return null;
        }
        try {
            Log.d(LOG_TAG, "Call createDynamicddsISubscriptionManager");
            cneDataFactory.createDynamicddsISubscriptionManager(new IToken.Stub() {},
                (int status, ISubscriptionManager service) -> {
                    results.status = status;
                    results.service = service;
                });
            Log.d(LOG_TAG, "createDynamicddsISubscriptionManager success");
    ...
    static class AidlToken extends vendor.qti.hardware.data.dynamicddsaidlservice.IToken.Stub {
        ...
        public String getInterfaceHash() {
            return vendor.qti.hardware.data.dynamicddsaidlservice.IToken.HASH;
        }

        @Override
        public int getInterfaceVersion() {
            return vendor.qti.hardware.data.dynamicddsaidlservice.IToken.VERSION;
        }
    }
}
