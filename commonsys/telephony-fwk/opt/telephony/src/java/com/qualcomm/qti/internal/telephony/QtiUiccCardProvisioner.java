...
public class QtiUiccCardProvisioner extends Handler {
    ...
    private QtiUiccCardProvisioner(Context context, CommandsInterface[] cis) {
        ...
        for (int index = 0; index < mNumPhones; index++) {
            ...
            mCis[index].registerForNotAvailable(this, EVENT_RADIO_NOT_AVAILABLE, index);
           ...
           case EVENT_GET_ICCID_DONE:
                ...
                if ((phoneId >= 0) && (phoneId < mNumPhones)) {
                    ...
                    if (!TextUtils.isEmpty(iccId)) {
                        ...
                        QtiSubscriptionManagerService.getInstance().updateSubscription(phoneId);
    ...
    private void queryUiccProvisionInfo(int phoneId, boolean useSimIORequest) {
        ...
        if (mSimIccId[phoneId] == null || !mSimIccId[phoneId].equals(iccId)) {
            if (uiccCard != null) {
                UiccProfile uiccProfile = mUiccController.getUiccProfileForPhone(phoneId);
                if (uiccSlot != null) {
                    ...
                    if(uiccProfile != null) {
                        ...
                        if (numOfApps == 0 && eUiccSupported) {
                            ...
                            if (!TextUtils.isEmpty(iccId)) {
                                mSimFullIccId[phoneId] = fullIccId;
                                QtiSubscriptionManagerService.getInstance().
    ...
    private String getFullIccIdFromUicc(int phoneId) {
        String fullIccId = null;
        UiccPort uiccPort = mUiccController.getUiccPort(phoneId);
        if (uiccPort != null) {
            fullIccId = uiccPort.getIccId();
        } else {
            logi(" queryUiccProvisionInfo, uiccPort[" + phoneId +"] object is null");
        }
    ...
    private void loadIccId(int phoneId) {
        ...
        if (uiccCard != null) {
            ...
            int numApps = uiccProfile.getNumApplications();
            for (int i = 0; i < numApps; i++) {
                UiccCardApplication app = uiccProfile.getApplicationIndex(i);
    ...
    private void updateIccAvailability(int phoneId) {
        ...
        if (mCardState[phoneId] == CardState.CARDSTATE_PRESENT &&
                (mSimIccId[phoneId] == null ||
                !mSimIccId[phoneId].equals(IccUtils.stripTrailingFs(
                getFullIccIdFromUicc(phoneId))) ||
    ...
    private int getCurrentProvisioningStatus(int slotId) {
        synchronized(sManualProvLock) {
            UiccSlot uiccSlot = mUiccController.getUiccSlotForPhone(slotId);
            if (uiccSlot != null && uiccSlot.isEuicc()) {
                logd("getCurrentProvisioningStatus for eUICC slot");

                if (mCardState[slotId] == CardState.CARDSTATE_ABSENT) {
                    return UiccProvisionStatus.CARD_NOT_PRESENT;
                }
                ...
                if (!subscriptionManager.isValidSubscriptionId(subId)) {
                    return UiccProvisionStatus.NOT_PROVISIONED;
                }

                int provStatus =
                        ((subscriptionManager.isActiveSubscriptionId(subId)))
                        ? UiccProvisionStatus.PROVISIONED : UiccProvisionStatus.NOT_PROVISIONED;
                return provStatus;
            }
    ...
    public int getCurrentUiccCardProvisioningStatus(int slotId) {
        ...
        if (canProcessRequest(slotId)) {
            if (mNumPhones == 1) {
                //SIM activate/deactivate is blocked for 1.4 HAL in Single SIM
                //With auto provisioning enabled, SIM status will always be PROVISIONED
                currentStatus = UiccProvisionStatus.PROVISIONED;
            } else {
                currentStatus = getCurrentProvisioningStatus(slotId);
            }
        }
        logd(" getCurrentUiccCardProvisioningStatus, state[" +
    ...
    public int activateUiccCard(int slotId) {
        ...
        } else if (mCis[slotId].canToggleUiccApplicationsEnablement()) {
            loge("activateUiccCard is not appropriate to use. For 1.5 HAL" +
                    " use enableUiccApplications");
            activateStatus = GENERIC_FAILURE;
        } else if (mNumPhones == 1) {
        ...
        } else if (mRequestInProgress.compareAndSet(false, true)) {
    ...
    public int deactivateUiccCard(int slotId) {
        ...
        if (!canProcessRequest(slotId)) {
            ...
            loge("deactivateUiccCard is not appropriate to use. For 1.5 HAL" +
                    " use enableUiccApplications");
    ...
    public void setDdsIfRequired() {
        ...
        int ddsPhoneId = QtiPhoneUtils.getInstance().getPhoneId(mContext, ddsSubId);
        logd("setDdsIfRequired: ddsSub = " + ddsSubId + " ddsPhone = " + ddsPhoneId
                + " needSetDds = " + mNeedSetDds);
        if (QtiPhoneUtils.getInstance().isValidPhoneId(ddsPhoneId) && mNeedSetDds) {
