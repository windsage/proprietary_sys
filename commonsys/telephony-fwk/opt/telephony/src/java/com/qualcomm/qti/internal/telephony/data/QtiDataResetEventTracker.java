...
public class QtiDataResetEventTracker {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case DctConstants.EVENT_DATA_ENABLED_CHANGED: {
                    ...
                    notifyResetEvent("DATA_ENABLED_CHANGED", true);
                    break;
                }
            }
        }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (TelephonyIntents.ACTION_SIM_STATE_CHANGED.equals(action)) {
                ...
                if (IccCardConstants.INTENT_VALUE_ICC_ABSENT.equals(stateExtra)
                    ...
                    notifyResetEvent("SIM_STATE_ABSENT OR DISABLED", false);
                }
            }
        }
    };
    ...
    public QtiDataResetEventTracker(int transportType, Phone phone,
        ...
        mDataEnabledCallback = new DataEnabledChangedListener(subId);
        mTelephonyManager.createForSubscriptionId(subId).registerTelephonyCallback(
                mContext.getMainExecutor(), mDataEnabledCallback);
    }

    private class DataEnabledChangedListener extends TelephonyCallback
            implements TelephonyCallback.UserMobileDataStateListener {

        private int mSubId = SubscriptionManager.INVALID_SUBSCRIPTION_ID;

        public DataEnabledChangedListener(int subId) {
            mSubId = subId;
        }

        @Override
        public void onUserMobileDataStateChanged(boolean enabled) {
            log("onUserMobileDataStateChanged: sub= " + mSubId + " enabled= " + enabled);
            if (!enabled) {
                ...
                public void onCellLocationChanged(CellLocation location) {
                    ...
                    if (location instanceof GsmCellLocation) {
                        ...
                        if (currentLocation == null || currentLocation.isEmpty()) {
                            log("Received invalid location. Ignore");
                            return;
                        }
                        if (mPreviousLocation != null) {
                            if (DBG) log("previous location: " + mPreviousLocation);
                            if (mPreviousLocation.getCid() != currentLocation.getCid()
                                    || mPreviousLocation.getLac() != currentLocation.getLac()) {
    ...
    public void dispose() {
        ...
        if (mDataEnabledCallback != null && mTelephonyManager != null) {
            mDataEnabledCallback.unregisterListener();
        }
    ...
    private void notifyResetEvent(String reason, boolean retry) {
        ...
        stopResetEventTracker();
        if (mListener != null) {
            mListener.onResetEvent(retry, reason);
        }
    }

    private void log(String log) {
        Rlog.d("QtiDataResetEventTracker", log);
    }
}
