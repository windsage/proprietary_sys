...
/*!
*@brief Enumeration identifying the DRM crypto mode type.
*/
typedef enum
{
  FILE_SOURCE_CRYPTO_MODE_NONE,     //Unencrypted data
  FILE_SOURCE_CRYPTO_MODE_AES_CTR,  //AES Counter Mode
  FILE_SOURCE_CRYPTO_MODE_AES_CBC,  //AES Cipher-Block Chaining Mode
} FileSourceCryptoModeType;
...
typedef enum
{
  ...
  FILE_SOURCE_MN_TYPE_DOLBY_VISION,
  ...
  FILE_SOURCE_MN_TYPE_DATA,
  FILE_SOURCE_MN_TYPE_MAX,
  FILE_SOURCE_MN_TYPE_MHM1,
  FILE_SOURCE_MN_TYPE_MHA1,
...
typedef enum
{
  ...
  FILE_SOURCE_MD_ANDROID_HAPTIC,  //! Flag to indicate HAPTIC (in OGG)
...
typedef struct FS_CONTENT_PROTECTION_INFOTYPE {
  ...
  FileSourceCryptoModeType eCryptoMode;
  //! Encrypted blocks in protection pattern
  uint8 ucCryptByteBlock;
  //! Unencrypted blocks in protection pattern
  uint8 ucSkipByteBlock;
  ...
  //! Default Initialization Vector size
  uint8 ucDefaultInitVectorSize;
...
typedef struct
{
  ...
  //! Sample file offset and last sample index in chunk
  int64 offset;
  int64 last_sample_index_in_chunk;
...
typedef struct
{
  ...
  //!Mhm1 Config Profile level
  uint32     ucMhacProfileLevel;
  //!Mhm1 Config Channel layout count
  uint32     ucMhacChannelLayout;
  //!Mhap compatibleset
  uint8* pMhapCompatibleSets;
  //!Mhap compatibleset size
  uint32 ulMhapCompatibleSetSize;
}FileSourceAudioInfo;
