...
bool MMParserExtractor::updateVP9CSD(uint8* buf, uint32 size, uint8* csd, uint32 *csdSize) {
    if ((size < 6) || (*csdSize < 6))
        return false;
    int32 bitDepth = 0;
    int32 profileLowBit = (buf[0] & 0x20) >>5;
    int32 profileHighBit = (buf[0] & 0x10) >>4;
    int32 profile = profileHighBit * 2 + profileLowBit;
    int32 frame_type = buf[0] & 0x04;
    if ((frame_type == 0) && (profile >= 2)) {
        bitDepth = (buf[4] & 0x40) ? 12 : 10;
    } else {
      bitDepth = 8;
    }
    csd[0] = 0x01;  //FEATURE PROFILE
    csd[1] = 0x01;  // length
    csd[2] = profile;
    csd[3] = 0x03;  //FEATURE BITDEPTH
    csd[4] = 0x01; // length
    csd[5] = bitDepth;
    *csdSize = 6;
    return true;
...
void MMParserExtractor::setVideoTrackMeta(Track* track, FileSourceVideoInfo& info) {
    ...
    if (FILE_SOURCE_MN_TYPE_VP9 == minorType) {
        ...
        if (!ret)
        {
          uint8 csd[9] = {0};
          uint32 csdSize = 9;
          FileSourceSampleInfo info = {};
          memset(&info, 0, sizeof(FileSourceSampleInfo));
          uint32 buffLen = m_pFileSourceWrapper->GetTrackMaxFrameBufferSize(track->trackId);
          void *buf = malloc(buffLen);
          if (buf) {
            m_pFileSourceWrapper->GetNextMediaSample(track->trackId, (uint8*)buf, &buffLen, info);
            m_pFileSourceWrapper->SeekAbsolutePosition(0);
            updateVP9CSD((uint8*)buf, buffLen, csd, &csdSize);
            AMediaFormat_setBuffer(track->meta, AMEDIAFORMAT_KEY_CSD_0, csd, csdSize);
            if (buf)
                free(buf);
            }
        }
        ...
        if (info.bIsPrimary) {
            ...
            if (info.ulXMPSize) {
               AMediaFormat_setInt64(mFileMetaData,
                     "xmp-offset" /*AMEDIAFORMAT_KEY_XMP_OFFSET*/, (int64_t)info.ullXMPOffset);
               AMediaFormat_setInt64(mFileMetaData,
                     "xmp-size" /*AMEDIAFORMAT_KEY_XMP_SIZE*/, (int64_t)info.ulXMPSize);
               LOGV("Mmparserextractor xmp offset %d size %d",info.ullXMPOffset,info.ulXMPSize);
            }
        }
