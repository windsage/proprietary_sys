...
#define PARSER_DSF    0x00100000
#define PARSER_DSDIFF 0x00200000
#define PARSER_MOV    0x00400000   //.mov,.mp4,.3gp
#define PARSER_MHAS   0x00800000
...
public:
    ...
    FileSourceStatus GetClipPSSHData(uint8* pPsshData,uint32* size);
...
private:
    ...
    void setPSSHData();
    bool updateVP9CSD(uint8* buf, uint32 size, uint8* csd, uint32* csdSize);
    ...
    void setCryptoInfo(AMediaFormat* meta, FS_CONTENT_PROTECTION_INFOTYPE *info);
    ...
    void setChannelMask(Track* track, uint32_t channelCount,uint32_t hapticChannelCount);
...
private:
    ...
    void setSampleEncryptionMetaData(AMediaFormat *meta, FileSourceSampleInfo &info);
