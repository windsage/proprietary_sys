...
public abstract class BleBroadcastSourceInfoUpdater implements CachedBluetoothDevice.Callback,
    ...
    public void forceUpdate() {
        if (mCachedDevice != null &&
            ...
            Log.d(TAG, "srcInfos size: " + srcInfos.size() + " mPreferenceMap size: " + mPreferenceMap.size());
            if (srcInfos.size() == mPreferenceMap.size()) {
                Log.e(TAG, "New srcInfos size is same with previous");
            } else {
                //force remove of source info entries before updating
                Log.d(TAG, "force remove all soource infos entries before updating");
                removeAllBleBroadcastSourceInfosFromPreference();
            }
