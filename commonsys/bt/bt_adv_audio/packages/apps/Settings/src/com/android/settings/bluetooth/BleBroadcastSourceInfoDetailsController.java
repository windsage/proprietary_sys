...
public class BleBroadcastSourceInfoDetailsController extends BluetoothDetailsController
    ...
    private void forceUpdateBroadcastSourceToLoseSync() {
        if (mBleBroadcastSourceInfo != null &&
            (mBleBroadcastSourceInfo.getMetadataSyncState() == BleBroadcastSourceInfo.BROADCAST_ASSIST_PA_SYNC_STATE_IN_SYNC)) {
             Log.e(TAG, "triggerUpdateBroadcastsource with PA off");
             //force source to lose the PA sync
             mMetadataSyncState = BleBroadcastSourceInfo.BROADCAST_ASSIST_PA_SYNC_STATE_IDLE;
             triggerUpdateBroadcastSource();
             // Wait 200 ms for the update to complete
    ...
    private int getSyncState(int metadataSyncState, int audioSyncState) {
        ...
        if (metadataSyncState != BleBroadcastSourceInfo.BROADCAST_ASSIST_PA_SYNC_STATE_IN_SYNC) {
            //dont consider audio control state if user desires to lost the PS
            //Service forces BISes to zero. can be extended with SYNC_NONE option
     ...
     private void triggerUpdateBroadcastSource() {
         if (mScanAssistanceMgr != null) {
              ...
              Log.e(TAG, "triggerUpdateBroadcastSource:  " + syncState);
              mScanAssistanceMgr.updateBroadcastSource(
                      mBleBroadcastSourceInfo.getSourceId(),
                      syncState, mBisIndicies, mGroupOp);
