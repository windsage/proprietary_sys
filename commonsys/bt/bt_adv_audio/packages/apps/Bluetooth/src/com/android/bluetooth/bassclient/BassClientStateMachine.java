...
final class BassClientStateMachine extends StateMachine {
    ...
    // 200ms timeout for source adding
    static final int ADD_SOURCE_TIMEOUT_MS = 200;

    // 100ms timeout for source updating
    static final int UPDATE_SOURCE_TIMEOUT_MS = 100;

    // 100ms timeout for source removing
    static final int REMOVE_SOURCE_TIMEOUT_MS = 100;

    // 200ms timeout for source selecting
    static final int SELECT_SOURCE_TIMEOUT_MS = 200;
    ...
    //mMTUSize will get updated in onMtuChanged
    ...
    private PeriodicAdvertisingCallback mLocalPeriodicAdvCallback = new BassPACallback();
    private ArrayList<PeriodicAdvertisingCallback> mPeriodicAdvCallbacks;
    ...
    private final Map<BluetoothDevice, PeriodicAdvertisingCallback> mPeriodicAdvCallbackMap;
    ...
    private boolean mUpdateSourceInProgress = false;
    private boolean mAddSourceInProgress = false;
    private BluetoothDevice mAddedSource = null;

    //types of command for  set broadcast PIN operation
    public int FRESH = 1;
    private int QUEUED = 2;

    private int GATT_AUTH_FAIL = 137;
    ...
    private boolean mIsPASyncing = false;
    ...
    private boolean mUpdateAsNoPASTEnabled = false;
    private boolean mVcpForBroadcast = false;
    private boolean mPinReverse = false;
    private boolean mAddAsNoPASTEnabled = false;
    private boolean mBisSyncFirstIndex = false;

    private int BROADCAST_SOURCE_ID_LENGTH = 3;
    private byte mTempSourceId = 0;
    private final byte mBaseSourceId = 0x50;
    ...
    BassClientStateMachine(BluetoothDevice device, BCService svc,
        ...
        mSourceIndexMap = new HashMap<Integer, Byte>();
        ...
        // Initialize PA Callbacks
        mMaxPANum = SystemProperties.getInt("persist.vendor.service.bt.maxPANum", 10);
        mPeriodicAdvCallbacks = new ArrayList<PeriodicAdvertisingCallback>();
        for (int i = 0; i < mMaxPANum; i++) {
            PeriodicAdvertisingCallback bassPACallback = new BassPACallback();
            mPeriodicAdvCallbacks.add(bassPACallback);
        }
        mPeriodicAdvCallbackMap = new HashMap<BluetoothDevice, PeriodicAdvertisingCallback>();
        ...
        mIsWhitelist = SystemProperties.getBoolean("persist.vendor.service.bt.wl", false);
        ...
        mUpdateAsNoPASTEnabled = SystemProperties.getBoolean("persist.vendor.service.bt.updateNoPast", false);
        mVcpForBroadcast = SystemProperties.getBoolean("persist.vendor.service.bt.vcpForBroadcast", true);
        mPinReverse = SystemProperties.getBoolean("persist.vendor.service.bt.pinReverse", false);
        mAddAsNoPASTEnabled = SystemProperties.getBoolean("persist.vendor.service.bt.addNoPast", false);
        mBisSyncFirstIndex = SystemProperties.getBoolean("persist.vendor.service.bt.bisSyncFirstIndex", false);
        mPublicAddrForcSrc = mService.isPublicAddrForSrc();
    }
    ...
    public void cleanup() {
        ...
        clearCharsCache();
        unregisterAllPASync();
        mPeriodicAdvCallbackMap.clear();
        mPeriodicAdvCallbacks.clear();
    ...
    void parseScanRecord(int syncHandle, ScanRecord record) {
        ...
        if (bmsAdvData != null && bmsAdvData.length >= SERVICE_DATA_MIN_LENGTH) {
            ...
            if (bmsAdvData != null) {
                Log.e(TAG, "Data length is " + bmsAdvData.length);
            }
            if (mAutoTriggerred == false) {
                mService.sendBroadcastSourceSelectedCallback(mDevice, null,
                  BleBroadcastAudioScanAssistCallback.BASS_STATUS_INVALID_SOURCE_SELECTED);
    ...
    public boolean selectBroadcastSource (ScanResult scanRes, boolean groupOp, boolean autoTriggerred) {
        ...
        if (isAddedBroadcastSourceIsLocal(scanRes.getDevice()) != true) {
            ...
            if (ApmConst.getQtiLeAudioEnabled()) {
                ...
                PeriodicAdvertisingCallback periodicAdvCallback =
                        mPeriodicAdvCallbackMap.get(scanRes.getDevice());
                ArrayList<BluetoothDevice> activeSyncedSrcList = mService.getActiveSyncedSource(mDevice);
                if (periodicAdvCallback != null && activeSyncedSrcList != null &&
                        activeSyncedSrcList.contains(scanRes.getDevice())) {
                    Log.w(TAG, "PA is already synced for " + scanRes.getDevice());
                    return true;
                }

                Log.i(TAG, "mPeriodicAdvCallbackMap size is: " + mPeriodicAdvCallbackMap.size());
                if (mPeriodicAdvCallbackMap.size() >= mMaxPANum) {
                    mNoStopScanOffload = true;
                    cancelActiveSync(null);
                }
            }
            PeriodicAdvertisingCallback periodicAdvCallback = allocatePeriodicAdvCallback(scanRes.getDevice());
            if (periodicAdvCallback == null) {
                Log.e(TAG, "no callback resource for selectBroadcastSource");
                return false;
            }
            mService.getBassUtils().leScanControl(true);
            try {
               Log.d(TAG, "calling registerSync for " + scanRes.getDevice() + ", PACallback: " + periodicAdvCallback);
               mPeriodicAdvManager.registerSync(scanRes, 0,
                                            PSYNC_TIMEOUT, periodicAdvCallback);
            ...
            if (scanRecord != null) {
                ...
                if (listOfUuids != null) {
                    ...
                    if (listOfUuids.containsKey(ParcelUuid.fromString(BassUtils.PUBLIC_BROADCAST_AUDIO_UUID))) {
                        pbpServiceData = listOfUuids.get(ParcelUuid.fromString(BassUtils.PUBLIC_BROADCAST_AUDIO_UUID));
                    }
                }
                String broadcastName = mService.getBassUtils().
                        getPublicBroadcastNameFromScanRecordBytes(scanRecord.getBytes());
                mService.updatePAResultsMap(scanRes.getDevice(), scanRes.getAddressType(),
                          ...
                          broadcastId, broadcastName, pbpServiceData);
            }
        }
        else {
            log("colocated case");
            ...
            ///*_BMS
            if (mService.isAutoLocalSourceAddEnabled() == false &&
                    (mBAService == null || mBAService.isBroadcastActive() != true)) {
            ...
            int paInterval = mBAService.BroadcastGetAdvInterval();
            int colocatedAddressType = mService.getLocalBroadcastSourceType();
            BluetoothDevice colocatedSrcDevice = mService.getLocalBroadcastSource();
            ...
            mService.updatePAResultsMap(colocatedSrcDevice, colocatedAddressType,
                ...
                mBAService.BroadcatGetAdvHandle(),
                paInterval,
                broadcastId, mBAService.getBroadcastName(), mBAService.getPublicBroadcastServiceData());
                BaseData localBase = mService.getBASE(mBAService.BroadcatGetAdvHandle());
                if (localBase == null) {
                    localBase = new BaseData(mBAService.getNumSubGroups(),
    ...
    private void cancelActiveSync(BluetoothDevice sourceDev) {
        ...
        ArrayList<BluetoothDevice> activeSyncedSrcList = mService.getActiveSyncedSource(mDevice);
        ArrayList<BluetoothDevice> cancelSyncedSrcList = new ArrayList<BluetoothDevice>();
        if (activeSyncedSrcList != null ) {
            log("activeSyncedSrc = " + activeSyncedSrcList);
            if (sourceDev == null) {
                isCancelSyncNeeded = true;
                cancelSyncedSrcList = activeSyncedSrcList;
            } else if(activeSyncedSrcList.contains(sourceDev)) {
                isCancelSyncNeeded = true;
                cancelSyncedSrcList.add(sourceDev);
            }
        }
        if (isCancelSyncNeeded) {
            ArrayList<BluetoothDevice> removedSyncedSources = new ArrayList<BluetoothDevice>();
            for (BluetoothDevice src : cancelSyncedSrcList) {
                PeriodicAdvertisingCallback periodicAdvCallback = mPeriodicAdvCallbackMap.get(src);
                Handler smHandler = getHandler();
                if (smHandler != null) {
                    smHandler.removeEqualMessages(PSYNC_ACTIVE_TIMEOUT, src);
                }
                try {
                    log("calling unregisterSync for " + src + ", PACallback:" + periodicAdvCallback);
                    mPeriodicAdvManager.unregisterSync(periodicAdvCallback);
                    mIsPASyncing = false;
                } catch (IllegalArgumentException ex) {
                    Log.w(TAG, "unregisterSync:IllegalArguementException");
                    //ignore
                }
                mPeriodicAdvCallbackMap.remove(src);
                removedSyncedSources.add(src);
            }
            for (BluetoothDevice src : removedSyncedSources) {
                mService.removeActiveSyncedSource(mDevice, src);
                //clearPAResults if source is not registered by any peer device
                if (!mService.isRegisteredSyncSource(src)) {
                    mService.clearPAResults(src);
                }
            }
    ...
    private PeriodicAdvertisingCallback allocatePeriodicAdvCallback(BluetoothDevice sourceDevice) {
        log("allocatePeriodicAdvCallback for " + sourceDevice);
        if (sourceDevice == null) {
            log("sourceDevice is null");
            return null;
        }
        for (PeriodicAdvertisingCallback callback : mPeriodicAdvCallbacks) {
            if (!mPeriodicAdvCallbackMap.containsValue(callback)) {
                mPeriodicAdvCallbackMap.put(sourceDevice, callback);
                return callback;
            }
        }
        log("No resource for Periodic Advertising Callback");
        return null;
    }

    private void unregisterAllPASync() {
        log("unregisterAllPASync: " + mDevice);
        for (PeriodicAdvertisingCallback callback : mPeriodicAdvCallbacks) {
            if (mPeriodicAdvCallbackMap.containsValue(callback)) {
                try {
                    log("calling unregisterSync for PACallback: " + callback);
                    mPeriodicAdvManager.unregisterSync(callback);
                } catch (IllegalArgumentException ex) {
                    Log.w(TAG, "unregisterSync:IllegalArguementException");
                }
            }
    ...
    public boolean isRegisteredSyncSource(BluetoothDevice sourceDevice) {
        if (sourceDevice != null &&
                mPeriodicAdvCallbackMap.get(sourceDevice) != null) {
            log("sourceDevice " + sourceDevice + " is registered");
            return true;
        }
        log("sourceDevice " + sourceDevice + " is not registered");
        return false;
    }
    ...
    /* Internal periodc Advertising manager callback
      *
      */
     private class BassPACallback extends PeriodicAdvertisingCallback{
           @Override
             ...
             int status) {
               log ("onSyncEstablished" + "syncHandle" + syncHandle +
                ...
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    // clear PAResults for outdated source device
                    BluetoothDevice srcDevice = mService.getDeviceForSyncHandle(syncHandle);
                    if (srcDevice != null && !srcDevice.equals(device)) {
                        log("clear PA result for outdated source device");
                        mService.clearPAResults(srcDevice);
                    }
                    ...
                    mService.updatePAResultsMap(device,
                         ...
                         BCService.INVALID_BROADCAST_ID, null, null);
                    sendMessageDelayed(PSYNC_ACTIVE_TIMEOUT, device, PSYNC_ACTIVE_TIMEOUT_MS);
                    ...
                    mAutoTriggerred = false;
                    mPeriodicAdvCallbackMap.remove(device);
                    mService.clearPAResults(device);
                    if (mAddSourceInProgress && device.equals(mAddedSource)) {
                        mAddSourceInProgress = false;
                    }
                }
           }

           private void sourceFoundNotifcation(int syncHandle, boolean checkPublicSource,
                   boolean encrypted) {
               BluetoothDevice srcDevice = mService.getDeviceForSyncHandle(syncHandle);
               if (srcDevice == null) {
                   log("No device found");
               ...
               BCService.PAResults res = mService.getPAResults(srcDevice);
               if (res == null) {
                   log("No PA record found");
                   return;
               }
               if (checkPublicSource && !res.isPublicBroadcast() ||
                   !checkPublicSource && res.isPublicBroadcast()) {
                   return;
               }
               BluetoothDevice activeDevice = mService.getActiveLeAudioDevice();
               if (activeDevice != null && !activeDevice.equals(mDevice)) {
                   log("Ignore this source for inactive peer");
                   return;
               }
               if (res.getIsNotified() || mService.isAddedSource(srcDevice)) {
                   return;
               }
               res.updateIsNotified(true);
               String program = mService.getProgramInfoFromBASE(syncHandle);
               if (program == null) {
                   program = srcDevice.getName();
               }
               log("Program info: " + program);
               BluetoothLeBroadcastMetadata metaData =
                       mService.getBassUtils().getBroadcastMetadata(
                               srcDevice, res.mBroadcastId, program,
                                       mService.getBASE(syncHandle));
               BluetoothLeBroadcastMetadata.Builder build =
                       new BluetoothLeBroadcastMetadata.Builder(metaData);
               build.setEncrypted(encrypted);
               if (res.isPublicBroadcast()) {
                   build.setEncrypted(res.isEncrypted()).
                         setPublicBroadcast(true).
                         setAudioConfigQuality(res.getAudioQuality());
                   if (res.getMetaData() != null) {
                       build.setPublicBroadcastMetadata(BluetoothLeAudioContentMetadata
                               .fromRawBytes(res.getMetaData()));
                   }
                   String broadcastName = res.getBroadcastName();
                   if (broadcastName != null) {
                       build.setBroadcastName(broadcastName);
                   }
                   log("Public broadcast, broadcast name = " + broadcastName);
               }
               metaData = build.build();
               BassClientService bassClientService = mService.getBassClientService();
               if (bassClientService != null) {
                   log("Notify broadcast source found");
                   bassClientService.getCallbacks().
                           notifySourceFound(metaData);
                   if (!checkPublicSource) {
                       if (mAddSourceInProgress && srcDevice.equals(mAddedSource)) {
                           log("Add source is in progress");
                       } else {
                           log("cancel active sync after source found");
                           mNoStopScanOffload = true;
                           cancelActiveSync(srcDevice);
                       }
                   }
           ...
           public void onPeriodicAdvertisingReport(PeriodicAdvertisingReport report) {
               ...
               if (first != null && first.booleanValue() == true) {
                   log( "onPeriodicAdvertisingReport first time, syncHandle=" + report.getSyncHandle());
                   parseScanRecord(report.getSyncHandle(),report.getData());
                   mFirstTimeBisDiscoveryMap.put(report.getSyncHandle(), false);
                   sourceFoundNotifcation(report.getSyncHandle(), true, false);
               }
           }
           @Override
           ...
           public void onBigInfoAdvertisingReport(int syncHandle, boolean encrypted) {
               log("onBIGInfoAdvertisingReport: syncHandle=" + syncHandle +
                       " ,encrypted =" + encrypted);
               sourceFoundNotifcation(syncHandle, false, encrypted);
           }
       };
    ...
    private void broadcastReceiverState(BleBroadcastSourceInfo state, int index, int max_num_srcInfos) {
        ...
        if (bassClientService != null) {
            bassClientService.getCallbacks().
                    notifyReceiveStateChanged(mDevice, index,
                            mService.convertFromSourceInfo(state));
        }
    }
       ...
       if (srcInfo.getMetadataSyncState() == BleBroadcastSourceInfo.BROADCAST_ASSIST_PA_SYNC_STATE_SYNCINFO_REQ) {
           log("Initiate PAST procedure");
            ...
            if (isAddedBroadcastSourceIsLocal(srcInfo.getSourceDevice()) &&
                ...
                if (broadcastId != srcInfo.getBroadcasterId()) {
            ...
            if (res != null) {
                ...
                if (mNoPast == false && syncHandle != BCService.INVALID_SYNC_HANDLE) {
                    if (isAddedBroadcastSourceIsLocal(srcInfo.getSourceDevice())) {
                        ...
                        try {
                            mPeriodicAdvManager.transferSetInfo(mDevice, serviceData, syncHandle,mLocalPeriodicAdvCallback);
                        } catch (IllegalArgumentException ex) {
    ...
    private void checkAndUpdateBroadcastCode(BleBroadcastSourceInfo srcInfo) {
        if (isAddedBroadcastSourceIsLocal(srcInfo.getSourceDevice())) {
            ...
            if (srcInfo.getEncryptionStatus() == BleBroadcastSourceInfo.BROADCAST_ASSIST_ENC_STATE_PIN_NEEDED &&
                ...
                log("old sourceID: " + mSetBroadcastPINSrcInfo.getSourceId() +
                        " old BroadcasterId: " + mSetBroadcastPINSrcInfo.getBroadcasterId() +
                        " new sourceID: " + srcInfo.getSourceId() +
                        " new BroadcasterId: " + srcInfo.getBroadcasterId());
                if (mSetBroadcastPINSrcInfo.getBroadcasterId() ==
                        srcInfo.getBroadcasterId()) {
                    log("Replace source id");
                    mSetBroadcastPINSrcInfo.setSourceId(srcInfo.getSourceId());
                }
                Message m = obtainMessage(BassClientStateMachine.SET_BCAST_CODE);
            ...
            } else if (srcInfo.getEncryptionStatus() == BleBroadcastSourceInfo.BROADCAST_ASSIST_ENC_STATE_BADCODE &&
                    mSetBroadcastPINSrcInfo != null) {
                log("Incorrect Broadcast code, clear saved broadcast pin code: " +
                        mSetBroadcastPINSrcInfo.getBroadcastCode().toString());
                mSetBroadcastPINSrcInfo = null;
            }
        }
   }
   ...
   private List<BleBroadcastSourceChannel> getListOfBisIndicies(int bisIndicies, int subGroupId, byte[] metaData) {
    ...
    private void processBroadcastReceiverState (byte[] receiverState, BluetoothGattCharacteristic characteristic) {
        ...
        if (receiverState.length > 0)
        {
            ...
            if (mSourceIndexMap.get(characteristic.getInstanceId()) == null) {
                ...
                if (mTempSourceId < NUM_OF_BROADCAST_RECEIVER_STATES) {
                    mSourceIndexMap.put(characteristic.getInstanceId(), mTempSourceId);
                    mTempSourceId++;
                }
            }
        }
        ...
        if (receiverState.length == 0 ||
            ...
            if (mPendingOperation == REMOVE_BCAST_SOURCE) {
                ...
                if (mTempSourceId < NUM_OF_BROADCAST_RECEIVER_STATES) {
                    srcInfo = new BleBroadcastSourceInfo((byte)(mTempSourceId + mBaseSourceId));
                    mSourceIndexMap.put(characteristic.getInstanceId(), srcInfo.getSourceId());
                    mTempSourceId++;
                } else {
        ...
        if (oldSourceInfo == null) {
            ...
            processPASyncState(srcInfo);
        } else if (srcInfo != null && srcInfo.isEmptyEntry() &&
                oldSourceInfo.isEmptyEntry()) {
            log("duplicate empty source info is received, ignore it");
            return;
        } else {
            ...
            if (oldSourceInfo.isEmptyEntry() == true) {
                ...
                if (isEmptyEntry) {
                    ...
                    if (mSetBroadcastPINSrcInfo != null &&
                            (oldSourceInfo.getBroadcasterId() ==
                            mSetBroadcastPINSrcInfo.getBroadcasterId())) {
                    ...
                    sendPendingCallbacks(REMOVE_BCAST_SOURCE,
                          srcInfo.getSourceId(), BluetoothGatt.GATT_SUCCESS);
                    // Notify Local source found after it is removed if search in progress
                    BassUtils bassUtils = mService.getBassUtils();
                    if (isAddedBroadcastSourceIsLocal(removedDevice) &&
                             bassUtils.isSearchInProgress() &&
                             bassUtils.isLocalLEAudioBroadcasting()) {
                        log("notify local source found after source is removed");
                        bassUtils.notifyLocalBroadcastSourceFound(null);
                    }
                } else {
        ...
        index = (int)mSourceIndexMap.get(characteristic.getInstanceId());
        log("index = " + index);
        if (index == -1) {
        ...
        if (mService.isPendingPASyncNotificationtoBA() && mService.isAutoLocalSourceAddEnabled()
                && isAddedBroadcastSourceIsLocal(srcInfo.getSourceDevice())
                && oldSourceInfo != null
                && oldSourceInfo.getMetadataSyncState() != BleBroadcastSourceInfo.BROADCAST_ASSIST_PA_SYNC_STATE_IN_SYNC
                && srcInfo.getMetadataSyncState() == BleBroadcastSourceInfo.BROADCAST_ASSIST_PA_SYNC_STATE_IN_SYNC) {
            if (mBAService == null) {
                mBAService = BroadcastService.getBroadcastService();
            }
            log("calling onPASyncedToBroadcastReceiver");
            mBAService.onPASyncedToBroadcastReceiver(mDevice);
            mService.setPASyncNotified();
        }
        if (srcInfo.getEncryptionStatus() == BleBroadcastSourceInfo.BROADCAST_ASSIST_ENC_STATE_BADCODE ||
                srcInfo.getMetadataSyncState() ==
                        BleBroadcastSourceInfo.BROADCAST_ASSIST_PA_SYNC_STATE_SYNC_FAIL) {
            ...
            if (srcInfo.getMetadataSyncState() ==
                ...
                srcInfo.getAudioSyncState() ==
                    BleBroadcastSourceInfo.BROADCAST_ASSIST_AUDIO_SYNC_STATE_SYNCHRONIZED) {
                log("Update source with PA and audio sync off");
                srcInfo.setMetadataSyncState(BleBroadcastSourceInfo.BROADCAST_ASSIST_PA_SYNC_STATE_IDLE);
                srcInfo.setAudioSyncState(BleBroadcastSourceInfo.BROADCAST_ASSIST_AUDIO_SYNC_STATE_NOT_SYNCHRONIZED);
                srcInfo.setEncryptionStatus(BleBroadcastSourceInfo.BROADCAST_ASSIST_ENC_STATE_UNENCRYPTED);
                Message m = obtainMessage(BassClientStateMachine.UPDATE_BCAST_SOURCE);
                m.arg1 = AUTO;
                m.obj = srcInfo;
                sendMessage(m);
            }
            Message m = obtainMessage(BassClientStateMachine.REMOVE_BCAST_SOURCE);
            m.arg1 = srcInfo.getSourceId();
            sendMessageDelayed(m, REMOVE_SOURCE_TIMEOUT_MS);
        }
    }
    private void handleEncFailure() {
        if (mBluetoothGatt != null) {
            ...
            m.obj = BluetoothProfile.STATE_DISCONNECTED;
            sendMessage(m);
        }
    }
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            if (status == GATT_AUTH_FAIL ||
                ...
                status == BluetoothGatt.GATT_INSUFFICIENT_ENCRYPTION) {
                 Log.e(TAG, "BC Discovery: Enc failure");
                 handleEncFailure();
                 return;
            }
            ...
            if (mDiscoveryInitiated == true) {
                ...
                if (status == BluetoothGatt.GATT_SUCCESS && mBluetoothGatt != null) {
                    Log.w(TAG, "onServicesDiscovered mMTUSize: " + mMTUSize);
                    //Don't send requestMtu, If MTU is already changed & is GTE to BASS_MAX_BYTES.
                    if (mMTUSize >= BASS_MAX_BYTES) {
                      acquireAllBassChars();
                    } else {
        ...
        public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor,
             ...
             if (status == GATT_AUTH_FAIL ||
                 status == BluetoothGatt.GATT_INSUFFICIENT_AUTHENTICATION ||
                 status == BluetoothGatt.GATT_INSUFFICIENT_ENCRYPTION) {
                 Log.e(TAG, "BC Desc Write: inc failure");
                 handleEncFailure();
                 return;
             }
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt,
            ...
            if (status == GATT_AUTH_FAIL ||
                status == BluetoothGatt.GATT_INSUFFICIENT_AUTHENTICATION ||
                status == BluetoothGatt.GATT_INSUFFICIENT_ENCRYPTION) {
                 Log.e(TAG, "BC Char Write: Enc failure");
                 handleEncFailure();
                 return;
            }
    ...
    public List<BleBroadcastSourceInfo> getAllBroadcastSourceInformation() {
        log( "getAllBroadcastSourceInformation: " +
                mBleBroadcastSourceInfos.values().size());
    ...
    class Disconnected extends State {
        ...
        public void enter() {
            ...
            if (!mIsWhitelist && mBluetoothGatt != null) {
        ...
        public boolean isUnicastSupported(BluetoothDevice device) {
            boolean ret = false;
            ParcelUuid[] uuids = null;
            if (device != null) {
                uuids = device.getUuids();
            }
            ParcelUuid ascs = ParcelUuid.fromString("0000184E-0000-1000-8000-00805F9B34FB");
            if (uuids != null) {
            ...
            log("isUnicastSupported for" + device + "returns: " + ret);
            return ret;
        }
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case CONNECT:
                    ...
                    if (mIsWhitelist == false) {
                        if (isUnicastSupported(mDevice)) {
                            mIsWhitelist = false;
                        } else {
                            mIsWhitelist = true;
                        }
                    }
                    log("mIsWhitelist>>: " + mIsWhitelist);
                    ...
                    if (state == BluetoothProfile.STATE_CONNECTED) {
                        ...
                        if (mNoCSIPReconn == false && mService != null &&
                           mService.isLockSupportAvailable(mDevice)) {
                            ...
                            mCsipWrapper.connect(mService.mCsipAppId, mDevice);
                            transitionTo(mConnecting);
                            break;
                            ////_CSIP*/
                        } else {
                ...
                case PSYNC_ACTIVE_TIMEOUT:
                    BluetoothDevice syncedSource = (BluetoothDevice)message.obj;
                    log("PSYNC_ACTIVE_TIMEOUT syncedSource: " + syncedSource);
                    cancelActiveSync(syncedSource);
                    break;
                default:
    ...
    private byte[] convertSourceInfoToUpdateSourceByteArray(BleBroadcastSourceInfo srcInfo) {
        ...
        if (isAddedBroadcastSourceIsLocal(existingSrcDevice) &&
                broadcastId != existingSI.getBroadcasterId()) {
             //trigger modify src with No PAST
             if (mUpdateAsNoPASTEnabled) {
    ...
    private byte[] convertSourceInfoToSetBroadcastCodeByteArray(BleBroadcastSourceInfo srcInfo) {
        ...
        if (srcInfo.getSourceDevice() != null &&
            ...
            //query the Encryption Key from BMS and update
            ///*_BMS
            byte[] mBC = mBAService.GetEncryptionKey(null);
            //actualPIN = mBAService.GetEncryptionKey(null);
            System.arraycopy(mBC, 0, actualPIN, 0, 16);
            //_BMS*/
            log("colocatedBcastCode is " + Arrays.toString(actualPIN));
            if (mPinReverse)
            {
                reverseBytes(actualPIN);
                log("colocatedBcastCode is after pin reverse" + Arrays.toString(actualPIN));
            }
        } else {
            //Can Keep as ASCII as is
            String reversePIN = new StringBuffer(srcInfo.getBroadcastCode()).reverse().toString();
            byte[] bCode = reversePIN.getBytes();
            System.arraycopy(bCode, 0, actualPIN, (16-bCode.length), bCode.length);
            if (mPinReverse)
            {
                reverseBytes(actualPIN);
                log("non-colocatedBcastCode is after pin reverse" + Arrays.toString(actualPIN));
            }
            ...
            switch (message.what) {
                ...
                case DISCONNECT:
                    ...
                    if (mBluetoothGatt != null) {
                        ...
                        if (mCsipConnected) {
                             mCsipWrapper.disconnect(mService.mCsipAppId, mDevice);
                             mCsipConnected = false;
                        }
                        transitionTo(mDisconnected);
                    } else {
                ...
                case SELECT_BCAST_SOURCE:
                    if (mAddSourceInProgress) {
                        Log.d(TAG, "add source is in progress, delay SELECT_BCAST_SOURCE msg");
                        sendMessageDelayed(SELECT_BCAST_SOURCE, message.arg1, message.arg2,
                                message.obj, SELECT_SOURCE_TIMEOUT_MS);
                    } else {
                ...
                case ADD_BCAST_SOURCE:
                    ...
                    mAddSourceInProgress = true;
                    mAddedSource = srcInfo.getSourceDevice();
                    byte[] addSourceInfo =  convertSourceInfoToAddSourceByteArray(srcInfo);
                    if (addSourceInfo == null) {
                        BCService.PAResults paRes = mService.getPAResults(srcInfo.getSourceDevice());
                        if (paRes != null && mService.getBASE(paRes.mSyncHandle) == null && mIsPASyncing) {
                            Log.d(TAG, "Base info is null, defer ADD_BCAST_SOURCE msg");
                            sendMessageDelayed(ADD_BCAST_SOURCE, srcInfo,
                                    ADD_SOURCE_TIMEOUT_MS);
                        } else {
                ...
                case UPDATE_BCAST_SOURCE:
                    ...
                    if (mBluetoothGatt != null &&
                        ...
                        transitionTo(mConnectedProcessing);
                        mUpdateSourceInProgress = true;
                        sendMessageDelayed(UPDATE_SOURCE_TIMEOUT,
                ...
                case REMOVE_BCAST_SOURCE:
                    ...
                    if (mUpdateSourceInProgress == true) {
                        log("Update source is in progress, delay remove msg");
                        sendMessageDelayed(REMOVE_BCAST_SOURCE, sourceId,
                                0, audioSrc, REMOVE_SOURCE_TIMEOUT_MS);
                        break;
                    }
                ...
                case PSYNC_ACTIVE_TIMEOUT:
                    ...
                    cancelActiveSync(syncedSource);
                    break;
                case UPDATE_SOURCE_TIMEOUT:
                    log("Update source timeout");
                    mUpdateSourceInProgress = false;
                    break;
                default:
    ...
    void sendPendingCallbacks(int pendingOp, byte sourceId, int status) {
        log("sendPendingCallbacks: pendingOp " + pendingOp +
                " status " + status);
        if (status != 0) {
        ...
        switch (pendingOp) {
            ...
            case ADD_BCAST_SOURCE:
                ...
                break;
            case UPDATE_BCAST_SOURCE:
                removeMessages(UPDATE_SOURCE_TIMEOUT);
                mUpdateSourceInProgress = false;
                if (mAutoTriggerred == false) {
    ...
    int getMaximumSourceCapacity() {
        return NUM_OF_BROADCAST_RECEIVER_STATES;
    }
