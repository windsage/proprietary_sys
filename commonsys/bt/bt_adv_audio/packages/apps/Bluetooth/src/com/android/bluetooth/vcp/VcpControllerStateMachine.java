...
final class VcpControllerStateMachine extends StateMachine {
    ...
    class Connected extends State {
        ...
        public void exit() {
            ...
            Log.d(TAG, "remove delayed messages");
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case SET_ABS_VOL_TIMEOUT: {
                    ...
                    if (mPtsTest) {
                        log("Skip retry set abs volume for pts test: " + device);
                        mRequestedVolume = -1;
                        mAbsVolRetryTimes = 0;
                        break;
                    }
    ...
    private void broadcastConnectionState(int newState, int prevState) {
        ...
        if (ApmConst.getQtiLeAudioEnabled()) {
            intent = new Intent(BluetoothVcp.ACTION_CONNECTION_STATE_CHANGED);
        } else {
            intent = new Intent(BluetoothVolumeControl.ACTION_CONNECTION_STATE_CHANGED);
        }
