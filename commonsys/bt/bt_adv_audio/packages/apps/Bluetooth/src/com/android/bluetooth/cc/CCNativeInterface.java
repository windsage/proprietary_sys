...
public class CCNativeInterface {
    ...
    private CCNativeInterface() {
     ...
     /**
     * This class is a singleton because native library
    ...
    public void init(int maxCcsClients, boolean inbandRingingEnabled) {
        initializeNative("00008fd1-0000-1000-8000-00805F9B34FB", maxCcsClients,
    ...
    public boolean callState(ArrayList<CallControlState> callList) {
        ...
        for (int i=0; i<len; i++) {
            ...
            cStateListBytes[3*i+1] =
                (byte) CCHalConstants.getCCsCallState(callList.get(i).mState);
  ...
  @VisibleForTesting(visibility = VisibleForTesting.Visibility.PACKAGE)
    ...
    @VisibleForTesting(visibility = VisibleForTesting.Visibility.PACKAGE)
    public boolean callControlResponse(int op, int index, int status,
                                             BluetoothDevice device) {
        return callControlResponseNative(op, index, status,
                                                getByteAddress(device));
    }

    @VisibleForTesting(visibility = VisibleForTesting.Visibility.PACKAGE)
    public boolean updateActiveProfile(BluetoothDevice device, int state) {
        return updateActiveProfileNative(getByteAddress(device), state);
    }
    ...
    private void callControlPointChangedRequest(int op, int[]call_indices,
                                                int count, byte[] dialNumber,
                                                byte[] address) {
        BluetoothDevice device = getDevice(address);
        String dialUri = new String(dialNumber, StandardCharsets.UTF_8);
        if (DBG) {
            Log.d(TAG, "CallControlPointChangedRequest: " + op +
                       ", dialNumber: " + dialUri);
        }
        CCService service = CCService.getCCService();
        if (service != null) {
            service.handleCallControlOps(op, call_indices, count,
                                         dialNumber, address);
        }
    }
    ...
    private native void initializeNative(String uuid, int max_ccs_clients,
                                          boolean inbandRingingEnabled);
    ...
    private native boolean updateActiveProfileNative( byte[] address, int state);
    private native boolean contentControlIdNative(int ccid);
    private native boolean disconnectNative(byte[] address);
    private native boolean callControlResponseNative(int op, int index,
