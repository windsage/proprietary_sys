...
static jboolean updateActiveProfileNative(JNIEnv* env, jobject object,
                                          jbyteArray address, jint state) {
  LOG(INFO) << __func__;
  std::shared_lock<std::shared_timed_mutex> lock(interface_mutex);
  if (!sCallControllerInterface) return JNI_FALSE;
  jbyte* addr = env->GetByteArrayElements(address, nullptr);
  ...
  RawAddress* tmpraw = (RawAddress*)addr;
  sCallControllerInterface->UpdateActiveProfile(*tmpraw, state);
  env->ReleaseByteArrayElements(address, addr, 0);
...
static JNINativeMethod sMethods[] = {
    ...
    {"updateActiveProfileNative", "([BI)Z", (void*)updateActiveProfileNative},
