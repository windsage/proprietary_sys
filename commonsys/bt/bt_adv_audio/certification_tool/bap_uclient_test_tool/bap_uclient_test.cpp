...
bt_acl_state_t state, int transport_link_type, bt_hci_error_code_t hci_reason, bt_conn_direction_t direction,
uint16_t acl_handle)
{
...
void do_bap_connect_and_start (char *p)
{
    ...
    for (uint8_t i = 0; i < args.serv.size(); i++) {
        ...
        std::vector<RawAddress> address;
        address.push_back(bap_bd_addr);
        sUcastClientInterface->Start(address, start_streams, false, false);
    }
}
...
void do_bap_start (char *p)
{
    ...
    for (uint8_t i = 0; i < ((args.cnt)/4); i++) {
        ...
        std::vector<RawAddress> address;
        address.push_back(bap_bd_addr);
        sUcastClientInterface->Start(address, streams, false, false);
    }
}
...
void do_bap_stop (char *p)
{
    ...
    for (uint8_t i = 0; i < ((args.cnt)/4); i++) {
        ...
        sUcastClientInterface->Stop(bap_bd_addr, streams, false);
    }
}
