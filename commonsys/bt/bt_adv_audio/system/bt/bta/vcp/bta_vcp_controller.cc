...
class VcpControllerImpl : public VcpController {
 ...
 public:
  ...
  void Connect(const RawAddress& address, bool isDirect) override {
    ...
    BTA_GATTC_Open(gatt_if, address, isDirect, BT_TRANSPORT_LE, false);
  }
  ...
  void OnServiceSearchComplete(uint16_t conn_id, tGATT_STATUS status) {
    ...
    // This code is used to pass PTS TC for invalid change counter or invalid opcode
    char value[PROPERTY_VALUE_MAX] = {0};
    bool is_vcp_pts_test = false;
    if ((osi_property_get("persist.vendor.service.bt.vcp_controller.pts", value, "false"))
        && (!strcmp(value, "true"))) {
       is_vcp_pts_test = true;
    }
    ...
    if (!is_vcp_pts_test) {
        ...
        LOG(WARNING) << "writing CCC for vcs volume_state_handle";
        ...
        GattOpsQueue::WriteDescriptor(gatt_if,
                conn_id, rendererDevice->vcs.volume_state_ccc_handle,
   ...
   void OnSetAbsVolume(uint16_t client_id, uint16_t conn_id, tGATT_STATUS status,
     ...
     if (status != GATT_SUCCESS) {
       ...
       if (status == VCS_INVALID_CHANGE_COUNTER ||
         ...
         LOG(ERROR) << __func__  << ": Error code: " << loghex(status)
                               << " device: " << rendererDevice->address
                               << " set volume retry count: "
                               << loghex(rendererDevice->vcs.set_volume_retry_count)
                               << " Read Volume State to update change counter";

         if (rendererDevice->vcs.set_volume_retry_count > 0) {
           rendererDevice->vcs.set_volume_retry_count--;
           rendererDevice->vcs.retry_cmd |= VCS_RETRY_SET_ABS_VOL;
           GattOpsQueue::ReadCharacteristic(gatt_if,
               conn_id, rendererDevice->vcs.volume_state_handle,
               VcpControllerImpl::OnVolumeStateReadStatic, nullptr);
         } else {
           rendererDevice->vcs.set_volume_retry_count = VCS_RETRY_COUNT;
           rendererDevice->vcs.retry_cmd &= ~VCS_RETRY_SET_ABS_VOL;
           LOG(ERROR) <<  __func__ << ": Retry attemtps exceed max";
         }
       } else {
         rendererDevice->vcs.set_volume_retry_count = VCS_RETRY_COUNT;
         rendererDevice->vcs.retry_cmd &= ~VCS_RETRY_SET_ABS_VOL;
   ...
   void OnMute(uint16_t client_id, uint16_t conn_id, tGATT_STATUS status,
     ...
     if (status != GATT_SUCCESS) {
       LOG(ERROR) << "Volume State Write failed" << rendererDevice->address
                            << " mute retry count: "
                            << loghex(rendererDevice->vcs.mute_retry_count)
                            << " Read Volume State";

       if (rendererDevice->vcs.mute_retry_count > 0) {
         rendererDevice->vcs.mute_retry_count--;
         rendererDevice->vcs.retry_cmd |= VCS_RETRY_SET_MUTE_STATE;
         GattOpsQueue::ReadCharacteristic(gatt_if,
             conn_id, rendererDevice->vcs.volume_state_handle,
             VcpControllerImpl::OnVolumeStateReadStatic, nullptr);
       } else {
         rendererDevice->vcs.mute_retry_count = VCS_RETRY_COUNT;
         rendererDevice->vcs.retry_cmd &= ~VCS_RETRY_SET_MUTE_STATE;
         LOG(ERROR) <<  __func__ << ": Retry attemtps exceed max";
       }
     } else {
...
void vcp_controller_gattc_callback(tBTA_GATTC_EVT event, tBTA_GATTC* p_data) {
  ...
  switch (event) {
    ...
    case BTA_GATTC_CLOSE_EVT: {
      if (!instance) {
        if (p_data){
           LOG(ERROR) << __func__ << ": p_data is not null, need to delete gatt op queue, conn_id : "<<p_data->close.conn_id;
