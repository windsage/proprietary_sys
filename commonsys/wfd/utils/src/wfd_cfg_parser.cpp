...
/* Max length for SoC ID */
#define MAX_SOC_ID_LENGTH 3
...
void  parseUIBCSupport(string uibcBuffer)
{
   ...
   if(bValid)
   {
      ...
      if(bHID)
      {
        ...
        // to check what are the supported input type of CustomUIBC
        string tagValue = getValueForTag(uibcBuffer, "CustomAppUIBCSupport");
        bool isCustomHIDCEnabled = atoi(tagValue.c_str()) ? true : false;
        string sHIDSup;
        if(isCustomHIDCEnabled)
            sHIDSup = getValueForTag(uibcBuffer,"CustomHIDCInputPaths");
        else
            sHIDSup = getValueForTag(uibcBuffer,"HIDInputPaths");
...
void limitCodecParams(uint8& uMaxLevel,string codecName)
{
    ...
    switch(mnHW_ID)
    {
        ...
        case 663:
        /*-----*/
        case 608: /* SOC id for crow */
        case 644: /* SOC Id for 4g crow */
        /*-----*/
        case 636: /* SoC ID for volcano */
        case 640:
        case 641:
        case 657:
        case 658:
...
void parseHWID()
{
    ...
    /*  1 extra byte for '\0' character */
    char buffer[MAX_SOC_ID_LENGTH + 1];
    buffer[MAX_SOC_ID_LENGTH] = '\0';
    FILE *device = NULL;
    device = fopen("/sys/devices/soc0/soc_id", "r");
    if (device)
    {
        result = fread(buffer, sizeof( char ), MAX_SOC_ID_LENGTH, device);
        fclose(device);
    }
    else
    {
...
{
    if (-1 != mnHW_Ver) {
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,"HW_Ver is already populated mnHW_Ver = %d",mnHW_Ver);
        return;
    }
    ...
    if (GetContext() == OSAL_VND_CTX) {
        ...
        property_get("ro.media.xml_variant.codecs", targetVariant, "default");
    }
    ...
    switch (mnHW_ID)
    {
        ...
        case 608:
        case 644:
           if (!(strncmp(targetVariant, "_crow_v0", PROPERTY_VALUE_MAX)))
           {
               mnHW_Ver = 0; //Higher Spec
           }
           else
        ...
        case 653:
        case 654:
           if(!(strncmp(targetVariant, "_ravelin", PROPERTY_VALUE_MAX)))
           {
               mnHW_Ver = 0; //Higher Spec
           }
           else
        ...
        case 642:
        case 643:
           if(!(strncmp(targetVariant, "_cliffs_v0", PROPERTY_VALUE_MAX)))
           {
               mnHW_Ver = 0; //Higher Spec
           }
           else
           ...
           break;
        case 623:
           if(!(strncmp(targetVariant, "pitti", PROPERTY_VALUE_MAX)))
           {
               mnHW_Ver = 0; //Higher Spec
           }
           else
        ...
        case 657:
        case 658:
           if(!(strncmp(targetVariant, "_volcano_v0", PROPERTY_VALUE_MAX)))
           {
                mnHW_Ver = 0; //Higher Spec
           }
           else
