...
android::Surface* getSurface()
{
    ...
    if (s != NULL)
    {
        ...
        if (s->client != NULL && s->client->initCheck() == NO_ERROR)
        {
            ...
            sp<IBinder> primaryDisplay = SurfaceComposerClient::getPhysicalDisplayToken(ids.front());
            if(primaryDisplay)
            {
                ui::DisplayState mainDpyState;
                status_t ret = s->client->getDisplayState(primaryDisplay, &mainDpyState);
                sp<IBinder> m_mainDpy = SurfaceComposerClient::getPhysicalDisplayToken(ids.front());
...
int configureVirtualDisplay(const sp<IBinder>& m_mainDpy,const ui::DisplayState& mainDpyState,
    ...
    */
    ...
    sp<IBinder> dpy = SurfaceComposerClient::createVirtualDisplay(
            std::string("venc-widi"), static_cast<bool>(pMMInfo->hdcp));
    if(dpy.get() == NULL)
    {
...
int createVDS(void* pInfo)
{
    ...
    if(pMMInfo && pMMInfo->producer)
    {
        ...
        const auto ids = SurfaceComposerClient::getPhysicalDisplayIds();
        sp<IBinder> mainDpy = SurfaceComposerClient::getPhysicalDisplayToken(ids.front());
        if(mainDpy)
        {
            ui::DisplayState mainDpyState;
...
void releaseVDS()
{
    ...
    if(dpy != NULL)
    {
        ...
        */
        bufferProducer = NULL;
        SurfaceComposerClient::destroyVirtualDisplay(dpy);
