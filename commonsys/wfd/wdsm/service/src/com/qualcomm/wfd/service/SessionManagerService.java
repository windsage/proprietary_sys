 ...
 *
 * Copyright (c) 2012 - 2019,2021-2024 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
public class SessionManagerService extends ISessionManagerService.Stub {
    ...
    private static final String sWFDEnableKey64 = "vendor.wfdservice64";

    // List of strings which contains various 64-bit targets.
    // This list of targets are restricted to run only 64-bit services.
    private static final List<String> Service64bitSupportedTargets =
    ...
    private static String mBuildProduct;
    ...
    public SessionManagerService(Context context) {
        ...
        mBoardPlatform = SystemProperties.get("ro.board.platform");
        mBuildProduct = SystemProperties.get("ro.build.product");
        Log.d(TAG, "Target platform: " + mBoardPlatform
                 + " Target build product: " + mBuildProduct);

        if (Service64bitSupportedTargets.contains(mBoardPlatform) ||
                     Service64bitSupportedTargets.contains(mBuildProduct)) {
            Log.d(TAG, "Enable 64-bit wfd service");
            SystemProperties.set(sWFDEnableKey64, "enable");
        } else {
            Log.d(TAG, "Enable 32-bit wfd service");
    ...
    protected void destroyService() {
        ...
        if (Service64bitSupportedTargets.contains(mBoardPlatform) ||
                     Service64bitSupportedTargets.contains(mBuildProduct)) {
            SystemProperties.set(sWFDEnableKey64, "disable");
            Log.d(TAG, "64-bit wfd service disabled");
        } else {
            SystemProperties.set(sWFDEnableKey, "disable");
            Log.d(TAG, "32-bit wfd service disabled");
        }
    }
    @Override
