...
public class WFDSession extends IWfdSession.Stub implements UIBCManager.HIDCEventListener, UIBCManager.EventListener {
    ...
    private Notification.Builder mNotificationBldr = null;

    private Notification mNotification = null;

    private boolean mDebug = false;

    private boolean mLatencyNotify = false;
    ...
    public int setCodecResolution(int codec, int profile, int level, int formatType, int value) {
        ...
        synchronized (mLock) {
            ...
            if(
                (codec == VideoFormat.WFD_VIDEO_H264.ordinal() &&
                 (level < H264Level.WFD_VIDEO_CODEC_LEVEL_3_1.ordinal() ||
                  level > H264Level.WFD_VIDEO_CODEC_LEVEL_5_2.ordinal()))
                ||
                (codec == VideoFormat.WFD_VIDEO_H265.ordinal() &&
                 (level < H265Level.WFD_VIDEO_CODEC_LEVEL_3_1.ordinal() ||
                  level > H265Level.WFD_VIDEO_CODEC_LEVEL_5_1.ordinal()))
                )
            {
    ...
    public int startUibcSession() {
        ...
        synchronized (mLock) {
            ...
            return nativeEnableUIBC(mSessionId,mNativePtr);
        }
    ...
    private void updateSinkState(int sessionId) {
        new Thread(new Runnable() {
            public void run(){
                Log.d(mTAG,"Updating sink state");
                Bundle b = new Bundle();
    ...
    private void eventCallback(String eventName, Object[] objectArray) {
        ...
        if ("Error".equalsIgnoreCase(eventName)) {
        ...
        } else if("CUSTOM_HIDC_SUPPORT".equalsIgnoreCase(eventName)) {
            Log.e(mTAG, "Custome HIDC is enabled");
            Bundle b = new Bundle(4);
            b.putString("event", (String)objectArray[0]);
            b.putInt("port_id", Integer.parseInt((String) objectArray[1]));
            b.putString("ip_address", (String)objectArray[2]);
            b.putString("uibc_parameter", (String)objectArray[3]);
            notify(b, -1);
            ...
            if (objectArray.length >= 2) {
                ...
                } else if("DISPLAY_EDID".equalsIgnoreCase(var)) {
                    String event = (String) objectArray[0];
                    String edidPayload = (String) objectArray[1];
                    for (int arrayCount = 2; arrayCount < objectArray.length; arrayCount++)
                    {
                        if (objectArray[arrayCount] != null) {
                            edidPayload = edidPayload + ((String) objectArray[arrayCount]);
                        }
                    }
                    Log.d(mTAG,"event: " + event + "; edidPayload: " + edidPayload);
                } else if("HW_CURSOR".equalsIgnoreCase(var)) {
                    String event = (String) objectArray[0];
                    String xorSupport = (String) objectArray[1];
                    int x = Integer.parseInt((String) objectArray[2],16);
                    int y = Integer.parseInt((String) objectArray[3],16);
                    int cursorPort = Integer.parseInt((String) objectArray[4],16);
                    Log.d(mTAG,"event: " + event + "; HW Cursor params: XorSupport: " + xorSupport +
                               ", X: " + x + ", Y: " + y + ", cursorPort: " + cursorPort);
                ...
                else if ("G2G_LATENCY_UPDATE".equalsIgnoreCase(var)){
                    mDebug = SystemProperties.getBoolean(sDebugKey, false);
                    Log.e(mTAG, "G2GLatency property value = " + mDebug);
                    if (mDebug)
                    {
                        int G2GLatency = Integer.parseInt((String) objectArray[1]);
                        createG2GLatencynotification(G2GLatency);
                    }
                }
            }
...
//                                            | DisplayManager.VIRTUAL_DISPLAY_FLAG_SUPPORTS_PROTECTED_BUFFERS;
    ...
    private void createG2GLatencynotification(int G2GLatency) {
        ...
        if(!mLatencyNotify)
        {
             mNotificationBldr = new Notification.Builder(mContext)
                     .setContentTitle("G2G-Latency")
                     .setContentText("NetworkLatency =" + G2GLatency)
                     .setOnlyAlertOnce(true)
                     .setUsesChronometer(true);

             if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                 mNotificationBldr.setSmallIcon(android.R.drawable.ic_menu_view);
                 ...
                 mNotificationBldr.setChannelId(channel.getId());
                 mNotificationBldr.setGroup(mTAG);
             }
             mNotification = mNotificationBldr.build();
                   mNotiMsgId = NOTI_MSG_ID.getAndIncrement();
                   mLatencyNotify = true;
        }
        else
        {
            mNotificationBldr.setContentText("NetworkLatency =" + G2GLatency);
        }
        mNotiMgr.notify(mNotiMsgId, mNotification);
