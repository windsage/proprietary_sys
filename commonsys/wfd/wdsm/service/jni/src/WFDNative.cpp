...
*
*
*  Copyright (c) 2012 - 2017, 2019-2024, Qualcomm Technologies, Inc.
*  All Rights Reserved.
        ...
        const jobject& wfdSessionObj, const jobject& eventDispatcherObj)
{
    ...
    ScopedLocalRef<jobject> eventObj(jniEnv);
    ...
    case WFD_UIBC_TOUCH:
     {
        ...
        if(uibcParam->multiMode) {
            if(uibcParam->prevEv->getPointerCount() < ev->parms.touch_event.num_pointers) {
               ...
               fillerEv->initialize(InputEvent::nextId(),MOTION_EVENT_DEVICE_ID,
                            ...
                            // TODO(b/112430608): ui::LogicalDisplayId::DEFAULT is almost
               ...
               ScopedLocalRef<jobject> fillerEvent = android_view_MotionEvent_obtainAsCopy(jniEnv, (const MotionEvent&) *fillerEv);
               LOGD("Injecting filler event with action %d",fillerAction);
               jniEnv->CallVoidMethod(eventDispatcherObject, gUIBCEventDispatcherClassInfo.addEvent, fillerEvent.get());
               delete fillerEv;
        ...
        if (ev->parms.touch_event.num_pointers > 0 && ev->parms.touch_event.num_pointers <= UIBC_MAX_TOUCH_INPUTS) {
            ...
            for(int i = 0; i < ev->parms.touch_event.num_pointers; i++) {
                uibcParam->prevPtrCoords[i] = pointerCoords[i];
                uibcParam->prevPtrProp[i] = pointerProps[i];
            }
        } else {
    ...
    case WFD_UIBC_KEY:
      {
        ...
        eventObj = android_view_KeyEvent_obtainAsCopy(jniEnv,*(const KeyEvent*)newEvent);
        delete newEvent;
        break;
     }
    ...
    if (!eventObj.get() && WFD_UIBC_ROTATE != ev->type) {
    ...
    else
    {
        if(injectEvent) {
          jniEnv->CallVoidMethod(eventDispatcherObject,
            gUIBCEventDispatcherClassInfo.addEvent, eventObj.get());
        }
    }
    DETACH_THREAD(isAttached);
    return TRUE;
  ...
  (JNIEnv * env, jobject wfdSessionObj, jobject keyEvent, jlong nativePtr)
{
    ...
    if(client)
    {
        KeyEvent ke = android_view_KeyEvent_obtainAsCopy(env,keyEvent);
