...
public class WFDSigmaActivity extends Activity {
    ...
    public static final Object SigmaP2pConnectLock = new Object();
    private static final int DISCOVERY_TIMEOUT = 10000; // 10 secs
    private static final int CONNECT_TIMEOUT = 420000; // 7 mins
    private static final int MAX_PEER_CONNECT_RETRY_CNT = 5; // 5 retry count
    ...
    */
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case NetManagerIF.WIFI_UTIL_DISCONNECTED: {
                    ...
                    if ((sigmaDiscoverySuccess) && (sigmaPeerDevice != null) && (sigmaP2pConnectSuccess)) {
                    ...
                    {
                        Toast.makeText(WFDSigmaActivity.this, "P2P disconnect received with no group formed. Ignore error",
                            Toast.LENGTH_LONG).show();
                    }
                }
                break;
    ...
    public int startWfdSession(String macAddress, String deviceType, String goIntentValue, String initWfdVal, String extSupport) {
        ...
        try {
            ...
            for (retryCnt = 0; retryCnt < MAX_PEER_CONNECT_RETRY_CNT; retryCnt++) {

                initReturn = discoverPeerAndConnect(macAddress.trim(), Integer.parseInt(goIntentValue), Integer.parseInt(initWfdVal));
                ...
                if (0 == initReturn) {
                    ...
                    Log.i(TAG, "P2P Connect Failed Retry counter : " + retryCnt);
                }
            }
    ...
    private int discoverPeerAndConnect(String peerMacAddress, int goIntentValue, int initWfd)
    {
        ...
        synchronized (SigmaP2pConnectLock) {
        if ((sigmaPeerDevice != null) && sigmaDiscoverySuccess)  {
            ...
            if (initWfd == 1) {
                ...
                try {
                    SigmaP2pConnectLock.wait(CONNECT_TIMEOUT);  //wait for P2P Connect result
                } catch (Exception e) {
                    Log.e(TAG, "SIGMA:Exception" + e);
                }
            }
    ...
    public void notifyConnectCompletion() {
        Log.d(TAG, "notify connect Completion");
        synchronized (SigmaP2pConnectLock) {
            SigmaP2pConnectLock.notifyAll();
        }
    }
