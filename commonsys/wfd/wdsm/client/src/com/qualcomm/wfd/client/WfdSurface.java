...
public class WfdSurface extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public void surfaceDestroyed(SurfaceHolder arg0) {
        ...
        /*Waiting for sink multimedia to get destroyed.
          Wait is for maximum of 1 second.
          If multimedia takes more than 1 sec to destroy,
          the screen gets destroyed giving video runtime error*/
        int timeout = 10;
        while(timeout != 0) {
            try
            {
                WfdStatus checkStatus;
                checkStatus = mWfdSession.getStatus();
                Log.d(mTAG, "Session state during surface destruction is "+ checkStatus.state);
                if((checkStatus.state == WfdEnums.SessionState.PAUSING.ordinal())
                   ||(checkStatus.state == WfdEnums.SessionState.STANDING_BY.ordinal())
                   ||(checkStatus.state == WfdEnums.SessionState.PLAYING.ordinal())
                   ||(checkStatus.state == WfdEnums.SessionState.TEARING_DOWN.ordinal())){
                    try {
                        Log.e(mTAG,"Adding sleep since Multimedia is not Paused yet");
                        Thread.sleep(100);
