    ...
    private static class WfdActionListenerImpl extends IWfdActionListener.Stub {
        ...
        public void notify(Bundle b, int sessionId) throws RemoteException {
            if(b != null) {
                ...
                else if("CUSTOM_HIDC".equalsIgnoreCase(event))
                {
                    Log.d(TAG, "Event=" + event);
                    Log.d(TAG, "Port_id=" + b.getInt("port_id"));
                    Log.d(TAG, "Ip_address=" + b.getString("ip_address"));
                    Log.d(TAG, "uibc_parameter=" + b.getString("uibc_parameter"));
                }
            }
        }
