        ...
        if (gMemHalAidl == nullptr) {
            read_lock.unlock();
            {
                ...
                if (gMemHalAidl == nullptr) {
                    ...
                    auto MemHalBinder = ::ndk::SpAIBinder(AServiceManager_checkService(instance.c_str()));
...
int  MemHal_SetProp_extn(const std::string& in_propname, const std::string& in_NewVal, const std::vector<int32_t>& in_req_details) {
    int rc = -1;
    std::string selfName = get_self_name();
    QLOGI(LOG_TAG, "%s : inside %s of client ", selfName.c_str(), __func__);
    if (getMemHalAidlAndLinkToDeath() == false) {
...
int  MemHal_SubmitRequest_extn(const std::vector<int32_t>& in_in_list, const std::vector<int32_t>& in_req_details) {
    int rc = -1;
    std::string selfName = get_self_name();
    QLOGI(LOG_TAG, "%s : inside %s of client ", selfName.c_str(), __func__);
    if (getMemHalAidlAndLinkToDeath() == false) {
