/*
 * Copyright (c) 2012, 2017, 2023-2024 Qualcomm Technologies, Inc.  All Rights Reserved.
...
import static org.codeaurora.telephony.utils.RILConstants.GENERIC_FAILURE;
import static org.codeaurora.telephony.utils.RILConstants.RADIO_NOT_AVAILABLE;
import static org.codeaurora.telephony.utils.RILConstants.RIL_REQUEST_OEM_HOOK_RAW;
import static org.codeaurora.telephony.utils.RILConstants.RIL_UNSOL_OEM_HOOK_RAW;
import static org.codeaurora.telephony.utils.RILConstants.RIL_ERRNO_INVALID_RESPONSE;
import static org.codeaurora.telephony.utils.RILConstants.SUCCESS;
...
public class QcrilOemhookMsgTunnel {
    ...
    static final String OEMHOOK_AIDL_SERVICE_NAME =
            "vendor.qti.hardware.radio.qcrilhook.IQtiOemHook/oemhook";
    ...
    public QcrilOemhookMsgTunnel(Integer instanceId) {
        ...
        mContext = QcrilMsgTunnelService.getContext();
        PowerManager pm = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE);
        mWakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, TAG);
        mWakeLock.setReferenceCounted(false);
        mWakeLockTimeout = TelephonyProperties.wake_lock_timeout().orElse(DEFAULT_WAKE_LOCK_TIMEOUT);
        mRequestMessagesWaiting = 0;
        mInstanceId = instanceId;

        if (isAidlAvailable(mInstanceId)) {
            logi("Initializing QtiOemHook AIDL");
            mQtiOemHookAidl = new QtiOemHookAidl(QcrilOemhookMsgTunnel.this, mInstanceId);
            mQtiOemHookAidl.initOemHookAidl();
    ...
    private static boolean isAidlAvailable(Integer instanceId) {
        int osVersion = SystemProperties.getInt("ro.board.api_level", 0);
        int prodVersion = SystemProperties.getInt("ro.product.first_api_level", 0);
        Log.d(TAG, "isAidlAvailable: osVersion= " + osVersion + " prodVersion= " + prodVersion);
        if ((osVersion > Build.VERSION_CODES.S_V2) ||
                ((osVersion == 0) && isAidlHalAvailable())) {
            try {
                return ServiceManager.isDeclared(
                        QcrilOemhookMsgTunnel.OEMHOOK_AIDL_SERVICE_NAME + instanceId);
    ...
    private final class ServiceNotificationCallback extends IServiceNotification.Stub {
        ...
        public void onRegistration(String fqName, String name, boolean preexisting) {
            ...
            if (!mIsServiceConnected) initOemHookHidl();
        }
    }
     ...
     */
    void clearRequestsList(int error) {
        QcRilRequest rr;
        synchronized (mRequestsList) {
            int count = mRequestsList.size();
    ...
    public void invokeOemRilRequestRaw(byte[] data, Message response) {
        ...
        if (mQtiOemHookAidl != null && mQtiOemHookAidl.mQtiOemHook != null) {
            acquireWakeLock();
            queueRequest(rr);
            try {
                logd("Calling AIDL oemHookRawRequest");
                mQtiOemHookAidl.mQtiOemHook.oemHookRawRequest(rr.mSerial, data);
    ...
    private void processOemHookResponse(int serial, int error, ArrayList<Byte> data) {
        ...
        processOemHookSendResponse(serial, error, ret);
    }


    void processOemHookSendResponse(int serial, int errorCode, byte[] data) {
        ...
        if (rr == null) {
            loge("Unexpected serial number in solicited response!serial: "
                    + serial + " error: " + errorCode);
            return;
        }

        if (errorCode != RadioError.NONE) { // Send the response with error to client
            respError = (int)errorCode;
        ...
        sendResponse(rr, respError, data);
    }
    ...
    private void processOemHookIndication(ArrayList<Byte> data) {
        ...
        byte[] ret = arrayListToPrimitiveArray(data);
        processOemHookSendIndication(ret);
    }

    public void processOemHookSendIndication(byte[] data) {
        ByteBuffer oemHookUnsolBuf = ByteBuffer.wrap(data);
        ...
        switch (oemHookUnsolId) {
            case RIL_UNSOL_OEM_HOOK_RAW:
                logd("Received RIL_UNSOL_OEM_HOOK_RAW message"
                      + "length=" + data.length + " " + Arrays.toString(data));
                ...
                if (isQcUnsolOemHookResp(oemHookUnsolBuf)) {
                    ...
                    mUnsolOemHookRawRegistrant.notifyRegistrant(
                        new AsyncResult(null, getArrayPart(data, INT_SIZE), null));
                }
                break;
            default:
    ...
    private void broadcastAudioStateChanged(byte[] data) {
        ...
        logd("AudioState data received: " + new String(data, 0, data.length - 1));
