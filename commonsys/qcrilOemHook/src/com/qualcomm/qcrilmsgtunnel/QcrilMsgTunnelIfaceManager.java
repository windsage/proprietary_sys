/*
 * Copyright (c) 2012, 2017, 2021 Qualcomm Technologies, Inc.  All Rights Reserved.
...
public class QcrilMsgTunnelIfaceManager extends IQcrilMsgTunnel.Stub {
    ...
    private int getPhoneCount() {
        TelephonyManager tm = (TelephonyManager) QcrilMsgTunnelService.getContext()
                .getSystemService(Context.TELEPHONY_SERVICE);
        return tm.getActiveModemCount();
    }

    public void handleOnMultiSimConfigChanged(int activeModemCount) {
        int prevActiveModemCount = mQcrilOemhookMsgTunnel.length;
        if (activeModemCount == prevActiveModemCount) {
            return;
        }
        if (activeModemCount > prevActiveModemCount) {
            switchToMultiSim(prevActiveModemCount, activeModemCount);
        } else {
            switchToSingleSim(prevActiveModemCount, activeModemCount);
        }
    }

    private void switchToSingleSim(int prevActiveModemCount, int activeModemCount) {
        for (int i = activeModemCount; i < prevActiveModemCount; i++) {
            mQcrilOemhookMsgTunnel[i].unSetOnUnsolOemHookExtApp(mMessageHandler);
        }
        mQcrilOemhookMsgTunnel = copyOf(mQcrilOemhookMsgTunnel, activeModemCount);
    }

    private void switchToMultiSim(int prevActiveModemCount, int activeModemCount) {
        mQcrilOemhookMsgTunnel = copyOf(mQcrilOemhookMsgTunnel, activeModemCount);
        // create second instance of QcrilOemhookMsgTunnel and register sub
