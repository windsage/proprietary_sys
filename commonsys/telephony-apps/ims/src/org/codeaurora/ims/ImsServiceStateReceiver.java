/*
 * Copyright (c) 2015-2016, 2020-2022, 2024 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
public class ImsServiceStateReceiver extends BroadcastReceiver {
    ...
    private void showHDIcon(boolean showHDIcon) {
        ...
        if (mShowHDIcon) {
            ...
            builder.setGroup(NOTIFICATION_GROUP + mPhoneId);
            if (mShowVOLTEIcon) {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (subManager != null) {
            subManager = subManager.createForAllUserProfiles();
            subInfo = subManager.getActiveSubscriptionInfoForSimSlotIndex(mPhoneId);
        }
        if (subInfo == null) {
        ...
        if (TelephonyManager.ACTION_SIM_APPLICATION_STATE_CHANGED.equals(intent.getAction())) {
            ...
            if (intentSubId == subId && TelephonyManager.SIM_STATE_LOADED == simStatus) {
                ...
                .equals(CarrierConfigManager.ACTION_CARRIER_CONFIG_CHANGED) || intent.getAction()
                .equals(QtiCallConstants.ACTION_ESSENTIAL_RECORDS_LOADED))) {
            ...
            if (subId == intentSubId) {
                updateHDIcon(mServiceSub.isVideoSupported(), mServiceSub.isVoiceSupported());
                mServiceSub.onCarrierConfigChanged();
            }
        }
    }
}
