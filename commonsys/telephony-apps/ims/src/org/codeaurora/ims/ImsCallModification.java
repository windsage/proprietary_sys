...
public class ImsCallModification {
    ...
    public ImsCallModification(ImsCallSessionImpl imsCallSessionImpl, Context context,
        ...
        this(imsCallSessionImpl, context, senderRxr, Looper.getMainLooper());
    }
    ...
    public ImsCallModification(ImsCallSessionImpl imsCallSessionImpl, Context context,
        ...
        mPhoneId = mImsCallSessionImpl.getPhoneId();
    ...
    private boolean isIndexValid() {
        final int index = mImsCallSessionImpl.getCallIdInt();
        return (index >= 0);
    }
    ...
    private boolean isVideoCrbtCall(CallDetails modifyCallDetails) {
        boolean isCrbtCall = modifyCallDetails.isCrbtCall();
        // Backward compatibility with old RIL/Modem API which doesn't support CRBT tlv
        boolean isCrbtCallBackwardCompatibility =
                (mImsCallSessionImpl.getInternalState() == DriverCallIms.State.ALERTING
                || mImsCallSessionImpl.getInternalState() == DriverCallIms.State.DIALING)
                && QtiImsExtUtils.isVideoCrbtSupported(mPhoneId, mContext);
        Log.d(this, "isCrbtCall: " + isCrbtCall + ", isCrbtCallBackwardCompatibility: "
                + isCrbtCallBackwardCompatibility);
        if ((isCrbtCall || isCrbtCallBackwardCompatibility)
                && modifyCallDetails.call_type == CallDetails.CALL_TYPE_VT_RX) {
            Log.i(this, "This is Video CRBT call");
            return true;
        }
        return false;
    }

    private boolean isVisualizedVoiceCall(CallDetails modifyCallDetails) {
        if (modifyCallDetails.isVisualizedVoiceCall()
                && modifyCallDetails.call_type == CallDetails.CALL_TYPE_VT_RX) {
            Log.i(this, "This is Visualized Voice call");
            return true;
        }
        return false;
    }
    ...
    private boolean validateIncomingModifyConnectionType(int newCallType) {
        ...
        if (!QtiImsExtUtils.allowVideoCallsInLowBattery(mPhoneId, mContext)) {
            isLowBattery = LowBatteryHandler.getInstance().isLowBattery(mPhoneId);
        }
    ...
    public void onReceivedModifyCall(CallModify callModify) {
        ...
        if (callModify.error()) {
            ...
            if (isVideoCrbtCall(callModify.call_details)
                    || isVisualizedVoiceCall(callModify.call_details)) {
                Log.e(this, "videocall onReceivedModifyCall: not notifying user about"
                        + "incoming modify call request as this is CRBT/UVS call. "
                        + "Don't cache the request: " + mCallModifyRequest);
                ret = false;
            } else if (this.mCallModifyRequest == null) {
        ...
        // If more than one upgrade request as part of UNSOL_MODIFY_CALL then give priority to RTT
        // upgrade and drop VT upgrade.
        if (rttChanged) {
            if (QtiImsExtUtils.isRttAutoUpgradeSupported(mPhoneId, mContext)) {
    ...
    private void retryAvpUpgrade(boolean shouldNotifyUser) {
        ...
        mIndex = mImsCallSessionImpl.getCallIdInt();
    ...
    public void rejectConnectionTypeChange(Message msg) {
        ...
        callModify.call_index  = mImsCallSessionImpl.getCallIdInt();
    ...
    private boolean isUpgradeRequestForRtt(CallDetails callDetails) {
        ...
        return ((callDetails.rttMode == ImsStreamMediaProfile.RTT_MODE_FULL) && isIndexValid());
    }
    ...
    private boolean canModifyRttCallType(int toCallType) {
        ...
        boolean isVideoRttSupported = QtiImsExtUtils.isRttSupportedOnVtCalls(mPhoneId, mContext);
