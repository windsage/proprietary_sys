...
public class CallDetails {
    ...
    public static final int CALL_TYPE_CALLCOMPOSER = 26;
     ...
     /* This is internal call type for USSD over IMS capability */
    public static final int CALL_TYPE_USSD = 27;

    /* This is internal call type to handle IMS Data Channel service */
    ...
    public int rttMode = ImsStreamMediaProfile.RTT_MODE_DISABLED;
    public String sipAlternateUri;
    private MultiIdentityLineInfo mLineInfo;
    private boolean mIsVosSupported = false;
    private boolean mIsVisualizedVoiceCall = false;
    private boolean mIsCrbtCall = false;
    private int mThreeDimensionalVideoType = QtiCallConstants.CALL_GFREE_3D_NONE;
    ...
    public CallDetails(CallDetails srcCall) {
        if (srcCall != null) {
            ...
            mIsVosSupported = srcCall.mIsVosSupported;
            mIsVisualizedVoiceCall = srcCall.mIsVisualizedVoiceCall;
            mIsCrbtCall = srcCall.mIsCrbtCall;
            mThreeDimensionalVideoType = srcCall.mThreeDimensionalVideoType;
        }
    }
    ...
    public boolean update(CallDetails update) {
        ...
        if (mIsVosSupported != update.mIsVosSupported) {
            mIsVosSupported = update.mIsVosSupported;
            hasChanged = true;
        }

        if (mIsVisualizedVoiceCall != update.mIsVisualizedVoiceCall) {
            mIsVisualizedVoiceCall = update.mIsVisualizedVoiceCall;
            hasChanged = true;
        }

        if (mIsCrbtCall != update.mIsCrbtCall) {
            mIsCrbtCall = update.mIsCrbtCall;
            hasChanged = true;
        }

        if (mThreeDimensionalVideoType != update.mThreeDimensionalVideoType) {
            mThreeDimensionalVideoType = update.mThreeDimensionalVideoType;
            hasChanged = true;
        }
    ...
    public void setVosSupport(boolean isVosSupported) {
        mIsVosSupported = isVosSupported;
    }

    public boolean getVosSupport() {
        return mIsVosSupported;
    }

    public void setVisualizedVoiceCall(boolean isVisualizedVoiceCall) {
        mIsVisualizedVoiceCall = isVisualizedVoiceCall;
    }

    public boolean isVisualizedVoiceCall() {
        return mIsVisualizedVoiceCall;
    }

    public void setCrbtCall(boolean isCrbtCall) {
        mIsCrbtCall = isCrbtCall;
    }

    public boolean isCrbtCall() {
        return mIsCrbtCall;
    }

    public void setThreeDimensionalVideoType(int threeDimensionalVideoType) {
        mThreeDimensionalVideoType = threeDimensionalVideoType;
    }

    public int getThreeDimensionalVideoType() {
        return mThreeDimensionalVideoType;
    }
    ...
    public String toString() {
        ...
        return (" " + call_type
                ...
                + ", " + mLineInfo
                + " isVosSupported " + mIsVosSupported
                + " isVisualizedVoiceCall " + mIsVisualizedVoiceCall
                + " isCrbtCall " + mIsCrbtCall
                + " threeDimensionalVideoType " + mThreeDimensionalVideoType);
