...
public class CameraController implements ImsMedia.CameraListener,
     ...
     */
    public static CameraController init(Context context, ImsMedia media) {
        if (sInstance == null) {
    ...
    public void setPreviewSurface(Surface surface, ImsCallSessionImpl session) {
        ...
        try {
            // We need to restart preview/recording if
            // - preview surface or surface size has changed
            // - or recording surface size has changed from current recording
            // We check for recording size changed here instead of
            // onCameraConfigChanged() to avoid back to back recording restarts
            // due to preview surface and recording surface changes.
            if (surface != null && ((mPreviewSurface != null
                    ...
                    mCamera.isRestartNeededOnRecordingSurfaceSizeChanged(mRecordingSurface))
                    ))) {
    ...
    public void onRecordingEnabled(int mediaId) {
        ...
        checkMediaId("onRecordingEnabled", mediaId);
        try {
    ...
    public void onRecordingDisabled(int mediaId) {
        ...
        checkMediaId("onRecordingDisabled", mediaId);
    ...
    public void onCameraConfigChanged(int mediaId, int w, int h, int fps,
        ...
        checkMediaId("onCameraConfigChanged", mediaId);
        ...
        try {
            ...
            mCamera.reconfigure(mCameraConfigIms);
            onUpdateRecorderFrameRate(mediaId, mRecorderFrameRate);
            updateCameraSession();
    ...
    public void onUpdateRecorderFrameRate(int mediaId, int rate) {
        ...
        checkMediaId("onUpdateRecorderFrameRate", mediaId);
    ...
    public void onClosed(QImsSessionBase session) {
        ...
        Log.i(this, "onClosed: Session=" + getSessionInfo(callSession));
        close(callSession);
    }
    ...
    private void checkMediaId(String msg, int mediaId) {
        if (mCameraOwner != null && mCameraOwner.getMediaId() != mediaId) {
            Log.v(this, msg +  " mediaId mismatch with mCameraOwner");
        }
    }
}
