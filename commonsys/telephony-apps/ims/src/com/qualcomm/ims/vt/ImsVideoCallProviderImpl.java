/* Copyright (c) 2014-2015, 2018-2020, 2023 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
public class ImsVideoCallProviderImpl extends ImsVideoCallProvider implements
    ...
    public void enableArMode(String cameraId) {
        log("enableArMode camera id = " + cameraId);
        mMedia.enableArMode(cameraId);
    }

    public void setLocalRenderingDelay(int delay) {
        log("setLocalRenderingDelay");
    ...
    public void onUpdateRecorderFrameRate(int rate) {
        log("onUpdateRecorderFrameRate = " + rate);
        if (!isSessionValid()) return;
        mCallSession.updateRecorderFrameRate(rate);
    }

    public void onRecordingEnabled() {
        log("onRecordingEnabled ");
        if (!isSessionValid()) return;
        mCallSession.updateRecordingEnabled();
    }

    public void onRecordingDisabled() {
        log("onRecordingDisabled ");
        if (!isSessionValid()) return;
        mCallSession.updateRecordingDisabled();
    }
