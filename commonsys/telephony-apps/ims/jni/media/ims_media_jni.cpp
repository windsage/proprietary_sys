...
static jint dpl_getDownscaledHeight(JNIEnv*, jobject) {
    jint def = 240;
    ALOGD("%s", __func__);

    if (imsMediaApis && imsMediaApis->getDownscaledHeight) {
        return static_cast<jint>(imsMediaApis->getDownscaledHeight());
    }
    return def;
}

static jint dpl_getDownscaledWidth(JNIEnv *, jobject) {
    jint def = 320;
    ALOGD("%s", __func__);

    if(imsMediaApis && imsMediaApis->getDownscaledWidth) {
        return static_cast<jint>(imsMediaApis->getDownscaledWidth());
    }
    return def;
}
...
static void onDataUsageEvent(JNIEnv *e, int32_t event, int32_t mediaID) {
    ...
    const unsigned short DATA_USAGE_WWAN = 0;
    const unsigned short DATA_USAGE_WLAN = 1;
    const unsigned short DATA_USAGE_C_IWLAN = 2;
    ...
    const unsigned short ratArray[] = {DATA_USAGE_WWAN, DATA_USAGE_WLAN, DATA_USAGE_C_IWLAN};
...
static jint dpl_setLocalRenderingParameters(JNIEnv *, jobject, jint width, jint height) {
    ALOGD("%s width = %d height = %d", __func__, width, height);
    jint ret = -1;
    if (!imsMediaApis) {
        ALOGE("%s: imsMediaApis is null", __func__);
    ...
    if (imsMediaApis->setLocalRenderingParameters) {
        return static_cast<jint>(imsMediaApis->setLocalRenderingParameters(
                    static_cast<int>(width),
                    static_cast<int>(height)));
    }

    ALOGE("%s: setLocalRenderingParameters is null", __func__);
    return ret;
}

static jint dpl_setLocalRenderingDelay(JNIEnv *, jobject, jint delay) {
    ...
    if (imsMediaApis->setLocalRenderingSDKDelay) {
        return static_cast<jint>(imsMediaApis->setLocalRenderingSDKDelay(
                    static_cast<int>(delay)));
    }

    ALOGE("%s: setLocalRenderingDelay is null", __func__);
    return ret;
}
...
static JNINativeMethod sMethods[] =
{
    ...
    {"nativeSetLocalRenderingParameters",   "(II)I",                              (void *)dpl_setLocalRenderingParameters},
    {"nativesetLocalRenderingDelay",   "(I)I",                              (void *)dpl_setLocalRenderingDelay},
    {"nativeGetDownscaledHeight",          "()I",                                (void *)dpl_getDownscaledHeight},
    {"nativeGetDownscaledWidth",           "()I",                                (void *)dpl_getDownscaledWidth}
};
...
#define IMPL_SYM_SET_LOCAL_RENDERING_PARAMETERS      "setLocalRenderingParameters"
#define IMPL_SYM_SET_LOCAL_RENDERING_DELAY      "setLocalRenderingSDKDelay"
#define IMPL_SYM_DOWN_HEIGHT                  "getDownscaledHeight"
#define IMPL_SYM_DOWN_WIDTH                   "getDownscaledWidth"
...
struct ImsMediaApis *load_ims_media_lib(const char *path)
{
    ...
    ret->setLocalRenderingParameters
        = (ImsMediaSetLocalRenderingParametersFuncPtr) dlsym(handle,
                IMPL_SYM_SET_LOCAL_RENDERING_PARAMETERS);
    ret->setLocalRenderingSDKDelay
        = (ImsMediasetLocalRenderingDelayFuncPtr) dlsym(handle,
                IMPL_SYM_SET_LOCAL_RENDERING_DELAY);
    ret->getDownscaledWidth
        = (ImsMediaIntVoidFuncPtr) dlsym(handle,
                IMPL_SYM_DOWN_WIDTH);
    ret->getDownscaledHeight
        = (ImsMediaIntVoidFuncPtr) dlsym(handle,
                IMPL_SYM_DOWN_HEIGHT);
