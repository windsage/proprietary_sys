...
public class MbnFileActivate extends Activity {
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (MbnTestUtils.PDC_DATA_ACTION.equals(action)) {
                ...
                final byte[] result = intent.getByteArrayExtra("list_data");
                if (result != null) {
                    mExecutor.execute( new Runnable() {
                        public void run() {
                            ArrayList<MbnMetaInfo> mbnMetaInfoList = parseConfigList(result);
                            if (mbnMetaInfoList != null) {
                                Message msg = mHandler.obtainMessage(EVENT_PDC_CONFIG_LIST_DONE);
                                msg.obj = mbnMetaInfoList;
                                msg.sendToTarget();
                            }
                        }
                    });
    ...
    private Handler mHandler = new Handler() {
        ...
        public void dispatchMessage(Message msg) {
            ...
            case EVENT_PDC_CONFIG_LIST_DONE:
                ...
                if (msg.obj != null) {
                    ArrayList<MbnMetaInfo> result = (ArrayList<MbnMetaInfo>)msg.obj;
                    if (result != null) {
                        mMbnMetaInfoList.addAll(result);
                    }
                }

                //sort MBN list by meta info
                Collections.sort(mMbnMetaInfoList, new Comparator<MbnMetaInfo>() {
                    @Override
                    public int compare(MbnMetaInfo o1, MbnMetaInfo o2) {
                        return o1.getMetaInfo().compareToIgnoreCase(o2.getMetaInfo());
                    }
                });
                ...
                if (mMetaAdapter != null) {
                    log("mMbnMetaInfoList's size: " + mMbnMetaInfoList.size());
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = this;
        MbnTestUtils.setupEdgeToEdge(this);
    ...
    // False, otherwise
    private ArrayList<MbnMetaInfo> parseConfigList(byte[] data) {
        boolean ret = false;
        ...
        ArrayList<MbnMetaInfo> mbnMetaInfoList = new ArrayList<MbnMetaInfo>();
        for (int i = 0; i < numConfigs; i++) {
            ...
            mbnMetaInfoList.add(metaItem);
        }

        return mbnMetaInfoList;
    }
    ...
    private void setActivityView() {
        ...
        if (mMetaAdapter == null) {
            mMetaAdapter = new MetaInfoListAdapter(mContext, R.layout.meta_info_list,
                mMbnMetaInfoList);
