...
public class DeviceInfo extends PreferenceActivity {
    ...
    private class ObtainIdHandler extends Handler {
        ...
        private void handleGetEFRuim(Message msg) {
            ...
            if (ar.exception == null) {
                ...
                if (numOfBytes >= 0 && numOfBytes < NUM_BYTES_RUIM_ID) {
        ...
        private void handleGetDeviceIdentity(Message msg) {
            ...
            if (ar.exception == null) {
                ...
                // If the ESN/MEID is invalid, remove its preference
                if (invalidMeid(respId[2])) {
                    removePreference(KEY_ESN, getPreferenceScreen());
                }
                if (invalidMeid(respId[3])) {
                    removePreference(KEY_MEID, getPreferenceScreen());
                }
                // Need clear calling identity due to run in phone with system uid
    ...
    private void checkDangerousPermissions() {
        if (checkSelfPermission(Manifest.permission.READ_PHONE_STATE) ==
            ...
            Log.d(TAG,"DeviceInfo : READ_PHONE_STATE is NOT granted");
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions,
        ...
        if (requestCode == READ_PHONE_STATE_PERMISSION_CODE) {
            Log.v(TAG, "request Permission :" + Arrays.toString(permissions));
            ...
            displayAlertDialog(R.string.grant_permissions_prompt);
        }
    }
     ...
     * in this activity.
     *
     * Since there are a few fields that are specific to CDMA, we first check if there is any Phone
     * object with {@link TelephonyManager#PHONE_TYPE_CDMA}.
     *
     * If no CDMA type Phone is found, we go on to choose the Phone corresponding to the first slot
     * that has an active SIM card.
     *
     * If there is no SIM on the device, we simply return the Phone object corresponding to the
     * first slot.
     *
    ...
    private void updateImeiByPhone() {
        ...
        for (int slot = 0; slot < PhoneConstants.MAX_PHONE_COUNT_DUAL_SIM; slot++) {
            if (slot < mNumPhone) {
                ...
                if (phone != null) {
                    imei = phone.getImei();
                    Log.d(TAG, "phone getImei on slot " + slot + ": " + pii(imei));
                }
                ...
                if (qtiImeiInfo != null) {
                    for (int i = 0; i < qtiImeiInfo.length; i++) {
                      if (null != qtiImeiInfo[i] &&
                              qtiImeiInfo[i].getSlotId() == slot) {
                          imei = qtiImeiInfo[i].getImei();
                          Log.d(TAG, "getImei: " + pii(imei) + " on slot" + slot);
                      }
                    }
                }
