...
import com.qualcomm.qti.wigig.WifiConfiguration.NetworkSelectionStatus;
import com.qualcomm.qti.wigig.WifiConfiguration.Status;
...
import com.qualcomm.qti.server.wigig.IpConfigStore;
...
import static com.qualcomm.qti.wigig.WifiConfiguration.INVALID_NETWORK_ID;
...
public class WigigConfigStore extends IpConfigStore {
    ...
    NetworkUpdateResult saveNetwork(WifiConfiguration config, int uid) {
        ...
        if (DBG) WLog.v(TAG, "saveNetwork got it back netId=" + netId);
        ...
        if (config != null) {
            if (DBG) {
                Log.e(TAG, "removeNetwork " + Integer.toString(netId) + " key=" +
                        config.getKey() + " config.id=" + Integer.toString(config.networkId));
            }

            // cancel the last user choice
            if (config.getKey().equals(lastSelectedConfiguration)) {
    ...
    void loadConfiguredNetworks() {
        ...
        while (!done) {
            ...
            if (showNetworks) {
                WLog.v(TAG, "loadConfiguredNetworks:  ");
            ...
            for (int i = 1; i < lines.length; i++) {
                ...
                if (mConfiguredNetworks.getByConfigKey(config.getKey()) != null) {
                    ...
                    if (showNetworks) WLog.v(TAG, "Ignoring loaded configured for network " + config.networkId
    ...
    public void setLastSelectedConfiguration(int netId) {
        ...
        if (netId == WifiConfiguration.INVALID_NETWORK_ID) {
            ...
            if (selected == null) {
                ...
                lastSelectedConfiguration = selected.getKey();
    ...
    private NetworkUpdateResult addOrUpdateNetworkNative(WifiConfiguration config, int uid) {
        ...
        if (netId == INVALID_NETWORK_ID) {
            WifiConfiguration savedConfig = mConfiguredNetworks.getByConfigKey(config.getKey());
            if (savedConfig != null) {
        ...
        setVariables: {
            ...
            if (!((newNetwork == false) && (savedConfig != null) &&
                   ...
                   && (config.BSSID != null)) {
                Log.i(TAG, "Setting BSSID for " + config.getKey() + " to " + config.BSSID);
    ...
    void handleSSIDStateChange(int netId, boolean enabled) {
        ...
        WLog.v(TAG, "SSID " + (enabled ? "re-enabled" : "temp disabled") + " from supplicant: "
                + config.getKey());
        if (enabled) {
