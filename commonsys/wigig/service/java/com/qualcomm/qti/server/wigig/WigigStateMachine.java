...
import com.qualcomm.qti.wigig.SupplicantState;
...
import com.qualcomm.qti.wigig.WifiSsid;
...
public class WigigStateMachine extends StateMachine {
    ...
    /**
     * copy of WifiManager.WIFI_COUNTRY_CODE_CHANGED_ACTION, which is a hidden field
     */
    public static final String WIFI_COUNTRY_CODE_CHANGED_ACTION =
        "android.net.wifi.COUNTRY_CODE_CHANGED";

    /**
     * copy of WifiManager.EXTRA_COUNTRY_CODE, which is a hidden field
     */
    ...
    private final ExternalCallbackTracker<IWpsCallback> mProcessingWpsCallbacks;
    ...
    public WigigStateMachine(Context context, Handler handler, String wlanInterface, 
                             WigigNetworkMonitor networkMonitor) {
        super("WigigStateMachine", handler);
        mContext = context;
        ...
        // TODO update WIFI resources
        mPrimaryDeviceType = "10-0050F204-5"; /* mContext.getResources().getString(
        ...
        filter.addAction(WIFI_COUNTRY_CODE_CHANGED_ACTION);
        ...
        mContext.registerReceiver(
                new BroadcastReceiver() {
                    ...
                    public void onReceive(Context context, Intent intent) {
                        ...
                        } else if (action.equals(WIFI_COUNTRY_CODE_CHANGED_ACTION)) {
                            sendMessage(CMD_COUNTRY_CODE_CHANGED,
                                    intent.getStringExtra(EXTRA_COUNTRY_CODE));
                        }
                    }
                }, filter);
        ...
        mTcpBufferSizes = mCapConfigStoreHal.getConfigString(
            "npt.tcp_buffer_sizes", "524288,1048576,2097152,262144,524288,1048576");
            /*mContext.getResources().getString(R.string.config_wifi_tcp_buffers)); */

        mProcessingActionListeners = new ExternalCallbackTracker<>(getHandler());
        mProcessingWpsCallbacks = new ExternalCallbackTracker<>(getHandler());
    ...
    private boolean startTethering(ArrayList<String> available) {
        ...
        for (String intf : available) {
            ...
            try {
                ...
                if (ifcg != null) {
                    // IP address will be set by tethering service
                    ifcg.setInterfaceUp();
    ...
    private void clearLinkProperties() {
        ...
        synchronized (mDhcpResultsLock) {
            mDhcpResults = new DhcpResultsParcelable();
        }
...
//        }
    ...
    class DefaultState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WigigManager.FORGET_NETWORK:
                case WigigManager.SAVE_NETWORK:
                    sendActionListenerFailure(message.arg2, WigigManager.BUSY);
                    break;
                case WigigManager.START_WPS:
                case WigigManager.CANCEL_WPS:
                    sendWpsCallbackFailed(message.arg2, WigigManager.BUSY);
                    break;
                case CMD_GET_CONFIGURED_NETWORKS:
            ...
            switch(message.what) {
                ...
                case WigigManager.SAVE_NETWORK:
                    ...
                    if (config == null) {
                        ...
                        sendActionListenerFailure(message.arg2, WigigManager.ERROR);
                        break;
                    }
                ...
                case WigigManager.START_WPS:
                    ...
                    if (wpsResult.status == Status.SUCCESS) {
                        sendWpsCallbackStarted(message.arg2, wpsResult.pin);
                        transitionTo(mWpsRunningState);
                    } else {
                        loge("Failed to start WPS with config " + wpsInfo.toString());
                        sendWpsCallbackFailed(message.arg2, WigigManager.ERROR);
                    }
                    break;
                case WigigMonitor.NETWORK_CONNECTION_EVENT:
    ...
    private class WigigNetworkAgent extends NetworkAgent {
      ...
      private static final String PROVIDER_NAME = "WigigNetworkAgentProvider";

        public WigigNetworkAgent(Looper l, Context c, String TAG, NetworkInfo ni,
                NetworkCapabilities nc, LinkProperties lp, int score) {
          super(c, l, TAG, nc, lp, score,
              new NetworkAgentConfig.Builder().setLegacyType(ni.getType()).setLegacyTypeName(ni.getTypeName()).build(),
              new NetworkProvider(c, l, PROVIDER_NAME));
        }
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case CMD_ADD_LOCAL_ROUTE:
                    ...
                    if (mNetworkAgent != null && mNetworkAgent.getNetwork() != null &&
                        mNetworkAgent.getNetwork().netId == network.netId) {
        ...
        public void enter() {
            if (DBG) {
                ...
                if (getCurrentWifiConfiguration() != null) {
                    key = getCurrentWifiConfiguration().getKey();
                }
            ...
            switch (message.what) {
                ...
                case WigigMonitor.SCAN_FAILED_EVENT:
                    ...
                    synchronized (mScanResultCache) {
                        ...
                        for (ScanResult result : mScanResults) {
                            if (!result.wifiSsid.toString().equals(mWifiInfo.getWifiSsid().toString())) {
                                // skip different SSID
                                continue;
                            }
    ...
    public void connect(WifiConfiguration config, int netId, IBinder binder,
        ...
        getHandler().post(() -> {
            ...
            Message message =
                    obtainMessage(WigigManager.CONNECT_NETWORK, netId, callbackIdentifier, config);
            sendMessage(message);
        });
    }
    ...
    public void save(WifiConfiguration config, IBinder binder, IActionListener callback) {
        getHandler().post(() -> {
            ...
            Message message =
                    obtainMessage(WigigManager.SAVE_NETWORK, -1 , callbackIdentifier, config);
            sendMessage(message);
        });
    }
    ...
    public void forget(int netId, IBinder binder, IActionListener callback) {
        getHandler().post(() -> {
            ...
            Message message =
                    obtainMessage(WigigManager.FORGET_NETWORK, netId , callbackIdentifier);
            sendMessage(message);
        });
    }

    private void sendWpsCallbackStarted(int callbackIdentifier, String pin) {
        IWpsCallback wpsCallback;
        synchronized (mProcessingWpsCallbacks) {
            // not the final callback so do not remove from the tracker
            wpsCallback = mProcessingWpsCallbacks.get(callbackIdentifier);
        }
        if (wpsCallback != null) {
            try {
                wpsCallback.onStarted(pin);
            } catch (RemoteException e) {
                // no-op (client may be dead, nothing to be done)
            }
        }
    ...
    private void sendWpsCallbackSucceeded(int callbackIdentifier) {
        IWpsCallback wpsCallback;
        synchronized (mProcessingWpsCallbacks) {
            wpsCallback = mProcessingWpsCallbacks.remove(callbackIdentifier);
        }
        if (wpsCallback != null) {
            try {
                wpsCallback.onSucceeded();
            } catch (RemoteException e) {
                // no-op (client may be dead, nothing to be done)
            }
        }
    ...
    private void sendWpsCallbackInProgress(int callbackIdentifier) {
        IWpsCallback wpsCallback;
        synchronized (mProcessingWpsCallbacks) {
            // not the final callback so do not remove from the tracker
            wpsCallback = mProcessingWpsCallbacks.get(callbackIdentifier);
        ...
        if (wpsCallback != null) {
            try {
                wpsCallback.onFailed(WigigManager.IN_PROGRESS);
            } catch (RemoteException e) {
                // no-op (client may be dead, nothing to be done)
            }
        }
    ...
    private void sendWpsCallbackFailed(int callbackIdentifier, int reason) {
        IWpsCallback wpsCallback;
        synchronized (mProcessingWpsCallbacks) {
            wpsCallback = mProcessingWpsCallbacks.remove(callbackIdentifier);
        }
    ...
    /**
     * Trigger WPS and provide status via the provided callback.
     */
    public void startWps(WpsInfo config, IBinder binder, IWpsCallback callback) {
        getHandler().post(() -> {
            int callbackIdentifier = ++mCallbackIdentifier;
            if (callback != null && binder != null) {
                mProcessingWpsCallbacks.add(binder, callback, callbackIdentifier);
            }

            Message message =
                    obtainMessage(WigigManager.START_WPS, -1, callbackIdentifier, config);
            sendMessage(message);
        });
    }

    /**
     * Cancel ongoing WPS process
     */
    public void cancelWps(IBinder binder, IWpsCallback callback) {
        getHandler().post(() -> {
            ...
            Message message =
                    obtainMessage(WigigManager.CANCEL_WPS, -1, callbackIdentifier);
            sendMessage(message);
        });
    }
    ...
    class WpsRunningState extends State {
        ...
        public void enter() {
            ...
            callbackIdentifier = mSourceMessage.arg2;
        }
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case WigigMonitor.WPS_SUCCESS_EVENT:
                    ...
                    break;
                case WigigMonitor.NETWORK_CONNECTION_EVENT:
                    sendWpsCallbackSucceeded(callbackIdentifier);
                ...
                case WigigMonitor.WPS_OVERLAP_EVENT:
                    sendWpsCallbackFailed(callbackIdentifier, WigigManager.WPS_OVERLAP_ERROR);
                    mSourceMessage.recycle();
                    mSourceMessage = null;
                    transitionTo(mDisconnectedState);
                    break;
                case WigigMonitor.WPS_FAIL_EVENT:
                    ...
                    if ((message.arg1 != WigigManager.ERROR) || (message.arg2 != 0)) {
                        sendWpsCallbackFailed(callbackIdentifier, message.arg1);
                        mSourceMessage.recycle();
                        mSourceMessage = null;
                        transitionTo(mDisconnectedState);
                    } else {
                ...
                case WigigMonitor.WPS_TIMEOUT_EVENT:
                    sendWpsCallbackFailed(callbackIdentifier, WigigManager.WPS_TIMED_OUT);
                    mSourceMessage.recycle();
                    mSourceMessage = null;
                    transitionTo(mDisconnectedState);
                    break;
                case WigigManager.START_WPS:
                    sendWpsCallbackInProgress(message.arg2);
                    break;
                case WigigManager.CANCEL_WPS:
                    if (mWigigNative.cancelWps()) {
                        sendWpsCallbackSucceeded(message.arg2);
                    } else {
                        sendWpsCallbackFailed(message.arg2, WigigManager.ERROR);
                    }
                    transitionTo(mDisconnectedState);
    ...
    private void checkAndAddLocalRoute() {
        ...
        // TODO the addLegacyRouteForNetId was deprecated and cannot be used. Comment
        // this out for now because it can break the build. Need to find an alternative
        // later
        /*
        for (LinkAddress addr : addrs) {
            ...
            try {
                mNwService.addLegacyRouteForNetId(mNetworkAgent.getNetwork().netId, route, 0);
            } catch (Exception e) {
