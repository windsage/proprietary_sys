...
extern pthread_rwlock_t log_rw_mutex;
...
#define IMS_COMMON1(priority,f) \
        if(priority & logPriorityValue) {\
          if (bInit_Success) {\
            pthread_rwlock_rdlock(&log_rw_mutex);\
            MSG_2(MSG_SSID_IMS, MSG_LEGACY_HIGH,str_cat([SIMSVT] %d %d ,f), \
              getpid(),gettid());\
            pthread_rwlock_unlock(&log_rw_mutex);}\
...
typedef struct {
  int iHeight;
  int iWidth;
  uint8_t *pChromaPlaneBuf;
  int chromaBufStride;

  //Y-Plane
  uint8_t *yPlanedataPtr;
  int32_t yPlanePixelStride;
  int32_t yPlaneRowStride;

  //U-Plane
  uint8_t *cbDataPtr;  
  int32_t cbPlanePixelStride;
  int32_t cbPlaneRowStride;
  
  //V-Plane
  uint8_t*crDataPtr;
  int32_t crPlanePixelStride;
  int32_t crPlaneRowStride;
} IMAGE_UV_DATA;


void RotateYUVPlanesByMountAngle(uint8_t *pFramebuffer, IMAGE_UV_DATA *stUVData, int iRotation);
...
#define ERROR1(f)                 __android_log_print(ANDROID_LOG_ERROR, "SIMSVT", f);\
                                  if(bInit_Success) {\
                                    pthread_rwlock_rdlock(&log_rw_mutex);\
                                    MSG_SPRINTF_2(MSG_SSID_IMS, MSG_LEGACY_ERROR,str_cat([SIMSVT] %d %d ,f), \
                                      getpid(),gettid());\
                                    pthread_rwlock_unlock(&log_rw_mutex); }
#define ERROR2(f,a1)              __android_log_print(ANDROID_LOG_ERROR, "SIMSVT", f,a1);\
                                  if(bInit_Success) {\
                                    pthread_rwlock_rdlock(&log_rw_mutex);\
                                    MSG_SPRINTF_3(MSG_SSID_IMS, MSG_LEGACY_ERROR,str_cat([SIMSVT] %d %d ,f), \
                                      getpid(),gettid(),a1);\
                                    pthread_rwlock_unlock(&log_rw_mutex); }
#define ERROR3(f,a1,a2)           __android_log_print(ANDROID_LOG_ERROR, "SIMSVT", f,a1,a2);\
                                  if(bInit_Success) {\
                                    pthread_rwlock_rdlock(&log_rw_mutex);\
                                    MSG_SPRINTF_4(MSG_SSID_IMS, MSG_LEGACY_ERROR,str_cat([SIMSVT] %d %d ,f), \
                                      getpid(),gettid(),a1,a2);\
                                    pthread_rwlock_unlock(&log_rw_mutex); }
#define ERROR4(f,a1,a2,a3)        __android_log_print(ANDROID_LOG_ERROR, "SIMSVT", f,a1,a2,a3);\
                                  if(bInit_Success) {\
                                    pthread_rwlock_rdlock(&log_rw_mutex);\
                                    MSG_SPRINTF_5(MSG_SSID_IMS, MSG_LEGACY_ERROR,str_cat([SIMSVT] %d %d ,f), \
                                      getpid(),gettid(),a1,a2,a3);\
                                    pthread_rwlock_unlock(&log_rw_mutex); }
#define ERROR5(f,a1,a2,a3,a4)     __android_log_print(ANDROID_LOG_ERROR, "SIMSVT", f,a1,a2,a3,a4);\
                                  if(bInit_Success) {\
                                    pthread_rwlock_rdlock(&log_rw_mutex);\
                                    MSG_SPRINTF_6(MSG_SSID_IMS, MSG_LEGACY_ERROR,str_cat([SIMSVT] %d %d ,f), \
                                      getpid(),gettid(),a1,a2,a3,a4);\
                                    pthread_rwlock_unlock(&log_rw_mutex); }
#define ERROR6(f,a1,a2,a3,a4,a5)  __android_log_print(ANDROID_LOG_ERROR, "SIMSVT", f,a1,a2,a3,a4,a5);\
                                  if(bInit_Success) {\
                                    pthread_rwlock_rdlock(&log_rw_mutex);\
