...
static sp<hardware::media::omx::V1_0::IGraphicBufferSource>
...
QPE_VIDEO_ERROR VideoTransmitter::EncodeConfigure(QP_VIDEO_CONFIG tCodecConfig)
{
  ...
  if ( (m_codecConfig.eHALVersion != CAMERA_HAL1) &&
    ...
    mGraphicBufferSource->configure( new TWOmxNode(m_NodeId),
  ...
  if (!isTrebleEnabled) {
    ...
    for (uint8_t i = 0; i < InputportFmt.nBufferCountActual; i++) {
      ...
      //uses as use buffer
      if (Inputmem[i] != NULL) {
        m_pEncInpBufHeader_iomx[i]->pBuffer = Inputmem[i]->unsecurePointer();
  ...
  if (!isTrebleEnabled) {
    ...
    for (uint8_t i = 0; i < portFmt.nBufferCountActual; i++) {
      ...
      //uses as use buffer
      if (Outputmem[i] != NULL) {
        m_pEncOutpBufHeader_iomx[i]->pBuffer = Outputmem[i]->unsecurePointer();
