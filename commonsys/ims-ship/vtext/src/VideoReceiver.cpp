                                      ...
                                      int belongs_to_drop_set, long long int ullCurSysTime)
{
  ...
  frameStateSet[nIdx].system_time_decode_video_frame =
...
void VideoReceiver::HandleEvent(void* data)
{
  ...
  if (VIDEO_SURFACE_PENDING == m_ePlayerState
      || (mNativeWindowSurfaceFar.get() == NULL)) {
...
QPE_VIDEO_ERROR VideoReceiver::ConfigureDecoder(QP_VIDEO_CONFIG tCodecConfig)
{
  ...
  if (!isTrebleEnabled) {
    ...
    for (uint8_t i = 0; i < portInpFmt.nBufferCountActual; i++) {
      ...
      //uses as use buffer
      if (mem != NULL) {
        m_pDecInpBufHeader_iomx[i]->pBuffer = mem->unsecurePointer();
