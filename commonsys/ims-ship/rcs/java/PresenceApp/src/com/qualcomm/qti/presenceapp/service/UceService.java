...
public class  UceService {
    ...
    public static Map<Integer, Boolean> isPublishTriggerReceived = new HashMap<Integer, Boolean>();
    ...
    public void publish(int instance, CapInfo caps) {
        ...
        if(isServiceInit.get(instance) == null || (!isServiceInit.get(instance))) {
            Log.e(LOG_TAG," publish not trigerred because uce service not yet up");
            return;
        }
        if(isPublishTriggerReceived.get(instance) == null || (!isPublishTriggerReceived.get(instance))) {
            Log.e(LOG_TAG," publish not trigerred because waiting for publish trigger to come");
            return;
        }
