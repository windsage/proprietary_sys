...
import com.qualcomm.listen.ListenTypes.SMLMatchResult;
...
public class ExtendedSmMgr {
    ...
    public ArrayList<IExtendedSmModel> getTrainableSoundModelList() {
        ...
        for (IExtendedSmModel item : mSoundModelList) {
            if (ISmModel.SUFFIX_DEFAULT_SOUND_MODEL.equalsIgnoreCase(item.getSoundModelSuffix())
                    && item.getSoundModelKeyphraseList().size() == 1) {
    ...
    public ArrayList<IExtendedSmModel> getCanMergeSoundModelList() {
        ...
        for (IExtendedSmModel item : mSoundModelList) {
            if (item.getSoundModelVersion() == ModelVersion.VERSION_2_0
                    && !item.isUdkSm()) {
    ...
    public void addSoundModel(Context context, String smFullName) {
        ...
        if (soundModel.isSupportExtendedOpaqueData()) {
            LogUtils.d(TAG, "addSoundModel: set opaque data transfer enabled.");
            ...
            for (ListenTypes.KeywordInfo keywordItem : modelInfo.keywordInfo) {
                ...
                */
                if (IExtendedSmModel.VERSION_0300 == modelInfo.version) {
                    ...
                    for (String userItem : keywordItem.activeUsers) {
                        if (userNameItem != null && userNameItem.equals(userItem)) {
                            indicesArray.add(userNameItem + "|" + keywordItem.keywordPhrase);
    ...
    private IExtendedSmModel.ModelVersion convertVersion(int version) {
        IExtendedSmModel.ModelVersion convertedVersion;
        switch (version) {
            case IExtendedSmModel.VERSION_0100:
                convertedVersion = ModelVersion.VERSION_1_0;
                break;
            case IExtendedSmModel.VERSION_0200:
                convertedVersion = ModelVersion.VERSION_2_0;
                break;
            case IExtendedSmModel.VERSION_0300:
                convertedVersion = ModelVersion.VERSION_3_0;
                break;
            case IExtendedSmModel.VERSION_0400:
                convertedVersion = ModelVersion.VERSION_4_0;
                break;
            case IExtendedSmModel.VERSION_0500:
                convertedVersion = ModelVersion.VERSION_5_0;
                break;
            case IExtendedSmModel.VERSION_0600:
                convertedVersion = ModelVersion.VERSION_6_0;
                break;
            case IExtendedSmModel.VERSION_0700:
                convertedVersion = ModelVersion.VERSION_7_0;
                break;
            case IExtendedSmModel.VERSION_0800:
                convertedVersion = ModelVersion.VERSION_8_0;
                break;
            case IExtendedSmModel.VERSION_0900:
                convertedVersion = ModelVersion.VERSION_9_0;
                break;
            default:
                convertedVersion = ModelVersion.VERSION_UNKNOWN;
                break;
        }
    ...
    private ListenTypes.SVASoundModelInfo query(String smFullName) {
        ...
        if (FileUtils.isExist(filePath)) {
            try {
                ...
                if (Utils.isSMLVersionMoreThan9() && !checkModelCompatibility(smBuffer)) {
                    return null;
                }
    ...
    public ByteBuffer getLanguageModel(Context context, ISmModel.ModelVersion version) {
        LogUtils.d(TAG, "getLanguageModel: enter");
        String filePath = null;
        ISettingsModel mSettingModel = new SettingsModel(context, null);
        if (version == ModelVersion.VERSION_5_0 || version == ModelVersion.VERSION_6_0) {
            LogUtils.e(TAG, "getLanguageModel: no corresponding language model");
        } else if (version == ModelVersion.VERSION_4_0) {
            filePath = Global.PATH_ROOT + "/" + mSettingModel.getUDK4BaseSoundModel();
            LogUtils.d(TAG, "getLanguageModel: "+mSettingModel.getUDK4BaseSoundModel());
        } else if (version == ModelVersion.VERSION_7_0) {
            filePath = Global.PATH_ROOT + "/" + mSettingModel.getUDK7BaseSoundModel();
            LogUtils.d(TAG, "getLanguageModel:sm7 "+mSettingModel.getUDK7BaseSoundModel());
        } else {
    ...
    public ByteBuffer convertModelToBuffer(String modelFileName) {
        String filePath = Global.PATH_ROOT + "/" + modelFileName;
        if (FileUtils.isExist(filePath)) {
            ByteBuffer byteBuffer = null;
            try {
                byteBuffer = FileUtils.readFileToByteBuffer(filePath);
                return byteBuffer;
            } catch (IOException e) {
                e.printStackTrace();
                return null;
            ...
            LogUtils.d(TAG, "convertModelToBuffer: " + filePath + " not exists");
            return null;
        }
    }

    private boolean checkModelCompatibility(ByteBuffer soundModel) {
        SMLMatchResult matchResult = new SMLMatchResult();
        int result = ListenSoundModel.checkModelCompatibility(soundModel, matchResult);
        if (result == ListenTypes.STATUS_SUCCESS) {
            LogUtils.d(TAG, "checkModelCompatibility success");
            return true;
        }
        LogUtils.d(TAG, "checkModelCompatibility failed, result = " + result);
        if (matchResult != null) {
            LogUtils.d(TAG, "checkModelCompatibility: matchResult: fPdkMatchResult = "
                    + matchResult.fSvaMatchResult + ", fSvaMatchResult = "
                    + matchResult.fSvaInternalMatchResult + ", fSvaInternalMatchResult = "
                    + matchResult.sPdkMatchResult + ", sUvMatchResult = "
                    + matchResult.sUdkMatchResult + ", sUdkMatchResult = "
                    + matchResult.sRnnMatchResult + ", sRnnMatchResult = "
                    + matchResult.sUvMatchResult + ", sUvMatchResult = "
                    + matchResult.sUvEaiMatchResult);
        }
        return false;
    }
}
