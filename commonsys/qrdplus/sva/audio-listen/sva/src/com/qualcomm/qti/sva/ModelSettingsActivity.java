...
public class ModelSettingsActivity extends Activity {
    ...
    private static final int FIRST_STAGE_MIN = 0;
    private static final int FIRST_STAGE_MAX = 100;
    private static final int SECOND_STAGE_PDK4_MIN = 0;
    private static final int SECOND_STAGE_PDK4_MAX = 99;
    private static final int SECOND_STAGE_MIN = -32768;
    private static final int SECOND_STAGE_MAX = 32767;
    ...
    private ISmModel.ModelVersion mSmVersion;
    private AlertDialog mDialogAddUser;
    private Toast mToast;
    //sm info layout variables
    private Map<String, View> mKeyphraseUserViewMap = new HashMap<>();
    private RelativeLayout mLayoutMultiKWThreshold;
    private Switch mSwitchMultiKWThreshold;
    private LinearLayout mLayoutGroupKW;
    private EditText[] mEtThresholdKWs = new EditText[5];
    private  int[] mEtThresholdKWsId = {R.id.et_multi_kw_0, R.id.et_multi_kw_1,
            R.id.et_multi_kw_2, R.id.et_multi_kw_3,R.id.et_multi_kw_4};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle(title + mSmVersion.getLable());
    ...
    private void showToast(String message, int min, int max) {
        String content = message + " threshold limits [" + min + ", " + max + "]";
        if (mToast == null) {
    ...
    private void initializeUI() {
        ...
        mEt1stKeyphrase.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                try {
                    ...
                    if (value > FIRST_STAGE_MAX || value < FIRST_STAGE_MIN) {
                        showToast("1st stage", FIRST_STAGE_MIN, FIRST_STAGE_MAX);
                        return;
                    }
                    mSettingModel.set1stKeyphraseConfidenceLevel(value);
        ...
        mEt2ndKeyphrase.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                try {
                    int value = Integer.valueOf(s.toString());
                    if (ISmModel.ModelVersion.VERSION_4_0 == mSmVersion) {
                        if (value > SECOND_STAGE_PDK4_MAX || value < SECOND_STAGE_PDK4_MIN) {
                            showToast("v4 model 2st stage", SECOND_STAGE_PDK4_MIN,
                                    SECOND_STAGE_PDK4_MAX);
                            return;
                        }
                    } else if (mSmVersion.ordinal() > ISmModel.ModelVersion.VERSION_4_0.ordinal()) {
                        if (value > SECOND_STAGE_MAX || value < SECOND_STAGE_MIN) {
                            showToast("2st stage", SECOND_STAGE_MIN,
                                    SECOND_STAGE_MAX);
                            return;
                        }
                    }
                    mSettingModel.set2ndKeyphraseConfidenceLevel(value);
                } catch (NumberFormatException e) {
                    e.printStackTrace();
                }
            }
        ...
        if (ISmModel.ModelVersion.VERSION_2_0 == mSmVersion) {
        ...
        mEt1stUser.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                try {
                    ...
                    mSettingModel.set1stUserConfidenceLevel(value);
                } catch (NumberFormatException e) {
                    e.printStackTrace();
                }
            }
        ...
        if (!mIsTrainedModel || mSmVersion.ordinal() > ISmModel.ModelVersion.VERSION_4_0.ordinal())
        {
        ...
        mEt2ndUser.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                try {
                    ...
                    if (ISmModel.ModelVersion.VERSION_4_0 == mSmVersion) {
                        if (value > FIRST_STAGE_MAX || value < FIRST_STAGE_MIN) {
                            showToast("v4 model 2st stage", FIRST_STAGE_MIN,
                                    FIRST_STAGE_MAX);
                            return;
                        }
                    } else if (mSmVersion.ordinal() > ISmModel.ModelVersion.VERSION_4_0.ordinal()) {
                    ...
                    mSettingModel.set2ndUserConfidenceLevel(value);
                } catch (NumberFormatException e) {
                    e.printStackTrace();
                }
            }
        ...
        //for multi kw threshold settings
        mSwitchMultiKWThreshold =  findViewById(R.id.sw_set_multi_first_stage_threshold);
        mSwitchMultiKWThreshold.setChecked(mSettingModel.getMultiKWThresholdEnabled());
        mLayoutMultiKWThreshold = findViewById(R.id.layout_multi_kw_threshold);
        mLayoutGroupKW = findViewById(R.id.layout_group_multi_kw_threshold);
        if(mSwitchMultiKWThreshold.isChecked()){
            mLayoutGroupKW.setVisibility(View.VISIBLE);
        }else{
            mLayoutGroupKW.setVisibility(View.GONE);
        }
        mLayoutMultiKWThreshold.setTag(mSwitchMultiKWThreshold);
        mLayoutMultiKWThreshold.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Switch switchMultiKWThreshold = (Switch) view.getTag();
                boolean bChecked = !switchMultiKWThreshold.isChecked();
                switchMultiKWThreshold.setChecked(bChecked);
                mSettingModel.setMultiKWThresholdEnabled(bChecked);
                if(bChecked){
                    mLayoutGroupKW.setVisibility(View.VISIBLE);
                }else{
                    mLayoutGroupKW.setVisibility(View.GONE);
                }
                LogUtils.d(TAG, "onClick: multi kw threshold  bChecked = " + bChecked);
            }
        });
        String[] conf = mSettingModel.getMultiFirstKWConfidenceLevel().split(SPLIT_STRING);
        for(int index = 0; index < mEtThresholdKWsId.length; index++){
            mEtThresholdKWs[index] =  findViewById(mEtThresholdKWsId[index]);
            mEtThresholdKWs[index].addTextChangedListener(
                    new MutiKWThresholdTextWatcher(mEtThresholdKWs[index], index));
            mEtThresholdKWs[index].setText(conf[index]);
        }
    ...
    private void initSmInfoLayout() {
        ...
        for (IKeyphraseModel keyphraseModel : mExtendedSmModel.getSoundModelKeyphraseList()) {
            ...
            for (IUserModel userModel : keyphraseModel.getUserList()) {
                ...
                View deleteUserView = rowUser.findViewById(R.id.iv_delete_user);
                ...
                if (mSmVersion.getVersionNumber()
                            < ISmModel.ModelVersion.VERSION_3_0.getVersionNumber()) {
                    ...
                    layoutUser.setEnabled(false);
                    layoutUser.setClickable(false);
            ...
            // + add user row
            if (!mIsUdkSm && mSmVersion.getVersionNumber()
                                     < ISmModel.ModelVersion.VERSION_3_0.getVersionNumber()) {
    ...
    private class MutiKWThresholdTextWatcher implements TextWatcher {
        View mView;
        int mIndex;
        MutiKWThresholdTextWatcher(View view, int index){
            mView = view;
            mIndex = index;
        }

        private void saveMultiKWConfidenceLevel(String confidencelevels) {
            String[] conf = mSettingModel.getMultiFirstKWConfidenceLevel().split(SPLIT_STRING);
            StringBuilder sb = new StringBuilder();
            for(int i = 0; i < conf.length; i++){
                if(i == mIndex){
                    sb.append(confidencelevels);
                }else{
                    sb.append(conf[i]);
                }
                if(i != conf.length -1){
            ...
            LogUtils.d(TAG, "saveMultiKWConfidenceLevel:" + sb.toString());
            mSettingModel.setMultiFirstKWConfidenceLevel(sb.toString());
        }
        ...
        public void afterTextChanged(Editable s) {
            try {
                ...
                saveMultiKWConfidenceLevel(s.toString());
            } catch (NumberFormatException e) {
                e.printStackTrace();
            }
        }
