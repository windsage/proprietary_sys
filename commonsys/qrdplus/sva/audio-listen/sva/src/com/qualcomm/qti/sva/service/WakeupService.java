...
public class WakeupService extends Service implements IWakeupListener {
    ...
    private InitSoundModelsTask mInitSoundModelsTask;
    ...
    private void processRecognitionEvent(String smName,
        ...
        if ((SoundTrigger.RECOGNITION_STATUS_SUCCESS == event.status
                || SoundTrigger.RECOGNITION_STATUS_FAILURE == event.status)
            ...
            String success =
                    String.valueOf(SoundTrigger.RECOGNITION_STATUS_SUCCESS == event.status);
            LogUtils.d(TAG, "processRecognitionEvent: filePath = " + filePath);
            String[] params = new String[]{filePath, captureHandle, smHandle, smName, success};
            startRecordingActivity();
            mLABTask = new LookAheadBufferTask();
            mLABTask.execute(params);
            // conflict when low latency bargein is enabled
            // delay tone player to ensure lab recording works
            if (bDetectionTone) {
                LogUtils.d(TAG, "processRecognitionEvent: play tone delayed!");
        ...
        if (Global.getInstance().getIsToggleXmlParam()) {
            ...
            //only handle UI change when success status
            if (SoundTrigger.RECOGNITION_STATUS_SUCCESS == event.status) {
                turnOnDisplay();
                // dismissKeyguard();
            }
        ...
        if (SoundTrigger.RECOGNITION_STATUS_SUCCESS != event.status) {
            LogUtils.d(TAG, "processRecognitionEvent: not success status, return directly");
            return;
        }
        ...
        if (!extendedSmModel.getSoundModelActionName().equals(getString(R.string.none))) {
            boolean isDebugMode = Global.isSvaDebugModeEnabled(getBaseContext());
            if (!isDebugMode) {
        ...
        DetectionConfLevel confLevel = null;
        if (settingsModel.getOpaqueDataTransferEnabled()
                && extendedSmModel.isSupportExtendedOpaqueData()) {
            int[] fconfidencelevels =null;
            boolean bMultiKWThresholdEnabled = settingsModel.getMultiKWThresholdEnabled();
            if(bMultiKWThresholdEnabled){
                String[] conf = settingsModel.getMultiFirstKWConfidenceLevel().split("#");
                fconfidencelevels = new int[conf.length];
                for(int i = 0; i < conf.length; i++){
                    fconfidencelevels[i] = Integer.valueOf(conf[i]);
                }
                confLevel = OpaqueDataHelper.parse(event.data, fconfidencelevels);
            }else{
                confLevel = OpaqueDataHelper.parse(event.data, null);
            }

            LogUtils.d(TAG, "after opaquedata parse = " + confLevel.toString());
        }
        ...
        DetectionEventContainer eventContainer = new DetectionEventContainer(
                ...
                vwuDetectionData, extendedSmModel.getSessionId(), smName, confLevel);
    ...
    public void onRecognition(SoundTrigger.RecognitionEvent recognitionEvent) {
        ...
        // Ignore the abort event because capture preempted by another audio use case.
        if (SoundTrigger.RECOGNITION_STATUS_ABORT == recognitionEvent.status) {
            LogUtils.d(TAG, "onRecognition: abort status, discard recognition");
            needRestart = true;
            return;
        }
    ...
    public void onResourcesAvailable() {
        LogUtils.d(TAG, "onResourcesAvailable needRestart = "+ needRestart);
    ...
    public void onDestroy() {
        ...
        if (null != mInitSoundModelsTask && !mInitSoundModelsTask.isCancelled()) {
            mInitSoundModelsTask.cancel(true);
            mInitSoundModelsTask = null;
        }
    ...
    private void asyncInitSoundModels() {
        mInitSoundModelsTask = new InitSoundModelsTask();
        mInitSoundModelsTask.execute();
    }

    private void startRecordingActivity() {
        Intent intent = new Intent(WakeupService.this, RecordingActivity.class);
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        intent.addFlags(Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS);
        startActivity(intent);
    }

    private class InitSoundModelsTask extends AsyncTask<Void, Void, Void> {
        @Override
        ...
        protected void onPostExecute(Void unused) {
            ...
            LogUtils.d(TAG, "InitSoundModelsTask:onPostExecute");
            sendResponseToAllClients(Msg.MSG_QUERY_SOUND_MODEL_DONE,
    ...
    public class LookAheadBufferTask extends AsyncTask<String, Void, Integer> {
        ...
        private final int VOICE_REQUEST_CANCELED = -100;
        ...
        protected Integer doInBackground(String... strings) {
            ...
            mSmName = strings[3];
            boolean detectionSuccess = Boolean.parseBoolean(strings[4]);
            // create directory first
            FileUtils.createDirIfNotExists(Global.PATH_ROOT);
            FileUtils.createDirIfNotExists(Global.PATH_VOICE_REQUESTS);
            //for now, st-hal is using AIDL on new platform, there will be no onResourcesAvailable
            // called when lab end, so need APP restart recognition by itself when lab end.
            if ((!mWakeupSession.isSupportConcurrency()) && (!Utils.isSMLVersionMoreThan9())) {
                needRestart = true;
            }
            int bSuccess = Global.getInstance().getRecordingsMgr().startHotWordRecording(
                    getApplicationContext(), mSmName, captureHandle, mFilePath, detectionSuccess);
            LogUtils.d(TAG, "LookAheadBufferTask:doInBackground: bSuccess  = " + bSuccess);
            if (bSuccess == VOICE_REQUEST_CANCELED) {
                IExtendedSmModel extendedSmModel
                        = Global.getInstance().getExtendedSmMgr().getSoundModel(mSmName);
                sendResponseToAllClients(Msg.MSG_ENABLED_ON_OFF, extendedSmModel);
            }
            return bSuccess >= 0 ? SUCCESS : FAILURE;
        }
        ...
        protected void onPostExecute(Integer integer) {
            ...
            if (-1 != mAssociateSmHandle &&
                    (mWakeupSession.isSupportConcurrency() || Utils.isSMLVersionMoreThan9())) {
    ...
    public class RestartRecognitionTask extends AsyncTask<String, Void, Integer> {
        ...
        protected void onPostExecute(Integer integer) {
            super.onPostExecute(integer);
            LogUtils.d(TAG, "RestartRecognitionTask:onPostExecute: integer = " + integer);
            if(0 != integer.intValue() && !mWakeupSession.isSupportConcurrency()){
                LogUtils.d(TAG, "RestartRecognitionTask:onPostExecute: needRestart " +
                        "changed from  = " + needRestart + " to true");
                needRestart = true;
            } else if (-16 == integer.intValue()) {
                // add for concurrent voice call param
                LogUtils.d(TAG, "RestartRecognitionTask:onPostExecute: needRestart " +
                        "changed from  = " + needRestart + " to true due to EBUSY");
                needRestart = true;
            }
        }
