...
Return<void> SystemResource::acquireSurface(uint32_t width, uint32_t height,
  ...
  int result = -1;
  int maxRetries = 200, maxRetriesSurface = 10;
  struct surfaceData *sData = nullptr;
  SurfaceComposerClient::Transaction t;
  uint32_t resourceId = getResourceId();
  uint32_t transformValue = -1;
  ...
  sData->mControl = (sData->mSurfaceComposer)
                        ->createSurface(android::String8("systemHelperService"),
                                        ...
                                        0, nullptr, LayerMetadata(), &transformValue);
  if (!sData->mControl) {
  ...
  if (!isOrientationPortrait() || transformValue != 0) {
    ...
    do {
      ...
      maxRetriesSurface--;
    } while (transformValue != 0 && maxRetriesSurface > 0);
