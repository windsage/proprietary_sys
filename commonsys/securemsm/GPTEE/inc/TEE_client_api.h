/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
        ...
        HLOS Global Platform TEE Client Library Functions Header
 ...
 *      When an operation completes, and if the Memory Reference is tagged as
 *      "output", this is updated by trusted application to reflect the actual
...
/**
 * @brief: Set up Environment for TEEC Communications
 * Before calling this function its neccesary to use MinkSocketFd to connect to
 * trustzoneaccess service, after which TEE update a client Object in mNativePtr
 * of MinkSocketFd, once the connection is established the mNativePtr must be
 * passed as parameter to this api using the internal getNativePtr() method.
 * @param[in] mClientEnv: mNative pointer stored in the java layer of MinkSocketFd
 * which is updated when trustzone access service is connected to TZ, this object
 * must be passed to GP Library before calling an GP standard API.
 * @return: TEEC_SUCCESS: In case the Object is valid Success is returned.
 */
TEEC_Result TEEC_SetEnvironment(void * mClientEnv);
/**
