...
static const int32_t SVA_KEYWORD_TYPE_PDK = 2;
static const int32_t SVA_KEYWORD_TYPE_UDK = 3;
...
typedef enum{
  ...
  LISTEN_ERECORDING_TOO_SHORT = -27, // Recording is too short
  LISTEN_ERECORDING_TOO_LONG = -28, // Recording is too long
  LISTEN_ECHOPPED_SAMPLE = -29, // Recording has chopped speech/keyword
  LISTEN_ECLIPPED_SAMPLE = -30, // Recording has clipped speech/keyword
  LISTEN_EFUNCTION_NOT_IMPLEMENTED = -31, // function not implemented
...
/*
 * This structure is added to compare the payload size passed by
 * APK for UDK training. With SML version 6 and before for UDK4
 * training, below params structure is expected.
*/
typedef struct sml_udk_params_v6 {
...
{ \
    ...
    } else if (lsmLibErr == kRecordingTooShort) { \
           ALOGE("%s: ERROR Recording is too short", __func__); \
           clientErr = LISTEN_ERECORDING_TOO_SHORT;  \
    } else if (lsmLibErr == kRecordingTooLong) { \
           ALOGE("%s: ERROR Recording is too long", __func__); \
           clientErr = LISTEN_ERECORDING_TOO_LONG;  \
    } else if (lsmLibErr == kChoppedSample) { \
           ALOGE("%s: ERROR Recording has chopped speech/keyword", __func__); \
           clientErr = LISTEN_ECHOPPED_SAMPLE;  \
    } else if (lsmLibErr == kClippedSample) { \
           ALOGE("%s: ERROR Recording has clipped speech/keyword", __func__); \
           clientErr = LISTEN_ECLIPPED_SAMPLE;  \
    } else { \
           clientErr = LISTEN_EFAILURE; \
    } \
}
