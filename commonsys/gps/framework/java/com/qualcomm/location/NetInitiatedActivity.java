...
public class NetInitiatedActivity extends AlertActivity implements DialogInterface.OnClickListener,
        IzatService.ISystemEventListener {
    ...
    private static final int GPS_NO_RESPONSE_TIME_OUT =
            IZatServiceContext.MSG_NET_INITIATED_BASE + 1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mHandler = new Handler(mIZatServiceCtx.getLooper()) {
            @Override
        ...
        timeout = intent.getIntExtra(GpsNetInitiatedHandler.NI_INTENT_KEY_TIMEOUT,
                default_response_timeout);
        default_response = intent.getIntExtra(GpsNetInitiatedHandler.NI_INTENT_KEY_DEFAULT_RESPONSE,
                GpsNetInitiatedHandler.GPS_NI_RESPONSE_ACCEPT);
        if (DEBUG) Log.d(TAG, "onCreate() : notificationId: " + notificationId + " timeout: " +
                timeout + " default_response:" + default_response);

        mHandler.sendMessageDelayed(mHandler.obtainMessage(GPS_NO_RESPONSE_TIME_OUT),
                (timeout * 1000));
        setupAlert();
    }
    ...
    protected void onResume() {
        ...
        mIZatServiceCtx.registerSystemEventListener(MSG_NET_INITIATED, this);
    }
