/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  Copyright (c) 2015 - 2016, 2020-2021, 2024 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
public class GeofenceServiceProvider implements IzatService.ISystemEventListener {
    ...
    private GeofenceServiceProvider(Context ctx) {
        ...
        if (LocIDLClientBase.getIDLServiceVersion().compareTo(IDLServiceVersion.V_AIDL) >= 0) {
            mGeofenceServiceIdlClient = new GeofenceServiceIdlClient(this);
    ...
    private final IGeofenceService.Stub mBinder = new IGeofenceService.Stub() {
        ...
        public int addGeofence(double latitude,
            ...
            mGeofenceServiceIdlClient.addGeofence(geofenceId, latitude, longitude, radius,
        ...
        public void removeGeofence(int geofenceId) {
            ...
            mGeofenceServiceIdlClient.removeGeofence(geofenceId);
        }
        ...
        public void updateGeofence(int geofenceId,
            ...
            mGeofenceServiceIdlClient.updateGeofence(geofenceId,
                                   transitionTypes,
        ...
        public void updateGeofenceData(int geofenceId,
            ...
            if (null != clData) {
                if (clData.mGeofenceMap.containsKey(geofenceId)) {
                    ...
                    if (gfData.isPaused()) {
                        mGeofenceServiceIdlClient.pauseGeofence(geofenceId);
                    }
        ...
        public void resumeGeofence(int geofenceId) {
            ...
            if (null != clData) {
                if (clData.mGeofenceMap.containsKey(geofenceId)) {
                    ...
                    mGeofenceServiceIdlClient.resumeGeofence(geofenceId,
    ...
    private void removeAllGeofences(ClientGeofenceData clData)
    {
        ...
        synchronized (clData.mGeofenceMap) {
            for(Integer key : clData.mGeofenceMap.keySet()) {
                mGeofenceServiceIdlClient.removeGeofence(key);
            }
            ...
            for (ClientGeofenceData geofenceData: prevUserData.values()) {
                synchronized (geofenceData.mGeofenceMap) {
                    for(GeofenceData geofence: geofenceData.mGeofenceMap.values()) {
                        ...
                        mGeofenceServiceIdlClient.removeGeofence(geofence.getGeofenceId());
                    }
                }
            }
            ...
            for (ClientGeofenceData geofenceData: currentUserData.values()) {
                synchronized (geofenceData.mGeofenceMap) {
                    for(GeofenceData geofence: geofenceData.mGeofenceMap.values()) {
                        ...
                        mGeofenceServiceIdlClient.addGeofence(geofence.getGeofenceId(),
    ...
    public void reportServiceDied() {
        ...
        for (ClientGeofenceData clData : mDataPerPackageAndUser.getAllData()) {
            for(Integer geofenceId : clData.mGeofenceMap.keySet()) {
                ...
                mGeofenceServiceIdlClient.removeGeofence(geofenceId);
                mGeofenceServiceIdlClient.addGeofence(geofenceId, gfData.getLatitude(),
                        gfData.getLongitude(), gfData.getRadius(),
                        gfData.getTransitionType().getValue(),
     ...
     *   AIDL Client
     * =================================================*/
    static class GeofenceServiceIdlClient extends LocIDLClientBase
            implements LocIDLClientBase.IServiceDeathCb {
        private final String TAG = "GeofenceAidlClient";
        ...
        private ILocAidlGeofenceService getGeofenceServiceIface() {
            if (null == mGeofenceService) {
                ...
                Log.d(TAG, "ILocAidlGnss.getService(), service = " + service);
                try {
                    if (service instanceof vendor.qti.gnss.ILocAidlGnss) {
                        mGeofenceService = service.getExtensionLocAidlGeofenceService();
                    }
                ...
                if (null == mGeofenceService) {
                    Log.e(TAG, "ILocAidlGeofenceService handle is null");
                    return null;
                }
            }
        ...
        private GeofenceServiceIdlClient(GeofenceServiceProvider provider) {
            ...
            if (null != mGeofenceService) {
                try {
                    ...
                    if (!ret) {
                        Log.e(TAG, "Failed to init LocAidlGeofenceService");
                    }
                    registerServiceDiedCb(this);
                    Log.d(TAG, "GeofenceServiceAidlClient aidl init");
                } catch (RemoteException e) {
                    Log.e(TAG, "Exception init LocAidlGeofenceService: " + e);
                }
            }
        }
        private int translateAidlLocError(int locationErr) {
        ...
        private class GeofenceServiceCallback extends ILocAidlGeofenceServiceCallback.Stub {
            @Override
            public void gnssGeofenceBreachCb(int count, int[] idList,
                    vendor.qti.gnss.LocAidlLocation location, int breachType,
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    Location locationObj = IDLClientUtils.translateAidlLocation(location);

                    mGeofenceServiceProvider.reportGeofenceTransition(idList[i],
            ...
            @Override
            public void gnssAddGeofencesCb(int count, int[] locationErrorList,
                    int[] idList) throws android.os.RemoteException {
                IDLClientUtils.fromIDLService(TAG);
                for (int i = 0; i < count; i++) {
                    int status = translateAidlLocError(locationErrorList[i]);
                    mGeofenceServiceProvider.reportGeofenceRequestStatus(
                            GEOFENCE_REQUEST_TYPE_ADD, idList[i], status);
                }
            }

            @Override
            public void gnssRemoveGeofencesCb(int count, int[] locationErrorList,
                    int[] idList) throws android.os.RemoteException {
                IDLClientUtils.fromIDLService(TAG);
                for (int i = 0; i < count; i++) {
                    int status = translateAidlLocError(locationErrorList[i]);
                    mGeofenceServiceProvider.reportGeofenceRequestStatus(
                            GEOFENCE_REQUEST_TYPE_REMOVE, idList[i], status);
                }
            }

            @Override
            public void gnssPauseGeofencesCb(int count, int[] locationErrorList,
                    int[] idList) throws android.os.RemoteException {
                IDLClientUtils.fromIDLService(TAG);
                for (int i = 0; i < count; i++) {
                    int status = translateAidlLocError(locationErrorList[i]);
                    mGeofenceServiceProvider.reportGeofenceRequestStatus(
                            GEOFENCE_REQUEST_TYPE_PAUSE, idList[i], status);
                }
            }

            @Override
            public void gnssResumeGeofencesCb(int count, int[] locationErrorList,
                    int[] idList) throws android.os.RemoteException {
                IDLClientUtils.fromIDLService(TAG);
                for (int i = 0; i < count; i++) {
                    int status = translateAidlLocError(locationErrorList[i]);
                    mGeofenceServiceProvider.reportGeofenceRequestStatus(
                            GEOFENCE_REQUEST_TYPE_RESUME, idList[i], status);
                }
            }
            @Override
            public final int getInterfaceVersion() {
                return ILocAidlGeofenceServiceCallback.VERSION;
            }
            @Override
            public final String getInterfaceHash() {
                return ILocAidlGeofenceServiceCallback.HASH;
            }
        }
        @Override
        public void onServiceDied() {
            Log.e(TAG, "ILocAidlGeofenceService died");
            mGeofenceService = null;
            ILocAidlGeofenceService geofenceIface = getGeofenceServiceIface();
            if (null != mGeofenceService) {
                try {
