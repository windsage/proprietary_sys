...
public class DebugReportActivity extends Activity {
    ...
    private XtraStatusReportCallback mXtraStatusReportCb;
    ...
    protected void onDestroy() {
        try {
            ...
            if (mDebugReportingService != null) {
                mIzatMgr.disconnectDebugReportingService(mDebugReportingService);
            } else {
                Log.e(TAG, "mDebugReportingService = null");
            }
    ...
    private void doInit() {
        try {
            ...
            Log.e(TAG, "DebugReport: doInit failed");
            return;
        } catch (Exception e) {
            Log.e(TAG, "DebugReport: doInit failed");
        }
        ...
        final Button registerForXtraStatus = (Button) findViewById(R.id.registerXtraStatusBtn);
        final Button unregisterForXtraStatus = (Button) findViewById(R.id.unregisterXtraStatusBtn);
        final Button getXtraStatus = (Button) findViewById(R.id.getXtraStatusBtn);
        ...
        registerForXtraStatus.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
            if (VERBOSE)
                Log.v(TAG, "Register for xtra Status");
                    mXtraStatusReportCb = null;
                    mXtraStatusReportCb = new XtraStatusReportCallback(DebugReportActivity.this,
                        getApplicationContext());
                    try {
                        mDebugReportingService.registerXtraStatusListener(mXtraStatusReportCb);
                    } catch(Exception e) {
                        Log.e(TAG, e.getMessage());
                    }
                }
        ...
        unregisterForXtraStatus.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (VERBOSE) {
                    Log.v(TAG, "Unregister for xtra Status");
                }
                if (mXtraStatusReportCb != null) {
                    try {
                        mDebugReportingService.unregisterXtraStatusListener();
                        mXtraStatusReportCb.saveReport();
                    } catch(Exception e) {
                        Log.e(TAG, e.getMessage());
                    }
                }
        ...
        getXtraStatus.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (VERBOSE) {
                    Log.v(TAG, "get xtra Status");
                }
                try {
                    mDebugReportingService.getXtraStatus();
                } catch(Exception e) {
                    Log.e(TAG, e.getMessage());
                }
            }
    ...
    private void logPdrReport(IZatPDRDebugReport pdrReport, PrintWriter fileWriter) {
        if (pdrReport != null) {
            ...
            s.append(" Gyro Calibration Status:" + pdrReport.getGyroCalStatus());
    ...
    private void logGpstimeReport(IZatGpsTimeDebugReport gpsTimeReport,
        ...
        if (gpsTimeReport != null) {
            ...
            s.append( " Leap Seconds:" +
                    gpsTimeReport.getLeapSeconds());
            s.append( " Leap Seconds Unc:" +
                    gpsTimeReport.getLeapSecUnc());
    ...
    private void logRfStateReport(IZatRfStateDebugReport rfStateReport, PrintWriter fileWriter) {
        if (rfStateReport != null) {
            ...
            s.append(" Jammer Signal Mask:" + rfStateReport.getJammerSignalMask() + "\n");
            List<IZatJammerSignalInfoSpec> izatJammerSignalInfoSpec =
                rfStateReport.getIzatJammerSignalInfoSpecList();
            if (izatJammerSignalInfoSpec == null) {
                s.append("<JammerInd, Agc> List : Invalid");
            } else {
                for(IZatJammerSignalInfoSpec info : izatJammerSignalInfoSpec) {
                    s.append(info.getSignalType() + " <JammerInd, Agc>:<" +
                            info.getJammerInd() + ", " + info.getAgc() + ">,");
                }
            }
            if (fileWriter != null) {
                fileWriter.println(s);
    ...
    void printXtraStatusReport(IZatXtraStatus xtraStatus, PrintWriter fileWriter) {
        synchronized(sReportLogLock) {
            if (xtraStatus == null) {
                return;
            }
            StringBuilder s = new StringBuilder();
            s.append("Xtra Enabled status: " + (xtraStatus.mEnabled ? "Enabled" : "Disabled")
                    + "\r\n ");
            s.append("Xtra Data status: " + xtraStatus.mXtraDataStatus.getValue() + "\r\n ");
            s.append("Xtra ValidityHrs: " + xtraStatus.mValidityHrs + "\r\n ");
            s.append("Xtra LastDownloadStatus: " + xtraStatus.mLastDownloadStatus + "\r\n ");

            if (fileWriter != null) {
                fileWriter.println(s);
            } else {
                if (VERBOSE) {
                    Log.v(TAG, "--- XTRA status Report ---");
                    Log.v(TAG, s.toString());
                }
            }
        }
