...
void populateFlpBatch(JNIEnv* env,  jobject obj, jobjectArray locations, void* pData) {
    ...
    jmethodID method_getFloat = env->GetMethodID(class_bundle, "getFloat", "(Ljava/lang/String;)F");
    ...
    for (int i=0; i<len; ++i) {
        ...
        jstring conformityindex_str = env->NewStringUTF("Conformity_index");
        jstring techmask_str = env->NewStringUTF("Tech_mask");
        if (extras_obj != nullptr) {
            ...
            diagLocation.conformityIndex = env->CallFloatMethod(extras_obj,
                    method_getFloat, conformityindex_str);
            diagLocation.techMask = (DiagLocationTechnologyMask)env->CallIntMethod(extras_obj,
                    method_getInt, techmask_str);
        }
        ...
        if (!isZero(diagLocation.conformityIndex)) {
            diagLocation.flags |= DIAG_LOCATION_HAS_CONFORMITY_INDEX_BIT;
        }
        if (diagLocation.elapsedRealTime != 0) {
            diagLocation.flags |= DIAG_LOCATION_HAS_ELAPSED_REAL_TIME_BIT;
        }
        if (diagLocation.techMask != 0) {
            diagLocation.flags |= DIAG_LOCATION_HAS_TECH_MASK_BIT;
        }
        ...
        LOC_LOGv("FLP batch, count: %d num: %d qualityType: %d timestamp: %llu lat: %f long: %f "
                "alt: %f speed: %f bearing: %f accu: %f conformityInd: %f techMask: %x flags: %x",
               len, i, diagLocation.qualityType, (unsigned long long)diagLocation.timestamp,
               diagLocation.latitude, diagLocation.longitude, diagLocation.altitude,
               diagLocation.speed, diagLocation.bearing, diagLocation.accuracy,
               diagLocation.conformityIndex, diagLocation.techMask, diagLocation.flags);
        env->DeleteLocalRef(qualitytype_str);
        env->DeleteLocalRef(conformityindex_str);
        env->DeleteLocalRef(techmask_str);
...
void nativeDiagLogFlpBatch(JNIEnv* env,  jobject obj, jobjectArray locations) {
    ...
    if (pData != nullptr) {
        ...
        LOC_LOGe("log_alloc failed, diagId: %x", diagId);
    }
}
