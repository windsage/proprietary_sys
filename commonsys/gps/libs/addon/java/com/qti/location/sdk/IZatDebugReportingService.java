...
public interface IZatDebugReportingService {
    ...
    /**
     * Registers to listen to xtra status.
     * <p>
     * This function enables applications to start receiving Xtra Status report
     * when xtra status changed or first registered.
     * If there is already a callback registered,
     * the new one one will replace the existing one. </p>
     *
     * @param listener Callback to receive the xtra status report.
     *         This parameter cannot be NULL, otherwise a
     *         {@link IZatIllegalArgumentException} is thrown.
     ...
     * None.
     */
    void registerXtraStatusListener(IZatXtraStatusCallback listener);
     /**
     * Deregisters to stop listening to xtra status.
     * @return
     * None.
     */
    void unregisterXtraStatusListener();
     /**
     * fetch xtra status once. should be called after registerXtraStatusListener()
     * @return
     * None.
     */
    void getXtraStatus();
...
/** @addtogroup IZatDebugReportingService
    @{ */
    interface IZatXtraStatusCallback {
    /**
     * Xtra status callback.
     * <p>
     * This API is called by the underlying service back
     * to applications when xtra status is available.
     * Applications should implement this interface.</p>
     *
     * @param xtraStatus {@link IZatXtraStatus} type.
     * The object that contains xtra status data:
     */
        void onXtraStatusChanged(IZatXtraStatus xtraStatus);
    }
    public enum IZatXtraDataStatus {
        ...
        private IZatXtraDataStatus(int value) {
            this.value = value;
        }
        public int getValue() {
            return value;
        }
        private static final Map<Integer, IZatXtraDataStatus> valueMap =
                new HashMap<Integer, IZatXtraDataStatus>();
        static {
            for (IZatXtraDataStatus type : IZatXtraDataStatus.values()) {
                valueMap.put(type.value, type);
            }
        }
        protected static IZatXtraDataStatus fromInt(int value) {
