...
interface IImsRadio {
  ...
  oneway void sendVosActionInfo(in int token, in vendor.qti.hardware.radio.ims.VosActionInfo vosActionInfo);
  oneway void setGlassesFree3dVideoCapability(in int token, in boolean enableGlassesFree3dVideo);
  oneway void abortConference(in int token, in vendor.qti.hardware.radio.ims.ConferenceAbortReasonInfo abortReason);
}
