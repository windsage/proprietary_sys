...
from techpack_setup import TECHPACKSetup


class TECHPACKCompatibility(CompatibilityCheck):
    def __init__(self):
        super(TECHPACKCompatibility, self).__init__()
        TECHPACKSetup(self.build_path).main()
    ...
    def _run_type_abi_cmd(self):
        ...
        comp_cmd = 'bash build.sh -j64 dist --target_only' \
                   ' --tech_package=techpackage-techpack'
    ...
    def _run_check_golden_dumps(self, library_name):
        ...
        commands_lst = ['cd $ANDROID_BUILD_TOP',
                        'cd vendor/qcom/proprietary/commonsys-intf/QIIFA-cmd/abi-dumps/techpack-unittest/abi-dumps/techpackages/techpackage-techpack/arm64/',
                        'ls -ltr ' + library_name]
    ...
    def _run_check_lsdumps(self, library_name):
        ...
        commands_lst = ['cd $ANDROID_BUILD_TOP',
                        'cd out/target/product/pineapple/qiifa_pineapple_current_cmd/abi-dumps/techpackages/techpackage-techpack/arm64/',
                        'ls -ltr ' + library_name]

        err = run_cmd(commands_lst)['stderr']
        return err

    def _run_delete_qiifa_fwk(self):
        """Delete out/soong/.intermediates/vendor/qcom/proprietary/commonsys-intf/QIIFA-fwk/
        """

        commands_lst = ['cd $ANDROID_BUILD_TOP',
                        'rm -rf out/soong/.intermediates/vendor/qcom/proprietary/commonsys-intf/QIIFA-fwk/']
