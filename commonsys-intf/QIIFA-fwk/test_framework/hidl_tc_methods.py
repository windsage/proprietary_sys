...
class HIDLTCMethods(object):
    ...
    def get_version(self, ver):
        ...
        if '-' in ver:
            ...
            start_num = int((ver.split('-')[0])[-1])
            end_num = int(ver.split('-')[1])
            for x in range(start_num, end_num + 1):
                version.append(str(major + (x * 0.1)))
        else:
            version.append(ver)
        return version
    ...
    def check_hal_files_are_present(self, tc_kwargs, xml=None):
        ...
        android_build_comp_files = str(os.getenv('ANDROID_BUILD_TOP')) + '/out/target/product/qssi_64/system/etc/vintf/'
        android_build_top = str(os.getenv('ANDROID_BUILD_TOP')) + '/hardware/interfaces/'

        vintf_files = os.listdir(android_build_comp_files)
        comp_files = []
        files = []

        for vf in vintf_files:
            if re.match(r"compatibility_matrix.\d*.xml|compatibility_matrix.current.xml", vf):
                comp_files.append(vf)

        for cf in comp_files:
            # Read xml from abs_file_path if not passes as argument
            xml = self.read_xml_from_file(android_build_comp_files + cf, xml)

            # Read and edit contents of file
            for ind, item in enumerate(xml['compatibility-matrix']['hal']):
                # Gather required information from xml files
                if item['@format'] == 'hidl':
                    hal_name = item['name']
                    folder = ('/'.join(str(hal_name).split('.')[2:]))
                    version = []
                    interfaces = []
                    ver = item['version']
                    if type(ver) is list:
                        for val in ver:
                            [version.append(x) for x in self.get_version(val.encode('ascii','ignore'))]
                    else:
                        [version.append(x) for x in self.get_version(ver.encode('ascii','ignore'))]
                    if type(item['interface']) is list:
                        for j, intf in enumerate(item['interface']):
                            interfaces.append(intf['name'].encode('ascii','ignore'))
                    else:
                        interfaces.append(item['interface']['name'].encode('ascii','ignore'))

                    # Checking if the hal file is present
                    for intf in interfaces:
                        for ver in version:
                            search_hal = folder + '/' + ver + '/' + intf + '.hal'
                            print(android_build_top + search_hal)
