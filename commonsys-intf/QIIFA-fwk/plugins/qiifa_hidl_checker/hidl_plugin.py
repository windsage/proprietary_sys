...
def buildfcmpath(file_path):
    ...
    for xml_file in files:
        ...
        if xml_file.startswith("compatibility_matrix."):
            ...
            if split_name[1].isdigit():
               if int(split_name[1]) >0 and int(split_name[1]) >= Constants.HIDL_MIN_SUPPORT_VERSION:
    ...
    removelessthanfourandver(FCM_GLOBAL_DICT,max_val)
...
def removelessthanfourandver(FCM_GLOBAL_DICT,max_val):
    if "current" in FCM_GLOBAL_DICT.keys():
        allowedminver = FCM_GLOBAL_DICT["current"] - Constants.HIDL_NO_OF_PREV_VERS_SUPP
    else:
        allowedminver = Constants.VINTF_PLATVER_MAP[max_val] - Constants.HIDL_NO_OF_PREV_VERS_SUPP
...
def parse_xml_files_for_check(txt_file_data, xml_files_path, target):
    #If running in graphics, Check if files are available, Don't run qiifa
    if (Constants.graphics_current_techpack) :
        Logger.logStdout.info("Skipping QIIFA for HIDL GRAPHICS Tech Pack Build ONLY")
    else:
        if (UtilFunctions.dirExists(xml_files_path)) :
            if target == "qssi":
                buildfcmpath(xml_files_path)
                handle_qssi_parse_and_check(txt_file_data, xml_files_path)
            else:
                parse_partial_xml_files(xml_files_path)
        else:
            Logger.logStdout.warning("QIIFA Check: paths_of_xml_files directory was not found for %s",target)
...
def handle_qssi_parse_and_check(txt_file_data, xml_files_path):
    ...
    for xml_file in files:
        if xml_file.startswith("compatibility_matrix."):
            ...
            level_val = None if split_name[1] not in FCM_GLOBAL_DICT.keys() else split_name[1]
            if level_val != None:
                file_path=os.path.join(root,xml_file)
                if level_val == "current" and not UtilFunctions.pathExists(file_path):
                    Logger.logStdout.info("The Compatibility matrix ( "+xml_file+" ) file is found")
                else:
            ...
            if x_file.startswith("compatibility_matrix."):
                ...
                if level_val != None:
                    ...
                    try:
                        ...
                        UtilFunctions.print_violations_on_stdout("HIDL Plugin",fd,full_create_parser.__name__,reason)
                        sys.exit(1)

                    #generate cmd file name and concatinate with db location
                    jsn_file_name = Constants.qiifa_cmd_prefix + '_' + str(FCM_GLOBAL_DICT[level_val]) + Constants.json_file_suffix
        ...
        if x_file.startswith("compatibility_matrix."):
            ...
            if level_val != None:
                ...
                try:
                    ...
                    reason = "Please verify the XML File, invalid syntax has been used."
                    Logger.logStdout.error(e)
                    UtilFunctions.print_violations_on_stdout("HIDL Plugin",fd,single_create_parser.__name__,reason)
                    sys.exit(1)
...
def handle_qssi_check(fcm_data, cmd_data, level_val):
    local_skipped_intf_list = get_skipped_intf_list(Constants.hidl_skipped_intf)
    for fcm_entry in fcm_data['compatibility-matrix']['hal']:
        ...
        try:
            if fcm_entry['@format'] == "aidl":
                Logger.logInternal.info("AIDL interface found:"+fcm_entry['name'])
                continue
            elif fcm_entry['@format'] == "native":
               Logger.logInternal.info("Native interface found:"+fcm_entry['name'])
               continue
        except:
            UtilFunctions.print_violators(fcm_entry['name'],"The format tag is not present in the Compatibility matrix or current.txt or QIIFA CMD. Please update before proceeding. Check go/qiifa for more information","","")
...
def block_by_block_check(fcm_hal, cmd_hal, level_val):
    ...
    try:
        if fcm_hal['@format'] != cmd_hal['@format']:
            ...
            #already reported
            return
        elif fcm_hal['@optional'] != cmd_hal['@optional']:
            Logger.logStdout.warning("The optional tag is mismatched in cmd and FCM for: " + cmd_hal['name'] + " Please update it.")
            return
        else:
            if not is_c_variant:
...
def version_block_check(fcm_version, cmd_version, intf_name, level_val):
    ...
    if res.err_code != Constants.HIDL_VERSION_NO_ERR:
        ...
        if level_val == "device":
            ...
            extra_fcm_hal = []
            extra_cmd_hal = []
            for sfv in FCM_GLOBAL_DICT.keys():
            ...
            if not x_file in Constants.legacy_xml_files:
                ...
                with open(file_path) as fd:
                    try:
                        ...
                        reason = "Please verify the XML File, invalid syntax has been used."
                        Logger.logStdout.error(e)
                        UtilFunctions.print_violations_on_stdout("HIDL Plugin",fd,common_parser.__name__,reason)
                        sys.exit(1)
...
def get_skipped_intf_list(skip_path):
    skipped_intf_list = []
    if(UtilFunctions.fileExists(skip_path)):
        skipped_intf_fh = open(skip_path, "r")
        skipped_intf_list = []
        for lib_path in skipped_intf_fh:
            skipped_intf_list.append(lib_path.rstrip())
        skipped_intf_fh.close()
    else:
        Logger.logStdout.error("Hidl skip interface file does not exist:" + skip_path)
    ...
    for hal in xml_file_data['compatibility-matrix']['hal']:
        try:
            if (hal['@format']) =='hidl':
                ...
                Logger.logInternal.info("AIDL interface found: " + hal['name'])
            elif (hal['@format']) == "native":
               Logger.logInternal.info("Native interface found:"+hal['name'])
            else:
                UtilFunctions.print_violators(hal['name'],"No versions found in FCM entry. Check go/qiifa for more information","","")
                return
        except:
            if "@format" not in hal.keys():
                UtilFunctions.print_violators(hal['name'],"The format tag is not present in the Compatibility matrix or current.txt or QIIFA CMD. Please update before proceeding. Check go/qiifa for more information","","")
                return
            else:
                UtilFunctions.print_violators(hal['name'],"Some tag is missing, Please verify before proceeding. Check go/qiifa for more information","","")
                return
...
def append_single_hash(hal, txt_file_data, is_for_single, is_for_create=False):
    ...
    for hash_info in txt_file_data:
        if hash_info['name'] == hal['name']:
            ...
            elif hal['@format'] != "aidl" or hal['@format'] != "native":
                #TODO: highlight this in meetings
                Logger.logStdout.error("No versions found in FCM entry for: " + hal['name'])
                sys.exit()
            else:
                Logger.logInternal.info("AIDL interface found: " + hal['name'])
...
def get_vndr_cmd_ver(path):
    for root, dr, files in os.walk(path):
        for file in files:
            Logger.logStdout.info("Scanning: " + file)
            if file.startswith("qiifa_hidl_cmd_") and file.endswith(".json"):
                if "2.0" in file:
                    Logger.logStdout.info("Vendor CMD version is 2.0")
                    return "2.0"
                else:
                    Logger.logStdout.info("Vendor CMD version is 1.0 (older vendor)")
                    continue
    return ""

def get_target_level(vndr_cmd):
    if len(vndr_cmd) > 0 and u'manifest' in vndr_cmd.keys():
        if u'@target-level' in vndr_cmd[u'manifest']:
            return vndr_cmd[u'manifest'][u'@target-level']
...
def load_qssi_cmds(path, target_level):
    ...
    try:
        ...
        for root, dirs, files in os.walk(path):
            for file in files:
                if file.startswith("qiifa_hidl_cmd_"):
                    filename = file.split('.')[0]
                    suffixcmdnm = filename.split('_')[3]
                    if suffixcmdnm.isdigit():
                        if int(suffixcmdnm) > 0 and int(suffixcmdnm) >= Constants.HIDL_MIN_SUPPORT_VERSION:
                            FCM_GLOBAL_DICT[suffixcmdnm]= int(suffixcmdnm)
                        if int(suffixcmdnm) > fcm_max_val:
                            fcm_max_val = int(suffixcmdnm)
                    elif "current" in suffixcmdnm:
                        current_exist = True
        if not current_exist:
            if "current" in FCM_GLOBAL_DICT.keys():
                del FCM_GLOBAL_DICT["current"]
        removelessthanfourandver(FCM_GLOBAL_DICT,fcm_max_val)
        for l_val in FCM_GLOBAL_DICT:
        ...
        traceback.print_exc()
        reason = "There is an issue with the Super Image generation. Please check the QIIFA Artifacts. Please contact QIIFA-squad for more info"
        Logger.logStdout.error(e)
        UtilFunctions.print_violations_on_stdout("HIDL Plugin","QIIFA HIDL Super Image",load_qssi_cmds.__name__,reason)    
...
def img_to_img_check(qssi_cmd, vndr_cmd, vndr_cmd_ver="", qssi_cmd_path=""):
    local_hidl_skipped_intf_path = UtilFunctions.validValue("", qssi_cmd_path, "/qiifa_hidl_skipped_interfaces.txt")
    Logger.logStdout.info(local_hidl_skipped_intf_path)

    local_skipped_intf_list = get_skipped_intf_list(local_hidl_skipped_intf_path)
    if not u'manifest' in vndr_cmd.keys():
        return
    for hal in vndr_cmd['manifest']['hal']:
        ...
        if local_skipped_intf_list and hal['name'] in local_skipped_intf_list:
            Logger.logInternal.info("Skipping HIDL interface for:" +hal['name'])
            continue
        ...
        if '@format' in hal and hal['@format'] == "aidl":
            Logger.logInternal.info("AIDL interface found: " + hal['name'])
            continue
        elif '@format' in hal and hal['@format'] == "native":
            Logger.logInternal.info("Native interface found:"+hal['name'])
            continue
...
def append_provide_info(parsed_xml_data, provider):
    ...
    if len(provide_data) == 0:
        ...
        if not isinstance(provide_data['manifest']['hal'],list):
            provide_data['manifest']['hal']=list(provide_data['manifest']['hal'])
        return
...
def run_img_to_img_checker(qssi_path,
    ...
    Logger.logStdout.info(qssi_path)
    qssi_json_cmd_path =''
    ...
    if UtilFunctions.dirExists(qssi_json_cmd_path) and UtilFunctions.dirExists(target_json_cmd_path):
        ...
        if qssi_cmds == None or vndr_cmd == None:
            ...
            img_to_img_check(m_qssi_cmds, vndr_cmd.get_cmd_data(), vndr_cmd_ver, qssi_json_cmd_path)
    else:
...
def hidl_checker_main(self,
    ...
    if flag == "check":
        if qssi_path != None and target_path != None:
            ...
            if not Constants.qiifa_out_path_target_value == "qssi":
...
class DataParsingHelper:
    ...
    def get_cmd_name(self):
        ...
        if self.__level_val != "":
            return Constants.qiifa_cmd_prefix + '_' + str(FCM_GLOBAL_DICT[self.__level_val]) + Constants.json_file_suffix
    ...
    def get_raw_xml_data(self):
        ...
        try:
            ...
            reason = "Please verify the XML File, invalid syntax has been used."
            Logger.logStdout.error(e)
            UtilFunctions.print_violations_on_stdout("HIDL Plugin",fd,get_raw_xml_data.__name__,reason)
            sys.exit(1)
    ...
    def get_cmd_data(self):
        ...
        if not UtilFunctions.fileExists(cmd_file_path):
            if (self.__level_val=="current"):
                Logger.logStdout.warning("The cmd file " + self.get_cmd_name() + " doesn't exist. Make sure it exists before proceeding.")
                return {};
            else:
