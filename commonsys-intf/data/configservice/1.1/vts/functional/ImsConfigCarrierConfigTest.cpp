...
*****************************************************************************/
#include "ImsConfigCarrierConfigTest.h"

std::string ImsConfigCarrierConfigTest::compressFileData(bool compressData)
{
  ...
  ALOGI("ImsConfigCarrierConfigTest : original file size %d", size);
  ...
  ALOGI("ImsConfigCarrierConfigTest : Copied %d characters", numCopied);
...
TEST_F(ImsConfigCarrierConfigTest, SetConfigTest) {
  ConfigData data;
  string buffer;
  ...
  if(pService != nullptr) {
    auto result = pService->setConfig(data, userData);
